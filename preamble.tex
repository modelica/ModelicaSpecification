...
% Remove cross as red
% Note: have basicstyle=\small\ttfamily in all languages,
...
\lstdefinelanguage{modelica}{% Language for use with the lstlisting environment.
    alsoletter={...},%
    basicstyle=\small\ttfamily,   % Font size for displayed listings.
    %otherkeywords={-, =, +, [, ], (, ), \{, \}, :, *, !},%
...
\lstdefinelanguage[short]{modelica}[]{modelica}{% Language for use with the \lstinline command.
    basicstyle=\ttfamily,         % Font size for inline code snippets.
}
...
% Introduce \nonnormative{....} to mark non-normative text.
% Text is in emph-style surrounded by []
%
% An optional argument can be used to only start the block with [ (s) and/or end it(e) with ]
% So \nonnormative[s]{a} \nonnormative[]{b} \nonnormative[e]{c} should create [\emph{abc}]
% which normally would be just \nonnormative{abc}
% But don't use it unless needed.
% A reason for using these arguments is to avoid having listings as part of the nonnormative-text since it seems messy.
% Similarly having environments using emph is a bit tricky
