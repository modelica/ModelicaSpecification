...
% Rudimentary workaround for not being able to use \lstinline in the index.
\newcommand{\indexinline}[1]{\texttt{#1}}
...
\newcommand{\willintroduce}[1]{\emph{#1}}
...
\newtheoremstyle{modelicadefinition}
  ...
  {\\}        % HEADPUNCT
  {.75em}     % HEADSPACE
  {#1~#2 \texttt{#3}\label{modelica:#3}\index{#3@\indexinline{#3}}} % CUSTOM-HEAD-SPEC {#1~#2. \texttt{#3}}
