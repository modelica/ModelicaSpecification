...
Default icon graphics, #3624

### Documentation

Stylesheets are supported, #3409
Replace modelica:// by modelica: #3254
Introduce EllipseClosure.Default #3655

### Plotting

Logarithmic axis support, #3297
Generalize vendor-specific markup in figures, #3630
Define order within legend and plotting order, #3483
...
Connector classes don't have to be balanced, only used connector components, #3356
Require evaluable ranges and conditions around 'connect', #3582

### Clocked

Real interval clock is now similar to rational interval clock (bug fix), #3754
...
Generalize to allow Integer flow, #3408
General record member referencing, #3516
Generalize stream connectors to records, #3358
size(A,j) defined as syntactic sugar for size(A)[j] #3792
Define time as keyword, #3797
Inherit experiment annotation, #2535
Solvability for non-real equations, #3798
spatialDistribution require that initialPoints span the range 0 to 1 (bug fix), #3648
While loop event generation (bug fix), #3360
Special cases for reinit function, #3568
Define empty class and allow extends from empty base-class, #3362
Show value of variables for asserts - to avoid having to use String-operator for that, #3641
Uniform treatment of variables in initial algorithms (bug fix), #3562
