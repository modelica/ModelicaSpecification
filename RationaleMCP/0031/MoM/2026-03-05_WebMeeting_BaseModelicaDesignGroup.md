...
Hans: We as a Modelica community haven't learned anything to resolve incompatibilites.
...
Hans: If one main objective is to engage with non-Modelica vendors, then it is even more important to be a pure subset, if they consider this as a firt step towards full Modelica support.
...
Francesco: To give an example: Priorities for start values are crucial. Obviously extra information is needed to express it. How to introduce it that's a matter of the preferred design but solveable?
...
Oliver: Can we agree on the following key strategic objective as a justifcation to proceed towards a standardization:
...
Dag: There is a cost of maintainig the specifications and keeping them aligned.
...
- Hauke: We're still interested. If Base Modelica had been existing, this would have been our preferred approach. If there is a standard comming this will be a strong reason to consider Base Modelica for future work.
- Christian: I would recommend to contiue and work towards a version 0.1. Experience from FMI is that having at least a alpha version out is crucial to get support to do more. Having a road map and an alpha version is a good signal.

Oliver: To sumarize this feedback I see support for a future series of design meetings to work towards a first release. This should start with defining a milestone and roadmap and continue with considering technical proposals. I'll setup a poll for a first meeting.
...
### Pros and cons of Base Modelica being a pure subset
Post-poned to the first Base Modelica Design Meetings.
