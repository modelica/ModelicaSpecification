...
\item
  ...
  \item
\begin{lstlisting}[language=modelica]
    end in A[...end... ] if A is variable declared in a
    non-function class.
\end{lstlisting}
  \item
...
\begin{lstlisting}[language=modelica]
  ...
  connect(c , d) ; // c is an outside
  connector, d  is an outside connector
\end{lstlisting}
\emph{and in the model for component m6:}

\begin{lstlisting}[language=modelica]
  connect(d, m7.c); // d is an outside
...
\subsection{Flexible Array Sizes}\doublelabel{flexible-array-sizes}
...
\subsection{Built-in Array Functions}\doublelabel{built-in-array-functions}
...
\begin{lstlisting}[language=modelica]
  B[i,j] := A[i,j], if i <= j, B[i,j] := A[j,i],
  if i > j.
\end{lstlisting}
\end{minipage}\\ \hline
\begin{minipage}[t]{0.47\columnwidth}\raggedright
...
\subsection{Vector, Matrix and Array Constructors}\doublelabel{vector-matrix-and-array-constructors}
...
\begin{lstlisting}[language=modelica]
  ...
  a[2].m[1], ...}, and does not require that
...
\begin{lstlisting}[language=modelica]
for i in 1:10 loop // i takes the values
  1,2,3,...,10
  for r in 1.0 : 1.5 : 5.5 loop // r takes the
    values 1.0, 2.5, 4.0, 5.5
    for i in {1,3,6,7} loop // i takes the
      values 1, 3, 6, 7
      for i in TwoEnums loop // i takes the values
...
\begin{lstlisting}[language=modelica]
  Real x[j];
equation
  for j in 1:j loop // The loop-variable j
\end{lstlisting}
...
\begin{lstlisting}[language=modelica]
  else 0; // May not move asin(x) out of then
algorithm
...
\begin{lstlisting}[language=modelica]
  function quadrature "Integrate function y=integrand(x) from x1
to x2"
  input Real x1;
  input Real x2;
...
\begin{lstlisting}[language=modelica]
  function quadrature2 "Integrate function y=integrand(x) from x1
to x2"
  input Real x1;
  input Real x2;
  input Integrand integrand; // Integrand is a partial function
...
\begin{lstlisting}[language=modelica]
function keyword followed by a function call to func_name
\end{lstlisting}
...
function foo
  input Real x;
  protected Real xtemp:=x; // Temporary used because myfoo
\end{lstlisting}
changes its input
...
\begin{lstlisting}[language=modelica]
  ...
  import Modelica.Math.ComplexNumbers.{Add,Sub}; // Accessed by
\end{lstlisting}
Add and Sub
...
encapsulated operator function '+' // short
\end{lstlisting}
...
encapsulated operator function 'String'
  import Complex;
  input Complex c;
  input String name := "j" "Name of variable representing
...
connector FluidPort
  ...
  flow Medium.MassFlowRate m_flow "> 0, if flow
into component";
  stream Medium.SpecificEnthalpy h_outflow "h close to port if
m_flow < 0";
  stream Medium.MassFraction X_outflow[Medium.nX] "X close
to port if m_flow < 0";
end FluidPort;
\end{lstlisting}
...
connector FluidPort
  ...
  flow Real m_flow "Flow of matter; m_flow > 0 if
flow into component";
  stream Real h_outflow "Specific variable in component if
m_flow < 0"
end FluidPort
...
\begin{lstlisting}[language=modelica]
if m_j.c.m_flow.min >= 0
  for all j = 1:N with j <> i
    and
    ...
    for j in cat(1,1:i-1, i+1:N) +
      ...
      *inStream(c_k.h_outflow)))/
      ...
      +
      sum(positiveMax(c_k.m_flow,s_i)))
      for j in 1:N and i <> j
        and m_j.c.m_flow.min < 0,
        for k in 1:M and c_k.m_flow.max
          > 0
...
\begin{lstlisting}[language=modelica]
if s_i > 0 then
  (s_i/eps)^2*(3-2*(s_i/eps))
  else 0);
  // Define function positiveMax(v,s_i) as a
  linear combination of max (v,0)
  // and of eps along alpha
  positiveMax((-m_j.c.m_flow,s_i)
  := alpha*max(-m_j.c.m_flow,0) +
  (1-alpha)*eps;
\end{lstlisting}
\end{minipage}\\ \hline
...
\begin{lstlisting}[language=modelica]
  ...
  y2 = subSample(u, factor=2*p -- 3); // fine (parameter
  expression)
...
\begin{lstlisting}[language=modelica]
  Clock c = Clock(angle > 0, 0.1) // before
\end{lstlisting}
first tick of c:
...
\begin{lstlisting}[language=modelica]
  Clock y2 = superSample(y1,2); // fine; y2 =
  subSample(u,2)
