...
homotopy(actual=actual,\linebreak[4]
...
x=x,\linebreak[4]
positiveVelocity=\ldots{},\linebreak[4]
initialPoints=\ldots{},\linebreak[4]
initialValues=\ldots{}) & 
...
\item
  ...
  \item
    end in A{[}\ldots{}end\ldots{} {]} if A is variable declared in a
    non-function class.
  \item
...
The rules for components in functions are described in \ref{function-as-specialized-class}.
...
\textbf{function} & See \ref{function-as-specialized-class} for restrictions
...
\textbf{end} foo;'' is conceptually treated as\linebreak[4]
...
\ref{equation-operators-for-overconstrained-equation-systems}.
...
\chapter{Scoping, Name Lookup, and Flattening}
...
\hypertarget{recursive-instantiation-of-components}{%
\subparagraph{Recursive instantiation of
components}\label{recursive-instantiation-of-components}}
...
present in B}{]}.\\ \hline
restricted subtype or\linebreak[4]
...
function subtype or\linebreak[4]
...
\chapter{Inheritance, Modification, and Redeclaration}
...
\hline& \textbf{Base}&\textbf{Class}\\ \hline
...
class\strut
\end{minipage}\\ \hline
package & yes & & & & & & & & & & & yes\\ \hline
operator & & yes & & & & & & & & & & yes\\ \hline
function & & & yes & & & & & & & & & yes\\ \hline
\vtop{\hbox{\strut operator}\hbox{\strut function}} & & & yes & yes & &
& & & & & & yes\\ \hline
type & & & & & yes & & & & & & & yes\\ \hline
...
yes\strut
\end{minipage}\\ \hline
connector & & & & & yes & yes & yes & yes & yes & & & yes\\ \hline
block & & & & & & yes & & & & yes & & yes\\ \hline
model & & & & & & yes & & & & yes & yes & yes\\ \hline
class & & & & & & & & & & & & yes\\ \hline
\end{longtable}
...
\textbf{package} PowerTrain // library from someone else
\begin{lstlisting}[language=modelica]
...
\textbar{} connect-clause

\textbar{} when-equation

\textbar{} component-reference function-call-args )
...
\subsection{Simple Equality Equations}
...
\begin{itemize}
\item
  (\textbf{if} pred \textbf{then} alt1 \textbf{else} alt2) = expr2;
\item
...
\textbf{for} for-indices \textbf{loop}
...
IDENT {[} \textbf{in} expression {]}

The following is one example of a prefix of a for-equation:

\textbf{for} IDENT \textbf{in} expression \textbf{loop}
...
\textbf{for} i \textbf{in} 1:10 \textbf{loop} // i takes the values
1,2,3,...,10

\textbf{for} r \textbf{in} 1.0 : 1.5 : 5.5 \textbf{loop} // r takes the
values 1.0, 2.5, 4.0, 5.5

\textbf{for} i \textbf{in} \{1,3,6,7\} \textbf{loop} // i takes the
values 1, 3, 6, 7

\textbf{for} i \textbf{in} TwoEnums \textbf{loop} // i takes the values
...
\textbf{constant} Integer j=4;

Real x{[}j{]};

\textbf{equation}

\textbf{for} j \textbf{in} 1:j \textbf{loop} // The loop-variable j
takes the values 1,2,3,4
...
\hypertarget{implicit-iteration-ranges-of-for-equations}{%
\paragraph{Implicit Iteration Ranges of
For-Equations}\label{implicit-iteration-ranges-of-for-equations}}
...
\textbf{for} i \textbf{loop} // Same as: for i in 1:size(x
...
to \ref{equation-operators-for-overconstrained-equation-systems}.

\subsection{If-Equations}

If-equations have the following syntax:

\textbf{if} expression \textbf{then}
...
\textbf{when} expression \textbf{then}

\{ equation ";" \}

\{ \textbf{elsewhen} expression \textbf{then}
...
\textbf{when} x \textgreater{} 2 \textbf{then}
...
\hypertarget{defining-when-equations-by-if-expressions-in-equality-equations}{%
\paragraph{Defining When-Equations by If-Expressions in Equality
Equations}\label{defining-when-equations-by-if-expressions-in-equality-equations}}
...
Boolean b(start=x.start\textgreater{}2);

\textbf{equation}

b = x\textgreater{}2;

v1 = \textbf{if} edge(b) \textbf{then} expr1 \textbf{else} pre(v1);

v2 = \textbf{if} edge(b) \textbf{then} expr2 \textbf{else} pre(v2);

{[}\emph{The equivalence is conceptual since pre() of a non
discrete-time Real variable or expression can only be used within a
when-clause. Example:}

\textbf{/* discrete*/ Real} x;

\textbf{input} \textbf{Real} u;

\textbf{output Real} y;\\
equation

\textbf{when} sample() \textbf{then}
...
\textbf{when} y1 \textgreater{} 3 \textbf{then}
...
\textbf{when} \emph{condition} \textbf{then}
...
\textbf{model} DoubleWhenConflict
...
\textbf{when} condition1 \textbf{then}

close = true;\\
\textbf{end} \textbf{when};

\textbf{when} condition2 \textbf{then}
...
\textbf{elsewhen} condition2 \textbf{then}
...
\textbf{der}(v) = \textbf{if} flying \textbf{then} -g \textbf{else} 0;

flying = \textbf{not}(h\textless{}=0 \textbf{and} v\textless{}=0);

\textbf{when} h \textless{} 0 \textbf{then}

reinit(v, -e*\textbf{pre}(v));\\
\textbf{end when;}
...
\textbf{assert}(T \textgreater{} 250 \textbf{and} T \textless{} 400,
"Medium model outside full accuracy range",\\
AssertionLevel.warning);

\textbf{assert}(T \textgreater{} 200 \textbf{and} T \textless{} 500,
"Medium model outside feasible region");
...
\textbf{when} y\textless{}0 \textbf{then}

terminate("The ball touches the ground");

\textbf{end} \textbf{when};

\textbf{end} ThrowingBall;

{]}

\subsection{Equation Operators for Overconstrained Connection-Based Equation Systems}

See \ref{equation-operators-for-overconstrained-equation-systems} for a description of this topic.
...
y = \textbf{if} u \textgreater{} uMax \textbf{then} uMax \textbf{else}
\textbf{if} u \textless{} uMin \textbf{then} uMin \textbf{else} u;
...
time \textgreater{}= discrete expression

time \textless{} discrete expression
...
x = \textbf{if} noEvent(u \textgreater{} uMax) \textbf{then} uMax
\textbf{elseif} noEvent(u \textless{} uMin) \textbf{then} uMin
\textbf{else} u;

y = noEvent( \textbf{if} u \textgreater{} uMax \textbf{then} uMax
\textbf{elseif} u \textless{} uMin \textbf{then} uMin \textbf{else} u);

z = smooth(0, \textbf{if} u \textgreater{} uMax \textbf{then} uMax
\textbf{elseif} u \textless{} uMin \textbf{then} uMin \textbf{else} u);
...
\textgreater{}= 0) then sqrt(x) else 0.{]}
...
Limit1 = noEvent(x1 \textgreater{} 1);

// Error since Limit1 is a discrete-time variable

\textbf{when} noEvent(x1\textgreater{}1) \textbf{or} x2\textgreater{}10
\textbf{then}
...
\textbf{equation} // Illegal example
...
\textbf{when} fastSample \textbf{then}

ticks := \textbf{if} pre(ticks) \textless{} 5 \textbf{then} pre(ticks)+1
\textbf{else} 0;
...
\textbf{when} fastSample \textbf{then} // fast sampling
...
\textbf{when} slowSample \textbf{then} // slow sampling (5-times slower)
...
\textbf{parameter} Boolean steadyState = true;

\textbf{parameter} Real y0 = 0 "start value for y, if not steadyState";
...
\textbf{if} steadyState \textbf{then}
...
Real y (start=0, fixed=\textbf{not} steadyState);

Real der\_y(start=0, fixed=steadyState) = \textbf{der}(y);
...
\textbf{when} \{\textbf{initial}(), sampleTrigger\} \textbf{then}
...
\textbf{connector} InPort = \textbf{input} Real;

\textbf{connector} OutPort = \textbf{output} Real;

\textbf{block} MatrixGain

\textbf{input} InPort u{[}size(A,2){]};

\textbf{output} OutPort y{[}size(A,1){]};

\textbf{parameter} Real A{[}:,:{]} = {[}1{]};

\textbf{equation}

y=A*u;

\textbf{end} MatrixGain;
...
\textbf{connect}(sinSource.y, gain.u); // Legal

\textbf{connect}(gain.y, gain2.u); // Legal

\textbf{connect}(gain2.y, x); // Legal
...
following} connect \emph{equations found in the model for component m0:}

\textbf{connect}(m1.c, m3.c); // m1.c and m3.c are inside connectors

\textbf{connect}(m2.c, m3.c); // m2.c and m3.c are inside connectors

\emph{and in the model for component m3 (c.x is a sub-connector inside
c):}

\textbf{connect}(c, m4.c); // \textbf{c} is an \textbf{outside}
connector, m4.c is an inside connector

\textbf{connect}(c.x, m5.c); // \textbf{c.x} is an \textbf{outside}
connector, m5.c is an inside connector

\textbf{connect}(c , d) ; // \textbf{c} is an \textbf{outside}
connector, \textbf{d} is an \textbf{outside} connector

\emph{and in the model for component m6:}

\textbf{connect}(d, m7.c); // \textbf{d} is an \textbf{outside}
connector, m7.c is an inside connector
...
\textbf{expandable} \textbf{connector} EngineBus
...
\textbf{connect}(bus.speed, sensor.speed); // provides the non-input
from sensor.speed

\textbf{connect}(bus.speed, actuator.speed);
...
\textbf{expandable} \textbf{connector} EngineBus // has predefined
signals

\textbf{import} SI=Modelica.SIunits;
...
\textbf{connect}(bus.speed, sensor.speed);
...
\textbf{import} Interfaces=Modelica.Electrical.Analog.Interfaces;

\textbf{expandable} \textbf{connector} ElectricalBus

Interfaces.PositivePin p12, n12; // OK

\textbf{flow} Modelica.SIunits.Current i; // not allowed

\textbf{end} ElectricalBus;
...
\textbf{connect}(p42, bus.p42); // Adds new electrical pin

\textbf{connect}(n42, bus.n42); // Adds another pin
...
\textbf{expandable} \textbf{connector} EmptyBus
...
\textbf{connect}(speed, bus1.speed); // ok, only one undeclared

// and it is unsubscripted

\textbf{connect}(bus1.pressure, bus2.pressure);

// not allowed, both undeclared

\textbf{connect}(speed, bus2.speed{[}2{]});

// introduces speed array (with element {[}2{]}).

\textbf{end} Controller;
...
\emph{1) Expandable connectors nested hierarchically. This means that
both outside and inside connectors must be included at every level of
the hierarchy in this elaboration process.}

\emph{2) When processing an expandable connector that possesses the}
...
\textbf{import} Modelica.Blocks.Interfaces.RealInput;

// Plant Side

\textbf{model} SparkPlug

RealInput spark\_advance;
...
\textbf{expandable} \textbf{connector} CylinderBus

Real spark\_advance;

\textbf{end} CylinderBus;
...
\textbf{connect}(spark\_plug.spark\_advance,
cylinder\_bus.spark\_advance);
...
Modelica.Thermal.HeatTransfer.Sensors.TemperatureSensor temp\_sensor;
...
\textbf{connect}(speed\_sensor.w, engine\_bus.engine\_speed);

// adds engine\_temp (as output)

\textbf{connect}(temp\_sensor.T, engine\_bus.engine\_temp);

// adds cylinder\_bus1 (a nested bus)

for i in 1:nCylinder loop

connect(cylinder{[}i{]}.cylinder\_bus,
engine\_bus.cylinder\_bus{[}i{]});
...
\item
  \begin{quote}
  z1 + z2 + (-z3) + ... + zn = \textbf{0}; // flow-variables
  \end{quote}
\end{itemize}
...
\textbf{connect}(resistor.p, ground.p);

\textbf{end} Circuit;

\textbf{model} Load

\textbf{extends} TwoPin;

Resistor resistor;\\
\textbf{equation}\\
\textbf{connect}(p, resistor.p);\\
\textbf{connect}(resistor.n, n);\\
\textbf{end} Load;
...
\item
  ...
  block:\\
  (1) the connection set includes variables from an outside public
  expandable connector, (2) the set contains variables from protected
  outside connectors, (3) it contains variables from one inside output
  connector, or (4) from one public outside input connector, or (5) the
  set is comprised solely of one variable from one inside input
  connector that is not part of an expandable connector. {[}\emph{i.e.,
  a connection set must -- unless the model or block is partial -
  contain one source of a signal (the last item (5) covers the case
...
\begin{quote}
\textbf{connector} Pin // a physical connector of
...
\textbf{connector} Plug // a hierarchical connector of
Modelica.Electrical.MultiPhase

\textbf{parameter} Integer m=3;

Pin p{[}m{]};

\textbf{end} Plug;

\textbf{connector} InputReal \textbf{= input} Real; // A causal input
connector

\textbf{connector} OutputReal = \textbf{output} Real; // A causal output
connector

\textbf{connector} Frame\_Illegal
...
\textbf{flow} Modelica.SIunits.Force f{[}3{]} "Cut force";

\textbf{flow} Modelica.SIunits.Torque t{[}3{]} "Cut torque";

\textbf{end} Frame\_Illegal;
\end{quote}
...
\begin{quote}
\textbf{connector} Frame\_a "correct connector"

\textbf{input} Modelica.SIunits.Position r0{[}3{]};

\textbf{input} Real S{[}3, 3{]};

\textbf{input} Modelica.SIunits.Velocity v{[}3{]};

\textbf{input} Modelica.SIunits.AngularVelocity w{[}3{]};

Modelica.SIunits.Acceleration a{[}3{]};

Modelica.SIunits.AngularAcceleration z{[}3{]};

\textbf{flow} Modelica.SIunits.Force f{[}3{]};

\textbf{flow} Modelica.SIunits.Torque t{[}3{]};

\textbf{end} Frame\_a;

\textbf{connector} Frame\_b "correct connector"

\textbf{output} Modelica.SIunits.Position r0{[}3{]};

\textbf{output} Real S{[}3, 3{]};

\textbf{output} Modelica.SIunits.Velocity v{[}3{]};

\textbf{output} Modelica.SIunits.AngularVelocity w{[}3{]};
...
\begin{quote}
\textbf{connector} Plug\_Expanded "correct connector"
...
\textbf{end} Plug\_Expanded;

\textbf{connector} Plug\_Expanded2 "correct connector"

\textbf{parameter} Integer m=3;

\textbf{final parameter} Integer n=m;

Real v{[}m{]};

\textbf{flow} Real i{[}n{]};

\textbf{end} Plug\_Expanded2;

\textbf{connector} Plug\_Expanded\_Illegal "connector is illegal"

\textbf{parameter} Integer m=3;

\textbf{parameter} Integer n=m;

Real v{[}m{]};

\textbf{flow} Real i{[}n{]};

\textbf{end} Plug\_Expanded\_Illegal;
\end{quote}
...
\textbf{type} Type // overdetermined type

\textbf{extends} \textless{}base type\textgreater{};

\textbf{function} equalityConstraint // non-redundant equality

\textbf{input} Type T1;

\textbf{input} Type T2;

\textbf{output} Real residue{[} \textless{}n\textgreater{} {]};

\textbf{algorithm}

residue := ...

\textbf{end} equalityConstraint;

\textbf{end} Type;

\textbf{record} Record

\textless{} declaration of record fields\textgreater{}

\textbf{function} equalityConstraint // non-redundant equality

\textbf{input} Record R1;

\textbf{input} Record R2;
...
\textbf{0} = Record.equalityConstraint(R1,R2);
...
\vtop{\hbox{\strut b = Connections.rooted(A.R);}\hbox{\strut b =
rooted(A.R); // deprecated}} & If the operator Connections.rooted(A.R)
...
\textbf{type} AC\_Angle "Angle of source, e.g., rotor of generator"

\textbf{extends} Modelica.SIunits.Angle; // AC\_Angle is a Real number

// with unit = "rad"

\textbf{function} equalityConstraint

\textbf{input} AC\_Angle theta1;

\textbf{input} AC\_Angle theta2;

\textbf{output} Real residue{[}0{]} "No constraints"
...
assert(abs(theta1 -- theta2) \textless{} 1.e-10, "Consistent angles");

\textbf{end} equalityConstraint;

\textbf{end} AC\_Angle;

\textbf{connector} AC\_Plug "3-phase alternating current connector"
...
\textbf{flow} SI.Current i{[}3{]} "Currents resolved in AC\_Angle
frame";
...
\textbf{model} AC\_Inductor

\textbf{parameter} Real X{[}3,3{]}, Y{[}3,3{]}; // component constants
...
omega = \textbf{der}(p.theta); // frequency of source

zeros(3) = p.i + n.i;

X*\textbf{der}(p.i) + omega*Y*p.i = p.v -- n.v;

\textbf{end} AC\_Inductor
...
\textbf{der}(p.theta) = 2*Modelica.Constants.pi*50 // 50 Hz;
...
\textbf{type} TransformationMatrix = Real{[}3,3{]};

\textbf{type} Orientation "Orientation from frame 1 to frame 2"

\textbf{extends}TransformationMatrix;

\textbf{function} equalityConstraint

\textbf{input} Orientation R1 "Rotation from inertial frame to frame 1";

\textbf{input} Orientation R2 "Rotation from inertial frame to frame 2";

\textbf{output} Real residue{[}3{]};
...
\textbf{end} Orientation;

\textbf{connector} Frame "3-dimensional mechanical connector"
...
\textbf{flow} SI.Force f{[}3{]} "Cut-force resolved in Frame";

\textbf{flow} SI.Torque t{[}3{]} "Cut-torque resolved in Frame";
...
\textbf{model} FixedTranslation

\textbf{parameter} Modelica.SIunits.Position r{[}3{]};
...
Table 10.1. \textbf{General forms of declaration of arrays.}
...
\textbf{type} TwoEnums = \textbf{enumeration}(one,two);
...
Table 10.2. \textbf{Declaration of arrays as 1-vectors, row-vectors, or
...
\begin{quote}
\emph{Modelica form 1}
\end{quote}\strut
\end{minipage} & \begin{minipage}[t]{0.17\columnwidth}\raggedright
\begin{quote}
\emph{Modelica form 2}
\end{quote}\strut
\end{minipage} & \begin{minipage}[t]{0.17\columnwidth}\raggedright
\begin{quote}
\emph{\# dimensions}
...
\textbf{type} Voltage = Real(unit = "V");

\textbf{type} Current = Real(unit = "A");

\textbf{connector} Pin

Voltage v; // type class of v = Voltage, type of v = Real

\textbf{flow} Current i; // type class of i = Current, type of i = Real

\textbf{end} Pin;

\textbf{type} MultiPin = Pin{[}5{]};

MultiPin{[}4{]} p; // type class of p is MultiPin, type of p is
Pin{[}4,5{]};

\textbf{type} Point = Real{[}3{]};
...
\subsection{Flexible Array Sizes}
...
Table 10.3. \textbf{Promote function (cannot be used in Modelica).}
...
for 1 \textless{}= j \textless{}= nA, size(C,j) = 1 for nA+1
\textless{}= j \textless{}= n, C{[}i\_1, ..., i\_nA, 1, ..., 1{]} =
A{[}i\_1, ..., i\_nA{]}\\ \hline
...
\ref{array-dimension-and-size-function} to \ref{matrix-and-vector-algebra-functions}}:
...
Table 10.4. \textbf{Built-in array dimension and size functions.}
...
\textgreater{} 0 and \textless{}= ndims(A).\strut
\end{minipage}\\ \hline
\begin{minipage}[t]{0.47\columnwidth}\raggedright
\begin{quote}
size(A)
...
Table 10.5. \textbf{Built-in dimensionality conversion functions.}
...
\end{quote}\strut
\end{minipage} & \begin{minipage}[t]{0.47\columnwidth}\raggedright
Returns the single element of array A. size(A,i) = 1 is required for 1
\textless{}= i \textless{}= ndims(A).\strut
\end{minipage}\\ \hline
\begin{minipage}[t]{0.47\columnwidth}\raggedright
\begin{quote}
vector(A)
...
dimension size \textgreater{} 1.\strut
\end{minipage}\\ \hline
\begin{minipage}[t]{0.47\columnwidth}\raggedright
\begin{quote}
matrix(A)
...
required for 2 \textless{} i \textless{}= ndims(A).\strut
\end{minipage}\\ \hline
...
Table 10.6. \textbf{Specialized array constructor functions.}
...
zeros at the other places.\strut
\end{minipage}\\ \hline
\begin{minipage}[t]{0.47\columnwidth}\raggedright
\begin{quote}
diagonal(v)
...
and all other elements zero.\strut
\end{minipage}\\ \hline
\begin{minipage}[t]{0.47\columnwidth}\raggedright
\begin{quote}
zeros($n_1$,$n_2$,$n_3$,...)
...
v=linspace(x1,x2,n),\\
v{[}i{]} = x1 + (x2-x1)*(i-1)/(n-1) for 1 \textless{}= i \textless{}= n.
It is required that n \textgreater{}= 2. The arguments x1 and x2 shall
be numeric scalar expressions.\strut
\end{minipage}\\ \hline
...
Table 10.7. \textbf{Array reduction functions and operators.}
...
A{[}1,...,1{]}+A{[}2,...,1{]}+....+A{[}end,...,1{]}+A{[}end,...,end{]}\strut
\end{minipage}\\ \hline
\begin{minipage}[t]{0.47\columnwidth}\raggedright
\begin{quote}
sum(\\
...
The type of sum(e(i, ..., j) \textbf{for} i \textbf{in} u, ..., j
\textbf{in} v) is the same as the type of e(i,...j).\strut
\end{minipage}\\ \hline
\begin{minipage}[t]{0.47\columnwidth}\raggedright
\begin{quote}
product(A)
...
The type of product(e(i, ..., j) \textbf{for} i \textbf{in} u, ..., j
\textbf{in} v) is the same as the type of e(i,...j).\strut
\end{minipage}\\ \hline
...
Table 10.8. \textbf{Reduction expressions with iterators.}

\begin{longtable}[]{@{}lll@{}}
\hline
\endhead
\emph{Function-name} & \emph{Restriction on expression1} & \emph{Result
if expression2 is empty}\\ \hline
...
type ( Modelica.Constants.inf for Real)\\ \hline
max & Scalar enumeration, Boolean, Integer or Real &
\vtop{\hbox{\strut Least value of type
(}\hbox{\strut -Modelica.Constants.inf for Real)}}\\ \hline

\end{longtable}

{[}\emph{Example}:

sum(i \textbf{for} i \textbf{in} 1:10) // Gives 1+2+...+10=55

// Read it as: compute the sum of i \textbf{for} i in the range 1 to 10.

sum(i\^{}2 \textbf{for} i \textbf{in} \{1,3,7,6\}) // Gives 1+9+49+36=95

\{product(j \textbf{for} j \textbf{in} 1:i) \textbf{for} i \textbf{in}
0:4\} // Gives \{1,1,2,6,24\}

max(i\^{}2 \textbf{for} i \textbf{in} \{3,7,6\}) // Gives 49
...
Table 10.9. \textbf{Matrix and vector algebra functions.}
...
does not have at least 2 dimensions.\strut
\end{minipage}\\ \hline
\begin{minipage}[t]{0.47\columnwidth}\raggedright
\begin{quote}
outerProduct(v1,v2)
...
above the diagonal of A, i.e., B := symmetric(A) -\textgreater{}
B{[}i,j{]} := A{[}i,j{]}, if i \textless{}= j, B{[}i,j{]} := A{[}j,i{]},
if i \textgreater{} j.\strut
\end{minipage}\\ \hline
\begin{minipage}[t]{0.47\columnwidth}\raggedright
\begin{quote}
cross(x,y)
...
x{[}3{]}*y{[}1{]}-x{[}1{]}*y{[}3{]}; x{[}1{]}*y{[}2{]}-x{[}2{]}*y{[}1{]}
{]} );\strut
\end{minipage}\\ \hline
\begin{minipage}[t]{0.47\columnwidth}\raggedright
\begin{quote}
...
i.e., cross(x,y) = skew(x)*y; skew(x) = {[}0, -x{[}3{]}, x{[}2{]};
x{[}3{]}, 0, -x{[}1{]}; -x{[}2{]}, x{[}1{]}, 0{]};\strut
\end{minipage}\\ \hline

\end{longtable}

\subsection{Vector, Matrix and Array Constructors}
...
\textbf{type} Angle = Real(unit="rad");

\textbf{parameter} Angle alpha = 2.0; // type of alpha is Real.
...
"\{" expression \textbf{for} iterators "\}"

or

array "(" expression \textbf{for} iterators ")"
...
IDENT \textbf{in} array\_expression
...
array(i \textbf{for} i \textbf{in} 1:10)

// Gives the vector 1:10=\{1,2,3,...,10\}

\{r \textbf{for} r \textbf{in} 1.0 : 1.5 : 5.5\}

// Gives the vector 1.0:1.5:5.5=\{1.0, 2.5, 4.0, 5.5\}

\{i\^{}2 \textbf{for} i \textbf{in} \{1,3,7,6\}\}
...
Real hilb{[}:,:{]}= \{ 1/(i+j-1) \textbf{for} i \textbf{in} 1:n, j
\textbf{in} 1:n\};

Real hilb2{[}:,:{]}=\{\{ 1/(i+j-1) \textbf{for} j \textbf{in} 1:n\}
...
R{[}i\_1, ..., i\_k, ..., i\_n{]} = A{[}i\_1, ..., i\_k, ..., i\_n{]},
for i\_k \textless{}= size(A,k),

R{[}i\_1, ..., i\_k, ..., i\_n{]} = B{[}i\_1, ..., i\_k - size(A,i),
..., i\_n{]}, for i\_k \textless{}= size(A,k) + size(B,k),

....

where 1 \textless{}= i\_j \textless{}= size(R,j) for 1 \textless{}= j
\textless{}= n.
...
\item
  ...
  ...) where\\
  n = max(2, ndims(A), ndims(B), ndims(C), ....). If necessary, 1-sized
...
\item
  ...
  {[}...;...{]}, e.g., {[}a, b; c, d{]} is parsed as {[}{[}a,b{]};
  {[}c,d{]}{]}.
\item
  {[}A{]} = promote(A,max(2,ndims(A))), i.e., {[}A{]} = A, if A has 2 or
...
Colors = \textbf{enumeration}(red,blue,green);
...
{[}\emph{Examples given the declaration} x{[}n,m{]}, v{[}k{]},
z{[}i,j,p{]}:

Table 10.10. \emph{\textbf{Examples of scalars vs. array slices created
with the colon index.}}

\begin{longtable}[]{@{}lll@{}}
\hline
\endhead
\emph{Expression} & \emph{\# dimensions} & \emph{Type of
value}\\ \hline
...
\textbf{type} ShirtSizes \textbf{= enumeration}(small, medium, large,
xlarge);
...
A{[}\textbf{end}-1,\textbf{end}{]} is A{[}size(A,1)-1,size(A,2){]}

A{[}v{[}\textbf{end}{]},\textbf{end}{]} is
...
Table 10.11. \textbf{Equality and assignment of arrays and scalars.}

\begin{longtable}[]{@{}llll@{}}
\hline
\endhead
\emph{Type of a} & \emph{Type of b} & \emph{Result of} a = b &
\emph{Operation} (j=1:n, k=1:m)\\ \hline
...
Table 10.12. \textbf{Array addition, subtraction, and string
concatenation.}
...
Table 10.13. \textbf{Array element-wise addition, subtraction, and
...
Table 10.14. \textbf{Unary operators. The element-wise (.+, .-) and
normal (+, -) operators give the same results.}
...
Table 10.14. \textbf{Scalar and scalar to array multiplication of
numeric elements}
...
Table 10.15. \textbf{Array element-wise multiplication}
...
Table 10.16. \textbf{Matrix and vector multiplication of arrays with
numeric elements.}
...
Table 10.17. \textbf{Division of scalars and arrays by numeric
elements.}
...
Table 10.18. \textbf{Element-wise division of arrays}
...
Table 10.19. \textbf{Element-wise exponentiation of arrays}
...
\item
  ...
  size(a{[}1{]}.m)=size(a{[}2{]}.m)=\ldots{}
\item
  The slicing operation can be combined with indexing, e.g. a.m{[}1{]}.
  It returns the array of components \{a{[}1{]}.m{[}1{]},
  a{[}2{]}.m{[}1{]}, \ldots{}\}, and does not require that
  size(a{[}1{]}.m)=size(a{[}2{]}.m). The number of subscripts on m must
  ...
  to be ":"), and is only valid if
  size(a{[}1{]}.m{[}\ldots{}{]})=size(a{[}2{]}.m{[}\ldots{}{]})..
\end{itemize}
...
// size(tf{[}1{]}.x1{[}:{]})=size(tf{[}2{]}.x1{[}:{]})=\ldots{}
=size(tf{[}m{]}.x1{[}:{]})
...
// size(tf{[}1{]}.x1{[}1:2{]})=size(tf{[}2{]}.x1{[}1:2{]})=\ldots{}
=size(tf{[}m{]}.x1{[}1:2{]})=\{2\}
...
\chapter{Statements and Algorithm Sections}
...
"(" output-expression-list ")" ":=" component-reference
function-call-args
...
Real xsquared{[}:{]}=\{x{[}i{]}*x{[}i{]} \textbf{for} i\};
...
\textbf{for} i \textbf{loop} // Same as: for i in 1:size(x,1) loop ...
...
\textbf{type} FourEnums=\textbf{enumeration}(one,two,three,four);

Real xe{[}FourEnums{]}= x;

Real xsquared3{[}FourEnums{]}=\{xe{[}i{]}*xe{[}i{]} \textbf{for} i\};

Real xsquared4{[}FourEnums{]}=\{xe{[}i{]}*xe{[}i{]} \textbf{for} i
\textbf{in} FourEnums\};

Real xsquared5{[}FourEnums{]}=\{x{[}i{]}*x{[}i{]} \textbf{for} i\};
...
Real xsquared2{[}FourEnums{]}=\{xe{[}i{]}*xe{[}i{]} \textbf{for} i
\textbf{in} FourEnums\};

\textbf{equation}

\textbf{for} i in FourEnums \textbf{loop}
...
\textbf{for} j, i \textbf{in} 1:2 \textbf{loop}
...
\textbf{while} expression \textbf{loop}
...
\ref{break-statement}, or a return-statement, see \ref{return-statement}, is executed),
...
\textbf{function} findValue "Returns position of val or 0 if not found"

\textbf{input} Integer x{[}:{]};

\textbf{input} Integer val;

\textbf{output} Integer index;

\textbf{algorithm}

index := size(x,1);

while index \textgreater{}= 1 \textbf{loop}

\textbf{if} x{[}index{]}== val \textbf{then}
...
Can only be used inside functions, see \ref{function-return-statement}.
...
\textbf{when} \{x \textgreater{} 2, sample(0,2), x \textless{} 5\}
\textbf{then}
...
\textbf{when} \{x\textgreater{}1, ..., y\textgreater{}p\} \textbf{then}

...

\textbf{elsewhen} x \textgreater{} y.start \textbf{then}
...
Boolean b1{[}N{]}(start=\{x.start\textgreater{}1, ...,
y.start\textgreater{}p\});

Boolean b2(start=x.start\textgreater{}y.start);

\textbf{algorithm}

b1:=\{x\textgreater{}1, ..., y\textgreater{}p\};

b2:=x\textgreater{}y.start;

\textbf{if} edge(b1{[}1{]}) \textbf{or} edge(b1{[}2{]}) \textbf{or} ...
edge(b1{[}N{]}) \textbf{then}

...

\textbf{elseif} edge(b2) \textbf{then}
...
A Modelica function is a specialized class (\ref{function-as-specialized-class}) using the
...
\textbf{function} \emph{functionname}

\textbf{input} TypeI1 in1;

\textbf{input} TypeI2 in2;

\textbf{input} TypeI3 in3 := \emph{default\_expr1} "Comment"
\textbf{annotation}(...);

...

\textbf{output} TypeO1 out1;

\textbf{output} TypeO2 out2 := \emph{default\_expr2};

...

\textbf{protected}

\textless{}\emph{local variables}\textgreater{}
...
\textless{}\emph{statements}\textgreater{}

...

\textbf{end} \emph{functionname};
...
\textbf{function} \textless{}\emph{functionname}\textgreater{}

\textbf{output} TypeO1 out1; // Intermixed declarations of inputs and
outputs

\textbf{input} TypeI1 in1; // not recommended since code becomes hard to
read
...
\textbf{output} TypeO2 out2;

\textbf{input} TypeI3 in3;

...

\textbf{end} \textless{}\emph{functionname}\textgreater{};
...
\textbf{for} i \textbf{in} 1:size(x,1) \textbf{loop}

\textbf{if} x{[}i{]} == val \textbf{then}
...
\textbf{function} evaluateLinear // pure function
...
\textbf{end} evaluateLinear;

\textbf{impure function} receiveRealSignal // impure function

\textbf{input} HardwareDriverID id;

\textbf{output} Real y;

\textbf{external} "C" y = receiveSignal(id);

\textbf{end} receiveRealSignal;

\emph{Examples of allowed optimizations of pure functions:}

\textbf{model} M // Assume sin, cos, asin are pure functions with normal
derivatives.\\
\textbf{input} Real x{[}2{]};\\
\textbf{input} Real w;\\
Real y{[}2{]}={[}cos(w),sin(w);-sin(w),cos(w){]}*x;\\
Real z{[}2{]}=\textbf{der}(y);\\
Real a=0*asin(w);\\
\textbf{end} M;
...
Real x=\textbf{if} noEvent(abs(x))\textless{}1 \textbf{then} asin(x)
\textbf{else} 0; // May not move asin(x) out of then
...
\textbar{} named-arguments

named-arguments: named-argument {[} "," named-arguments {]}

named-argument: IDENT "=" function-argument

function-argument : \textbf{function} name "(" {[} named-arguments {]}
")" \textbar{} expression
...
\textbf{function} RealToString

\textbf{input} Real number;

\textbf{input} Real precision := 6 "number of significantdigits";

\textbf{input} Real length := 0 "minimum length of field";

\textbf{output} String string "number as string";

...

\textbf{end} RealToString;
...
\textbf{function} quadrature "Integrate function y=integrand(x) from x1
to x2"
...
\textbf{input} Integrand integrand; // Integrand is a partial function,
see below

// With default: \textbf{input} Integrand integrand :=
Modelica.Math.sin;

\textbf{output} Real integral;

\textbf{algorithm}

integral :=(x2-x1)*(integrand(x1) + integrand(x2))/2;

\textbf{end} quadrature;

\textbf{partial function} Integrand
...
\textbf{function} Parabola

\textbf{extends} Integrand;
...
\textbf{function} quadrature2 "Integrate function y=integrand(x) from x1
to x2"
...
\textbf{function} keyword followed by a function call to func\_name
giving named formal parameter associations for the formal parameters to
be bound, e.g.:

\textbf{function} func\_name(..., formal\_parameter\_name = expr, ...)
...
\textbf{for} i \textbf{in} 1:N \textbf{loop}

area := area + quadrature(0, 1, \textbf{function} Sine(A=2, w=i*time));

\textbf{end for};

\textbf{end} Test;

\textbf{function} Sine "y = Sine(x,A,w)"

\textbf{extends} Integrand\textbf{;}
...
\textbf{function} Sine2 "y = Sine2(A,w,x)"

\textbf{input} Real A;

\textbf{input} Real w;

\textbf{input} Real x; // Note: x is now last in argument list.
...
area = quadrature(0, 1, integrand = \textbf{function} Sine2(A=2, w=3));
...
\textbf{partial} \textbf{function} SurfaceIntegrand
...
\textbf{end} SurfaceIntegrand;

\textbf{function} quadratureOnce
...
\textbf{input} SurfaceIntegrand integrand;

\textbf{output} Real z;

\textbf{algorithm}

z := quadrature(y1, y2, \textbf{function} integrand(y=x));

// This is according to case (d) and needs to bind the 2nd argument

\textbf{end} quadratureOnce;

\textbf{function} surfaceQuadrature
...
\textbf{function} quadratureOnce(y1=y1, y2=y2, integrand=integrand));

// Case (b) and (c)

\textbf{end} surfaceQuadrature;
...
\textbf{function} eigen "calculate eigenvalues and optionally
eigenvectors"

\textbf{input} Real A{[}:, size(A,1){]};

\textbf{output} Real eigenValues{[}size(A,1),2{]};

\textbf{output} Real rightEigenVectors{[}size(A,1),size(A,1){]};

\textbf{output} Real leftEigenVectors {[}size(A,1),size(A,1){]};
...
section are also briefly described in \ref{function-as-specialized-class}.}{]}
...
functions is also briefly described in \ref{function-as-specialized-class}.}{]}
...
\textbf{function} joinThreeVectors

\textbf{input} Real v1{[}:{]},v2{[}:{]},v3{[}:{]};

\textbf{output} Real vres{[}size(v1,1)+size(v2,1)+size(v3,1){]};

\textbf{algorithm}

vres := \textbf{cat}(1,v1,v2,v3);

\textbf{end} joinThreeVectors;
...
\textbf{function} collectPositive

\textbf{input} Real x{[}:{]};

\textbf{output} Real xpos{[}:{]};

\textbf{algorithm}

\textbf{for} i \textbf{in} 1:size(x,1) \textbf{loop}

\textbf{if} x{[}i{]}\textgreater{}0 \textbf{then}
...
\textbf{input} Real e1, e2;

\textbf{output} Real sum1;
...
\subsection{Record Constructor Functions}
...
\textbf{record} Complex "Complex number"
...
\textbf{input} Complex u, v;

\textbf{output} Complex w(re=u.re + v.re, im=u.im+v.re);
...
\textbf{package} Motors

\textbf{record} MotorData "Data sheet of a motor"

\textbf{parameter} Real inertia;

\textbf{parameter} Real nominalTorque;

\textbf{parameter} Real maxTorque;

\textbf{parameter} Real maxSpeed;

\textbf{end} MotorData;

\textbf{model} Motor "Motor model" // using the generic MotorData
...
\textbf{record} MotorI123 = MotorData( // data of a specific motor
...
\textbf{record} MotorI145 = MotorData( // data of another specific motor
...
\textbf{record} Record1;

\textbf{parameter} Real r0 = 0;

\textbf{end} Record1;

\textbf{record} Record2

\textbf{import} Modelica.Math.*;

\textbf{extends} Record1;

\textbf{constant} Real c1 = 2.0;

\textbf{constant} Real c2;

\textbf{parameter} Integer n1 = 5;

\textbf{parameter} Integer n2;

\textbf{parameter} Real r1 "comment";

\textbf{parameter} Real r2 = sin(c1);

\textbf{final} \textbf{parameter} Real r3 = cos(r2);
...
\textbf{function} Record1

\textbf{input} Real r0 := 0;

\textbf{output} Record1 \emph{result}(r0 = r0);

\textbf{end} Record1;

\textbf{function} Record2

\textbf{input} Real r0 := 0;

\textbf{input} Real c2;

\textbf{input} Integer n1 := 5;

\textbf{input} Integer n2;

\textbf{input} Real r1 "comment"; // the comment also copied from record

\textbf{input} Real r2 := Modelica.Math.sin(c1);

\textbf{input} Real r4;

\textbf{input} Real r5 := 5.0;

\textbf{input} Real r6{[}n1{]};

\textbf{input} Real r7{[}n2{]};

\textbf{output} Record2
\emph{result}(r0=r0,c2=c2,n1=n1,n2=n2,r1=r1,r2=r2,

r4=r4,r5=r5,r6=r6,r7=r7);

\textbf{protected}

\textbf{constant} Real c1 = 2.0; // referenced from r2

\textbf{final} \textbf{parameter} Real r3 = Modelica.Math.cos(r2);
...
\textbf{parameter} Demo.Record2 r3 = Demo.Record2(c2=2, n2=1, r1=1,
r4=4,
...
\textbf{record} MyFlange

Real tau;

\textbf{end} MyFlange;

\textbf{record} MyRecord1
...
\textbf{function} foo0 \textbf{annotation}(derivative=foo1);
\textbf{end} foo0;

\textbf{function} foo1 \textbf{annotation}(derivative(order=2)=foo2);
\textbf{end} foo1;

\textbf{function} foo2 \textbf{end} foo2;
...
\textbf{annotation}(derivative=foo1);
...
\textbf{input} Real \textbf{der}\_x;

...

\textbf{output} Real \textbf{der}\_y;

...

\textbf{annotation}(derivative(order=2)=foo2);
...
\textbf{input} Real der\_x;

...;

\textbf{input} Real der\_2\_x;

...

\textbf{output} Real der\_2\_y;
...
\textbf{record} ThermodynamicState "Thermodynamic state"
...
\textbf{end} ThermodynamicState;

\textbf{record} ThermoDynamicState\_der "Derivative"
...
\textbf{end} ThermodynamicState\_der;

\textbf{function} density

\textbf{input} ThermodynamicState state "Thermodynamic state";

\textbf{output} Density d "Density";

\textbf{algorithm }

...

\textbf{annotation}(derivative=density\_der);

\textbf{end} density;

\textbf{function} density\_der

\textbf{input} ThermodynamicState state "Thermodynamic state";

\textbf{input} ThermodynamicState\_der state\_der;

\textbf{output} DensityDerivative d "Density derivative";

\textbf{algorithm }

...

\textbf{end} density\_der;

\textbf{function} setState\_ph

\textbf{input} Pressure p;

\textbf{input} SpecificEnthalpy h;

\textbf{input} Integer phase = 0;

\textbf{output} ThermodynamicState state;

\textbf{algorithm}

...

\textbf{annotation}(derivative = setState\_ph\_der);

\textbf{end} setState\_ph;

\textbf{function} setState\_ph\_der

\textbf{input} Pressure p;

\textbf{input} SpecificEnthalpy h;

\textbf{input} Integer phase;

\textbf{input} PressureDerivative p\_der;

\textbf{input} SpecificEnthalpyDerivative h\_der;

\textbf{output} ThermodynamicState\_der state\_der;

\textbf{algorithm}

...

\textbf{end} setState\_ph\_der;

ThermodynamicState state1 = setState\_ph(p=..., h=..., phase=...);

Density rho1=density(state1);

DensityDerivative d\_rho1=\textbf{der}(rho1);

Density rho2=density(setState\_ph(p=..., h=..., phase=...));

DensityDerivative d\_rho2=\textbf{der}(rho2);
...
\textbf{function} f "Simple table lookup"

\textbf{input} Real x;

\textbf{input} Real y{[}:, 2{]};

\textbf{input} Real offset;
...
\textbf{annotation}(derivative(zeroDerivative=y, zeroDerivative=offset)
= f\_der,
...
\textbf{function} f\_der "Derivative of simple table lookup"
...
\textbf{input} Real x\_der;

\textbf{output} Real z\_der;
...
\textbf{function} f\_der "Second derivative of simple table lookup"
...
\textbf{input} Real x\_der2;

\textbf{output} Real z\_der2;
...
\textbf{function} f\_general\_der "Derivative of table lookup taking
...
\textbf{input} Real y\_der{[}:, 2{]};

\textbf{input} Real offset\_der;
...
\textbf{//annotation}(derivative(order=2) = f\_general\_der2);

\textbf{end} f\_general\_der;
...
\textbf{annotation}(derivative(noDerivative=y) = f\_der);

\textbf{end} f;

\textbf{function} f\_der
...
IDENT "=" \textbf{der} "(" name "," IDENT \{ "," IDENT \} ")" comment
...
\textbf{function} Gibbs

\textbf{input} Real p,T;
...
\textbf{function} Gibbs\_T=\textbf{der}(Gibbs, T);

\textbf{function} specificEnthalpy
...
\textbf{annotation}(\textbf{inverse}($u_k$ =
$f_2$(..., y, ....), $u_i$ =
$f_3$(..., y, ...), ...));
...
\textbf{function} h\_pTX

\textbf{input} Real p "pressure";

\textbf{input} Real T "temperature";

\textbf{input} Real X{[}:{]} "mass fractions";

\textbf{output} Real h "specific enthalpy";

\textbf{algorithm}

...

\textbf{annotation}(\textbf{inverse}(T = T\_phX(p,h,X)));

\textbf{end} h\_pTX;

\textbf{function} T\_phX

\textbf{input} Real p "pressure";

\textbf{input} Real h "specific enthalpy";

\textbf{input} Real X{[}:{]} "mass fractions";

\textbf{output} Real T "temperature";
...
\textbf{function} IDENT string-comment

\{ component-clause ";" \}

{[} \textbf{protected} \{ component-clause ";" \} {]}

\textbf{external} {[} language-specification {]} {[}
external-function-call {]} {[}annotation {]} ";"
...
\textbf{external} "builtin" y=sin(x);
...
\textbf{model} UserModel

\textbf{parameter} Real p=Modelica.Math.sin(2);
...
\begin{longtable}[]{@{}lll@{}}
\hline
\emph{Modelica} & FORTRAN 77\\ \hline
...
\textbf{input} Real y{[}size(x,1),:{]};

\textbf{input} Integer i;

\textbf{output} Real u1{[}size(y,1){]};

\textbf{output} Integer u2{[}size(y,2){]};

\textbf{external} "FORTRAN 77" myfoo(x, y, size(x,1), size(y,2),
...
\textbf{annotation}(arrayLayout = "columnMajor");
...
\textbf{output} Real z:=x;

\textbf{external} "FORTRAN 77" myfoo(x,y,z);
...
\textbf{protected} Real xtemp:=x; // Temporary used because myfoo
changes its input

\textbf{public} \textbf{input} Real y;

\textbf{output} Real z;

\textbf{external} "FORTRAN 77" myfoo(xtemp,y,z);
...
\textbf{package} ExternalFunctions
...
\textbf{der}(x)=-ExternalFunc1(x);

\textbf{der}(y)=-ExternalFunc2(y);

\textbf{end} Example;

\textbf{function} ExternalFunc1
...
Include="\#include \textbackslash{}"ExternalFunc1.h\textbackslash{}"");

\textbf{end} ExternalFunc1;

\textbf{function} ExternalFunc2

\textbf{input} Real x;

\textbf{output} Real y;

\textbf{external} "C" annotation(Include="\#include
\textbackslash{}"ExternalFunc3.c\textbackslash{}"");

\textbf{end} ExternalFunc2;

\textbf{function} ExternalFunc3
...
\textbf{end} ExternalFunc3;

\textbf{end} ExternalFunctions;

\textbf{package} MyExternalFunctions

\textbf{extends} ExternalFunctions;

\textbf{end} MyExternalFunctions;

\emph{Directory structure:}

\textbf{ExternalFunctions}

package.mo // contains the Modelica code from above

\textbf{Resources}

\textbf{Include} // contains the include files
...
\textbf{Library} // contains the object libraries for different
platforms
...
\textbf{\#ifdef} \_\_cplusplus

\textbf{extern} "C" \{

\textbf{\#endif}

\textbf{\#ifdef} \_MSC\_VER

\textbf{\#ifdef} EXTERNAL\_FUNCTION\_EXPORT

\textbf{\# define} EXTLIB2\_EXPORT \_\_declspec( dllexport )

\textbf{\#else }

\textbf{\# define} EXTLIB2\_EXPORT \_\_declspec( dllimport )

\textbf{\#endif}

\textbf{\#elif} \_\_GNUC\_\_ \textgreater{}= 4
...
\textbf{\# define} EXTLIB2\_EXPORT \_\_attribute\_\_
((visibility("default")))

\textbf{\#else}

\textbf{\# define} EXTLIB2\_EXPORT

\textbf{\#endif}

EXTLIB2\_EXPORT void ExternalFunc2(\textless{}function
arguments\textgreater{});
...
\textbf{output} Real u1;

\textbf{output} Integer u2;

\textbf{external} "C" myfoo(x, u1, y, u2);
...
\textbf{output} Real funcvalue;

\textbf{output} Integer out1;

\textbf{external} "C" funcvalue = myfoo(x, y, out1);
...
& \vtop{\hbox{\strut \emph{void Modelica\{Message,Warning,Error\}\linebreak[4]
...
ModelicaFormat\{Message,Warning,Error\}\linebreak[4]
...
& \vtop{\hbox{\strut \emph{void
ModelicaVFormat\{Message,Warning,Error\}\linebreak[4]
...
ModelicaAllocateStringWithErrorReturn & \vtop{\hbox{\strut \emph{char*
ModelicaAllocateStringWithErrorReturn(size\_t len)}}\hbox{\strut Same as
...
\textbf{extends} ExternalObject;

\textbf{function} constructor

\textbf{input} String fileName := "";

\textbf{input} String tableName := "";

\textbf{output} MyTable table;

\textbf{external} "C" table = initMyTable(fileName, tableName);

\textbf{end} constructor;

\textbf{function} destructor "Release storage of table"

\textbf{input} MyTable table;

\textbf{external} "C" closeMyTable(table);

\textbf{end} destructor;

\textbf{end} MyTable;

\emph{and used in the following way:}

\textbf{model} test "Define a new table and interpolate in it"
...
\textbf{function} interpolateMyTable "Interpolate in table"
...
\textbf{external} "C" y = interpolateMyTable(table, u);

\textbf{end} interpolateTable;
...
\subsubsection{Lookup of Imported Names}\doublelabel{lookup-of-imported-names-1}
...
\textbf{import} Modelica.Math.ComplexNumbers; // Accessed by
ComplexNumbers.Add

\textbf{import} Modelica.Math.ComplexNumbers.Add; // Accessed by Add

\textbf{import} Modelica.Math.ComplexNumbers.\{Add,Sub\}; // Accessed by
Add and Sub

\textbf{import} Modelica.Math.ComplexNumbers.*; // Accessed by Add

\textbf{import} Co = Modelica.Math.ComplexNumbers; // Accessed by Co.Add
...
\subsubsection{Mapping a Package/Class Hierarchy into a Directory Hierarchy (Structured Entity)}\doublelabel{mapping-a-packageclass-hierarchy-into-a-directory-hierarchy-structured-entity}

A directory shall contain a node, the file package.mo. The node shall
contain a stored-definition that defines a class {[}A{]} with a name
...
\subsubsection{Mapping a Package/Class Hierarchy into a Single File (Nonstructured Entity)}\doublelabel{mapping-a-packageclass-hierarchy-into-a-single-file-nonstructured-entity}

When mapping a package or class-hierarchy to a file {[}\emph{e.g. the
file} A.mo{]}, that file shall only define a single class {[}A{]} with a
...
\textbf{within} {[} packageprefixname {]} ";"
...
\textbf{within} Modelica.Mechanics;

\textbf{package} Rotational // Modelica.Mechanics.Rotational
...
\textbf{input} $A_{m}$ $u_{m}$ :=
$A_{m}$;

...

\textbf{input} $A_{n}$ $u_{n}$;
...
\textbf{operator record} Complex "Record defining a Complex number"

Real re "Real part of complex number";

Real im "Imaginary part of complex number";

\textbf{encapsulated} \textbf{operator} 'constructor'

\textbf{import} Complex;

\textbf{function} fromReal

\textbf{input} Real re;

\textbf{input} Real im := 0;

\textbf{output} Complex result(re=re, im=im);

\textbf{algorithm}

\textbf{annotation}(Inline=\textbf{true});

\textbf{end} fromReal;

\textbf{end} 'constructor';

\textbf{encapsulated} \textbf{operator} \textbf{function} '+' // short
hand notation, see \ref{specialized-classes}

\textbf{import} Complex;

\textbf{input} Complex c1;

\textbf{input} Complex c2;

\textbf{output} Complex result "= c1 + c2";
...
\textbf{encapsulated} \textbf{operator} '-'

\textbf{import} Complex;

\textbf{function} negate

\textbf{input} Complex c;

\textbf{output} Complex result "= - c";
...
\textbf{encapsulated} \textbf{operator} \textbf{function} '*'
...
\textbf{import} Complex; \textbf{input} Complex c1;
...
\textbf{output} Boolean result "= c1 == c2";
...
\textbf{encapsulated} \textbf{operator function} 'String'

\textbf{import} Complex;

\textbf{input} Complex c;

\textbf{input} String name := "j" "Name of variable representing
sqrt(-1) in the string";

\textbf{input} Integer significantDigits=6 "Number of significant digits
to be shown";

\textbf{output} String s;

\textbf{algorithm }

s := String(c.re, significantDigits=significantDigits);

\textbf{if} c.im \textless{}\textgreater{} 0 \textbf{then}

s := \textbf{if} c.im \textgreater{} 0 \textbf{then} s + " + "
\textbf{else} s + " - ";
...
\textbf{encapsulated} \textbf{function} j

\textbf{import} Complex;

\textbf{output} Complex c;
...
\textbf{encapsulated} \textbf{operator} \textbf{function} '0'
...
\textbf{function} eigenValues

\textbf{input} Real A {[}:,:{]};

\textbf{output} Complex ev{[}\textbf{size}(A, 1){]};
...
\textbf{for} i \textbf{in} 1:nx \textbf{loop}

ev{[}i{]} := Complex(eval{[}i, 1{]}, eval{[}i, 2{]});

\textbf{end for};

\textbf{end} eigenValues;
...
\textbf{operator record} ComplexVoltage = Complex(re(unit="V"),
im(unit="V"));

\textbf{operator record} ComplexCurrent = Complex(re(unit="A"),
im(unit="A"));

\textbf{connector} ComplexPin

ComplexVoltage v;

\textbf{flow} ComplexCurrent i;

\textbf{end} ComplexPin;
...
\textbf{extends} Icon; //Ok.

\textbf{operator record} B \ldots{} \textbf{end} B;

\textbf{end} A;

\textbf{package} A2

\textbf{extends} A(\ldots{}); // Not legal

\textbf{end} A2;

\textbf{package} A3=A(\ldots{}); // Not legal

{]}

\chapter{Stream Connectors}
...
\item
  For every outside connector {[}\emph{see \ref{inside-and-outside-connections}}{]}, one
...
\item
  For inside connectors {[}\emph{see \ref{inside-and-outside-connections}}{]}, variables
...
\textbf{connector} FluidPort

\textbf{replaceable package} Medium =
Modelica.Media.Interfaces.PartialMedium;

Medium.AbsolutePressure p "Pressure in connection point";

\textbf{flow} Medium.MassFlowRate m\_flow "\textgreater{} 0, if flow
into component";

\textbf{stream} Medium.SpecificEnthalpy h\_outflow "h close to port if
m\_flow \textless{} 0";

\textbf{stream} Medium.MassFraction X\_outflow{[}Medium.nX{]} "X close
to port if m\_flow \textless{} 0";
...
\section{Stream Operator inStream and Connection Equations}
...
{[}\emph{see definition in \ref{inside-and-outside-connections}}{]} are connected
...
\textbf{flow} Real m\_flow "Flow of matter; m\_flow \textgreater{} 0 if
flow into component";

\textbf{stream} Real h\_outflow "Specific variable in component if
m\_flow \textless{} 0"

\textbf{end} FluidPort

\textbf{model} FluidSystem

...

FluidComponent m\textsubscript{1}, m\textsubscript{2}, ...,
m\textsubscript{N};

FluidPort c\textsubscript{1}, c\textsubscript{2}, ...,
c\textsubscript{M};

\textbf{equation}

\textbf{connect}(m\textsubscript{1}.c, m\textsubscript{2}.c);

\textbf{connect}(m\textsubscript{1}.c, m\textsubscript{3}.c);

...

\textbf{connect}(m\textsubscript{1}.c, m\textsubscript{N}.c);

\textbf{connect}(m\textsubscript{1}.c, c\textsubscript{1});

\textbf{connect}(m\textsubscript{1}.c, c\textsubscript{2});

...

\textbf{connect}(m\textsubscript{1}.c, c\textsubscript{M});

...

\textbf{end} FluidSystem;

%\includegraphics[width=2.3in,height=2.08125in]{media/image27.emf}
...
\textbf{for} j \textbf{in} 1:N) +

sum(-c\textsubscript{k}.m\_flow* (\textbf{if}
--c\textsubscript{k}.m\_flow \textgreater{} 0 \textbf{then}
h\_mix\_in\textsubscript{i} \textbf{else}
\textbf{inStream}(c\textsubscript{k}.h\_outflow)

\textbf{for} k \textbf{in} 1:M);

\textbf{inStream}(m\textsubscript{i}.c.h\_outflow) =
h\_mix\_in\textsubscript{i};\strut
\end{minipage}\\ \hline
...
\textbf{for} q \textbf{in} 1:M \textbf{loop}

0 = sum(m\textsubscript{j}.c.m\_flow*(\textbf{if}
m\textsubscript{j}.c.m\_flow \textgreater{} 0 \textbf{then}
h\_mix\_out\textsubscript{q}

\textbf{else} m\textsubscript{j}.c.h\_outflow) \textbf{for} j
\textbf{in} 1:N) +

sum(-c\textsubscript{k}.m\_flow* (\textbf{if}
--c\textsubscript{k}.m\_flow \textgreater{} 0 \textbf{or} k==q
\textbf{then} h\_mix\_out\textsubscript{q}

\textbf{else} \textbf{inStream}(c\textsubscript{k}.h\_outflow)

\textbf{for} k \textbf{in} 1:M);

c\textsubscript{q}.h\_outflow = h\_mix\_out\textsubscript{q};
...
\textbf{for} j \textbf{in} cat(1,1:i-1, i+1:N) +

\textbf{sum}(\textbf{max}( c\textsubscript{k}.m\_flow
,0)*\textbf{inStream}(c\textsubscript{k}.h\_outflow) \textbf{for} k
\textbf{in} 1:M))/

(\textbf{sum}(\textbf{max}(-m\textsubscript{j}.c.m\_flow,0) \textbf{for}
j \textbf{in} cat(1,1:i-1, i+1:N) +

\textbf{sum}(\textbf{max}( c\textsubscript{k}.m\_flow ,0) \textbf{for} k
\textbf{in} 1:M));
...
c\textsubscript{q}.h\_outflow :=

(\textbf{sum}(\textbf{max}(-m\textsubscript{j}.c.m\_flow,0)*
m\textsubscript{j}.c.h\_outflow \textbf{for} j \textbf{in} 1:N) +

\textbf{sum}(\textbf{max}(c\textsubscript{k}.m\_flow,0)*\textbf{inStream}(c\textsubscript{k}.h\_outflow)
\textbf{for} k \textbf{in} cat(1,1:q-1, q+1:M))/

(\textbf{sum}(\textbf{max}(-m\textsubscript{j}.c.m\_flow,0) \textbf{for}
j \textbf{in} 1:N) +

\textbf{sum}(\textbf{max}( c\textsubscript{k}.m\_flow ,0) \textbf{for} k
\textbf{in} cat(1,1:q-1, q+1:M)));
...
\textbf{inStream}(m\textsubscript{1}.c.h\_outflow) =
m\textsubscript{1}.c.h\_outflow;

\textbf{N = 2, M = 0:}

\textbf{inStream}(m\textsubscript{1}.c.h\_outflow) =
m\textsubscript{2}.c.h\_outflow;

\textbf{inStream}(m\textsubscript{2}.c.h\_outflow) =
m\textsubscript{1}.c.h\_outflow;

\textbf{N = 1, M = 1:}

\textbf{inStream}(m\textsubscript{1}.c.h\_outflow) =
\textbf{inStream}(c\textsubscript{1}.h\_outflow);

// Additional equation to be generated

c\textsubscript{1}.h\_outflow = m\textsubscript{1}.c.h\_outflow;
...
\textbf{if} m\textsubscript{j}.c.m\_flow.min \textgreater{}= 0
\textbf{for} \textbf{all} j = 1:N with j \textless{}\textgreater{} i
\textbf{and}

c\textsubscript{k}.m\_flow.max \textless{}= 0 \textbf{for all} k = 1:M

\textbf{then }

\textbf{inStream}(m\textsubscript{i}.c.h\_outflow) =
m\textsubscript{i}.c.h\_outflow;

\textbf{else }

si = \textbf{sum}(\textbf{max}(-m\textsubscript{j}.c.m\_flow,0)
\textbf{for} j \textbf{in} cat(1,1:i-1, i+1:N) +
...
(\textbf{sum}(\textbf{positiveMax}(-m\textsubscript{j}.c.m\_flow,s\textsubscript{i})*m\textsubscript{j}.c.h\_outflow)
+

\textbf{sum}(\textbf{positiveMax}(c\textsubscript{k}.m\_flow,s\textsubscript{i})
*\textbf{inStream}(c\textsubscript{k}.h\_outflow)))/

(\textbf{sum}(\textbf{positiveMax}(-m\textsubscript{j}.c.m\_flow,s\textsubscript{i}))
+

\textbf{sum}(\textbf{positiveMax}(c\textsubscript{k}.m\_flow,s\textsubscript{i})))

\textbf{for} j in 1:N \textbf{and} i \textless{}\textgreater{} j
\textbf{and} m\textsubscript{j}.c.m\_flow.min \textless{} 0,

\textbf{for} k in 1:M \textbf{and} c\textsubscript{k}.m\_flow.max
\textgreater{} 0
...
\textbf{if} m\textsubscript{j}.c.m\_flow.min \textgreater{}= 0
\textbf{for} \textbf{all} j = 1:N \textbf{and}

c\textsubscript{k}.m\_flow.max \textless{}= 0 \textbf{for all} k = 1:M
\textbf{and} k \textless{}\textgreater{} q

\textbf{then}

c\textsubscript{q}.h\_outflow = 0;

\textbf{else}

s\textsubscript{q} \textbf{=}
(\textbf{sum}(\textbf{max}(-m\textsubscript{j}.c.m\_flow,0) \textbf{for}
j \textbf{in} 1:N) +
...
c\textsubscript{q}.h\_outflow =
(\textbf{sum}(\textbf{positiveMax}(-m\textsubscript{j}.c.m\_flow,s\textsubscript{q})*
m\textsubscript{j}.c.h\_outflow) +

\textbf{sum}(\textbf{positiveMax}(c\textsubscript{k}.m\_flow,s\textsubscript{q})
* \textbf{inStream}(c\textsubscript{k}.h\_outflow)))/
...
\textbf{for} j \textbf{in} 1:N \textbf{and}
m\textsubscript{j}.c.m\_flow.min \textless{} 0,

\textbf{for} k \textbf{in} 1:M \textbf{and} k \textless{}\textgreater{}
q \textbf{and} c\textsubscript{k}.m\_flow.max \textgreater{} 0
...
\textbf{positiveMax}(-m\textsubscript{j}.c.m\_flow, s\textsubscript{i})
= \textbf{max}(-m\textsubscript{j}.c.m\_flow, eps1); // so
s\textsubscript{i} is not needed

\emph{More sophisticated implementation, with smooth approximation,
applied only when {all} flows are small:}

// Define a "small number" eps (\textbf{nominal}(v) is the nominal value
of v -- see \ref{attributes-start-fixed-nominal-and-unbounded})

eps :=
relativeTolerance*\textbf{min}(\textbf{nominal}(m\textsubscript{j}.c.m\_flow));

// Define a smooth curve, such that
alpha(s\textsubscript{i}\textgreater{}=eps)=1 and
alpha(s\textsubscript{i}\textless{}=0)=0

alpha := \textbf{smooth}(1, \textbf{if} s\textsubscript{i}
\textgreater{} eps \textbf{then} 1 \textbf{else}

\textbf{if} s\textsubscript{i} \textgreater{} 0 \textbf{then}
(s\textsubscript{i}/eps)\^{}2*(3-2*(s\textsubscript{i}/eps))
\textbf{else} 0);

// Define function \textbf{positiveMax}(v,s\textsubscript{i}) as a
linear combination of \textbf{max}(v,0)\\
// and of eps along alpha

\textbf{positiveMax}((-m\textsubscript{j}.c.m\_flow,s\textsubscript{i})
:= alpha*\textbf{max}(-m\textsubscript{j}.c.m\_flow,0) +
(1-alpha)*eps;\strut
\end{minipage}\\ \hline
...
h\_c = actualStream(c.h); // (2)monitoring the enthalpy at port c
...
\chapter{Synchronous Language Elements}
...
\item
  ...
  // discrete controller\\
  E*xd = A*\textbf{previous}(xd) + B*yd;\\
  ud = C*\textbf{previous}(xd) + D*yd; }
\item
...
\emph{\textbf{when sample}(0,3) \textbf{then}}

\emph{xd = A*\textbf{pre}(xd) + B*y;\\
u = C*\textbf{pre}(xd) + D*y;}
...
\subsection{Clocks and Clocked Variables}
...
\begin{quote}
\textbf{The different kinds of discrete-time variables in Modelica}
\end{quote}\strut
\end{minipage}\\ \hline
...
{[}\emph{Examples:\\
} Clock c1 = \textbf{Clock}(...);\\
Clock c2 = c1;\\
Clock c3 = \textbf{subSample}(c2,4);\\
{]}
\end{quote}\strut
\end{minipage}\\ \hline
\begin{minipage}[t]{0.47\columnwidth}\raggedright
...
\item
  ...
  Resistor R;\\
  ...\\
  y1 = \textbf{previous}(u1); // fine\\
  y2 = \textbf{previous}(u2); // fine\\
  y3 = \textbf{previous}(u2{[}2{]}); // fine\\
  y4 = \textbf{previous}(c.im); // fine\\
  y5 = \textbf{previous}(2*u); // error (general expression, no
  Component Expression)\\
  y6 = \textbf{previous}(R); // error (component, no Component
  Expression)\\
  {]}
\item
  ...
  Examples:}\\
  Real u;\\
  \textbf{parameter} Real p=3;\\
  ...\\
  y1 = \textbf{subSample}(u, factor=3); // fine (literal)\\
  y2 = \textbf{subSample}(u, factor=2*p -- 3); // fine (parameter
  expression)\\
  y3 = \textbf{subSample}(u, factor=3*u); // error (general expression)
  {]}
\end{itemize}
...
{[}\emph{Example}:\\
\textbf{when Clock}() \textbf{then} // equations are on the same clock\\
x = A*\textbf{previous}(x) + B*u;\\
Modelica.Utilities.Streams.print\\
("clock ticks at = " + \textbf{String}(\textbf{sample}(time)));\\
\textbf{end when};
...
\textbf{when Clock}(2,1000) \textbf{then}

// periodic clock that ticks at 0, 0.002, 0.004, ...

y1 = \textbf{previous}(y1) + 1;

\textbf{end when};

\textbf{when Clock}(nextInterval, 1000) \textbf{then}

// interval clock that ticks at 0, 0.003, 0.007, 0.012, ...

nextInterval = \textbf{previous}(nextInterval) + 1;

y2 = \textbf{previous}(y2) + 1;
...
\textbf{Clock} c = \textbf{Clock}(angle \textgreater{} 0, 0.1) // before
first tick of c:

// \textbf{interval}(c) = 0.1
...
\emph{Clock c2 = Clock(c1, ``ImplicitTrapezoid``); // 100 ms,
ImplicitTrapezoid solver }

\emph{Clock c3 = Clock(c2, ````); // 100 ms, no solver} {]}\strut
\end{minipage}\\ \hline
...
conversion operator from this section.}{]}\strut
\end{minipage}\\ \hline
...
initial value of u can be used in such a case.}{]}
\end{quote}\strut
\end{minipage}\\ \hline
...
yc = \textbf{sample}(y, Clock(0.1));

\textbf{initial equation}
...
\textbf{Clock} u = \textbf{Clock}(x \textgreater{} 0);

\textbf{Clock} y1 = \textbf{subSample} (u,4);\\
\textbf{Clock} y2 = \textbf{superSample}(y1,2); // fine; y2 =
\textbf{subSample}(u,2)

\textbf{Clock} y3 = \textbf{superSample}(u ,2); // error

\textbf{Clock} y4 = \textbf{superSample}(y1,5); // error
...
\textbf{Clock} cBase = \textbf{subSample}(\textbf{superSample}(u,
resolution), shiftCounter)
...
\textbf{Clock} y1 = \textbf{shiftSample}(u,1,3); // ticks: 1/10, 4/10,
...
...
\textbf{Clock} y1 = \textbf{shiftSample}(u,2); // ticks: 2.0, 3.0, ...

\textbf{Clock} y2 = \textbf{shiftSample}(u,2,3);// error (resolution
must be 1)
...
\begin{minipage}[t]{0.47\columnwidth}\raggedright
\textbf{backSample}(u,\\
backCounter, resolution)\strut
\end{minipage} & \begin{minipage}[t]{0.47\columnwidth}\raggedright
\begin{quote}
The input argument u is either a Component Expression (see 
\ref{argument-restrictions-component-expressions}) or an expression of type Clock. {[}\emph{The first activation of
...
\begin{quote}
\textbf{model} NoClockVsSampleHold

\textbf{Clock} clk1 = \textbf{Clock}(0.1);

\textbf{Clock} clk2 = \textbf{subSample}(clk1,2);

Real x(start=0), y(start=0), z(start=0);

\textbf{equation}

\textbf{when} clk1 \textbf{then}

x = \textbf{previous}(x) + 0.1;

\textbf{end} \textbf{when};

\textbf{when} clk2 \textbf{then}

y = \textbf{noClock}(x); // most recent value of x

z = \textbf{sample}(\textbf{hold}(x)); // left limit of x
(infinitesimally delayed)!

\textbf{end} \textbf{when};

\textbf{end} NoClockVsSampleHold;
\end{quote}
...
\textbf{when} clause are also in a clocked partition.
...
ud1 = \textbf{sample}(y,c1); // incidence(e) = \{ud1\}

0 = f1(yd1, ud1, \textbf{previous}(ud1)); // incidence(e) = \{yd1,ud1\}

ud2 = \textbf{superSample}(yd1,2); // incidence(e) = \{ud2, yd1\}

0 = f2(yd2, ud2); // incidence(e) = \{yd2, ud2\}

// Base partition 2 // continuous-time partition

u = \textbf{hold}(yd2); // incidence(e) = \{u\}

0 = f3(\textbf{der}(x1), x1, u); // incidence(e) = \{x1,u\}

0 = f4(\textbf{der}(x2), x2, x1); // incidence(e) = \{x2,x1\}

0 = f6(y, x1, u); // incidence(e) = \{y,x1,u\}

// Identified as separate partition, but belonging to partition 2

0 = f5(\textbf{der}(x3), x3); // incidence(e) = \{x3\}
...
0 = f1(yd1,ud1,\textbf{previous}(yd1)); // incidence(e) = \{yd1,ud1\}

// Sub-Clock partition 1.2

ud2 = \textbf{superSample}(yd1,2); // incidence(e) = \{ud2\}
...
Real x(start=x\_start); // previous(x) = x\_start at first clock tick

Real der\_x(start=0); // previous(der\_x) = 0 at first clock tick
...
\textbf{when} \textbf{Clock}() \textbf{then}

first = false;

\textbf{if} \textbf{previous}(first) \textbf{then}
...
x = \textbf{previous}(x) +
\textbf{interval}()*\textbf{previous}(der\_x);
...
\textbf{model} InferenceTest "Specific clocks set on all partitions.
Decouple constraint"
...
\textbf{der}(x1) = -100*x1 + \textbf{hold}(z2);

// Controller submodels

\textbf{der}(z1) = -100*z1 + \textbf{sample}(x2,
\textbf{Clock}(\textbf{Clock}(1, 100), solverMethod="ImplicitEuler"));

w = 0.9*\textbf{previous}(w) + \textbf{superSample}(z1, 3);

\textbf{when} \textbf{Clock}(\textbf{Clock}(1,20),
solverMethod="ExplicitEuler") \textbf{then}

ww = \textbf{superSample}(w);

\textbf{end when}

\textbf{der}(z2) = -z2 + ww;

\textbf{end} InferenceTest;

{]}

\section{Initialization of Clocked Partitions}
...
used for clock inference, see \ref{clock-partitioning}.
\end{quote}\strut
\end{minipage}\\ \hline

\endhead
\begin{minipage}[t]{0.47\columnwidth}\raggedright
\textbf{interval}(u)\strut
\end{minipage} & \begin{minipage}[t]{0.47\columnwidth}\raggedright
\begin{quote}
...
\textbf{block} ClockedPI

\textbf{parameter} Real T "Time constant of continuous PI controller";

\textbf{parameter} Real k "Gain of continuous PI controller";
...
Discretization equation: der(x) = (x - \textbf{previous}(x))/Ts;\\
*/

\textbf{when} \textbf{Clock}() \textbf{then}

x = \textbf{previous}(x) + Ts/T*u;
...
\textbf{block} MixedController

\textbf{parameter} Real T "Time constant of continuous PI controller";

\textbf{parameter} Real k "Gain of continuous PI controller";

\textbf{input} Real y\_ref, y\_meas;

Real y;

\textbf{output} Real yc;
...
z = \textbf{if} firstTick() \textbf{then} 0 \textbf{else}
\textbf{previous}(z) + Ts/T*(uc - y\_meas);

y = xc + k*(xc + uc);

yc = \textbf{hold}(y);

\textbf{end} MixedController;
...
\begin{minipage}[b]{0.47\columnwidth}\raggedright
\begin{quote}
\textbf{Statements to define a state machine}
\end{quote}\strut
\end{minipage}\\ \hline
...
\ref{state-machine-semantics}.\strut
\end{minipage}\\ \hline
...
It is an error if the instance is not a state of a state machine.\strut
\end{minipage}\\ \hline
...
%\includegraphics[width=4.85417in,height=3.42708in]{media/image40.png}
\emph{For
...
~~Integer~fired~=~\textbf{max}(\textbf{if}~(\textbf{if}~t{[}i{]}.from~==~selectedState~\textbf{then}~(\textbf{if}~t{[}i{]}.immediate~\textbf{then}~c{[}i{]}~\textbf{else}~previous(c{[}i{]}))~\textbf{else}~false)~\textbf{then}~i~\textbf{else}~0~\\
\textbf{for}~i~\textbf{in}~1:size(t,1));
...
t{[}i{]}.immediate~\textbf{and}~t{[}i{]}.from~==~selectedState~\textbf{then}~c{[}i{]}~\textbf{else}~false)~\textbf{then}~i~\textbf{else}~0~\textbf{for}~i~\textbf{in}~1:size(t,1));
...
Integer~delayed~=~~max(\textbf{if}~(\textbf{if}~\textbf{not}~t{[}i{]}.immediate~\textbf{and}~t{[}i{]}.from~==~nextState
~\textbf{then}~c{[}i{]}~\textbf{else}~false)~\textbf{then}~i~\textbf{else}~0~\textbf{for}~i~\textbf{in}~1:size(t,1));
...
~~Integer~nextState~=~\textbf{if}~active~\textbf{then}~(\textbf{if}~fired~\textgreater{}~0~\textbf{then}~t{[}fired{]}.to~\textbf{else}~selectedState)~\textbf{else}~previous(nextState);
...
Boolean finalStates{[}nStates{]} = \{max(\textbf{if} t{[}j{]}.from == i
\textbf{then} 1 \textbf{else} 0 \textbf{for} j \textbf{in} 1:size(t,1))
== 0 \textbf{for} i \textbf{in} 1:nStates\};
...
\textbf{output}~Boolean~activeResetStates{[}nStates{]}~=~\{\textbf{if}~reset~\textbf{then}~true~\textbf{else}~previous(nextResetStates{[}i{]})~\textbf{for}~i~\textbf{in}~1:nStates\};

until a state is eventually executed, then its corresponding reset
condition is set to false:

Boolean~nextResetStates{[}nStates{]}~=~\textbf{if}~active~\textbf{then}~\{if~activeState~==~i~\textbf{then}~false~\textbf{else}~activeResetStates{[}i{]}~\textbf{for}~i~\textbf{in}~1:nStates\}
...
\textbf{model}~StateMachineSemantics~"Semantics~of~state~machines"

~~\textbf{parameter}~Integer~nStates;

~~\textbf{parameter}~Transition~t{[}:{]}~"Array~of~transition~data~sorted~in~priority";

~~\textbf{input}
Boolean~c{[}size(t,1){]}~"Transition~conditions~sorted~in~priority";

~~\textbf{input} Boolean~active~"true~if~the~state machine~is~active";

~~\textbf{input} Boolean~reset~"true~when~the~state
machine~should~be~reset";

~~Integer~selectedState~=~\textbf{if}~reset~\textbf{then}~1~\textbf{else}~previous(nextState);

~~Boolean~selectedReset~=~\textbf{if}~reset~\textbf{then}~true~\textbf{else}~previous(nextReset);
...
Integer~delayed~=~~~
max(\textbf{if}~(\textbf{if}~\textbf{not}~t{[}i{]}.immediate~\textbf{and}~t{[}i{]}.from~==~nextState
~\textbf{then}~c{[}i{]}~\textbf{else}~false)~\textbf{then}~i~\textbf{else}~0~\textbf{for}~i~\textbf{in}~1:size(t,1));

~~Integer~fired~=~max(previous(delayed),~immediate);

~~\textbf{output}~Integer~activeState~=~\textbf{if}~reset~\textbf{then}~1~\textbf{elseif}~fired~\textgreater{}~0~\textbf{then}~t{[}fired{]}.to~\textbf{else}~selectedState;

~~\textbf{output}~Boolean~activeReset~=~\textbf{if}~reset~\textbf{then}~true~\textbf{elseif}~fired~\textgreater{}~0~\textbf{then}~t{[}fired{]}.reset~\textbf{else}~selectedReset;

//~Update~states

~~Integer~nextState~=~\textbf{if}~active~\textbf{then}~activeState~\textbf{else}~previous(nextState);

~~Boolean~nextReset~=~\textbf{if}~active~\textbf{then}~false~\textbf{else}~previous(nextReset);

//~Delayed~resetting~of~individual~states

~~\textbf{output}~Boolean~activeResetStates{[}nStates{]}~=~\{\textbf{if}~reset~\textbf{then}~true~\textbf{else}~previous(nextResetStates{[}i{]})~\textbf{for}~i~\textbf{in}~1:nStates\};

~~Boolean~nextResetStates{[}nStates{]}~=~\textbf{if}~active~\textbf{then}~\{if~selectedState~==~i~\textbf{then}~false~\textbf{else}~activeResetStates{[}i{]}~\textbf{for}~i~\textbf{in}~1:nStates\}~\\
\textbf{else}~previous(nextResetStates);
...
\textbf{end}~StateMachineSemantics;
...
%\includegraphics[width=5.34375in,height=5.72917in]{media/image41.png}
...
\begin{quote}
\textbf{block} HierarchicalAndParallelStateMachine

\textbf{inner} Integer v(start=0);
...
transition(state2,state1,v \textgreater{}= 20, immediate=false);
...
\textbf{inner outer output} Integer v;
...
transition(stateA, stateB, v \textgreater{}= 6, immediate=false);

transition(stateB, stateC, v == 0, immediate=false);

transition(stateC, stateA, true, immediate=false, priority=2);

transition(stateC, stateD, count \textgreater{}= 2, immediate=false);
...
transition(stateX, stateY, stateX.i \textgreater{} 20, immediate=false,
reset=false);
...
\textbf{end} HierarchicalAndParallelStateMachine;
\end{quote}

\emph{The behavior of the state machine can be seen in the plots of v:}

%\includegraphics[width=4.16667in,height=2.91667in]{media/image42.png}
...
efficiency of the generated code}{]}\strut
\end{minipage}\\ \hline
...
that are of no interest to him and would confuse him}{]}\strut
\end{minipage}\\ \hline
\begin{minipage}[t]{0.47\columnwidth}\raggedright
Inline\strut
\end{minipage} & \begin{minipage}[t]{0.47\columnwidth}\raggedright
...
reduction needed for mechanical systems.}{]}\strut
\end{minipage}\\ \hline
\begin{minipage}[t]{0.47\columnwidth}\raggedright
LateInline\strut
\end{minipage} & \begin{minipage}[t]{0.47\columnwidth}\raggedright
...
signal is the derivative of another input signal.}{]}\strut
\end{minipage}\\ \hline
\begin{minipage}[t]{0.47\columnwidth}\raggedright
...
the output should generate an event when it changes.}{]}\strut
\end{minipage}\\ \hline
...
\textbf{annotation}"(" "experiment" "(" {[}experimentOption{]} \{,
experimentOption\}{]} ")"
...
\textbf{annotation} "(" {[} layer-annotations {]} ")"
...
\textbf{type} DrawingUnit = Real(\textbf{final} unit="mm");

\textbf{type} Point = DrawingUnit{[}2{]} "\{x, y\}";

\textbf{type} Extent = Point{[}2{]}
...
\textbf{partial} \textbf{record} GraphicItem
...
\textbf{record} Icon "Representation of the icon layer"
...
\textbf{record} Diagram "Representation of the diagram layer"
...
\textbf{type} Color = Integer{[}3{]}(min=0, max=255) "RGB
representation";

\textbf{constant} Color Black = zeros(3);

\textbf{type} LinePattern = \textbf{enumeration}(None, Solid, Dash, Dot,
DashDot, DashDotDot);

\textbf{type} FillPattern = \textbf{enumeration}(None, Solid,
Horizontal, Vertical,
...
\textbf{type} BorderPattern = \textbf{enumeration}(None, Raised, Sunken,
Engraved);

\textbf{type} Smooth = \textbf{enumeration}(None, Bezier);

\textbf{type} EllipseClosure = \textbf{enumeration}(None, Chord,
Radial);
...
%\includegraphics[width=2.08333in,height=1.66667in]{media/image43.png}
...
\textbf{type} Arrow = \textbf{enumeration}(None, Open, Filled, Half);

\textbf{type} TextStyle = \textbf{enumeration}(Bold, Italic, UnderLine);

\textbf{type} TextAlignment = \textbf{enumeration}(Left, Center, Right);

Filled shapes have the following attributes for the border and interior.

\textbf{record} FilledShape "Style attributes for filled shapes"
...
\textbf{record} DiagramMap

Extent extent = \{\{0, 0\}, \{0, 0\}\};

Boolean primitivesVisible = true;

\textbf{end} DiagramMap;
...
\textbf{extends} B \textbf{annotation}(
...
\textbf{extends} C annotation(DiagramMap(primitivesVisible=false));
...
\textbf{extends} GraphicItem;

\textbf{extends} FilledShape;
...
\textbf{if} overflow \textbf{then} \{255,0,0\} \textbf{else}
\{0,0,255\}))\}
...
\textbf{annotation} (Icon(graphics=\{Rectangle(extent={[}0,0; 20,20{]},

fillColor=\textbf{if} on \textbf{then} \{255,0,0\} \textbf{else}
\{0,0,255\})\},
...
\textbf{record} OnMouseUpSetBoolean
...
\textbf{record} OnMouseMoveXSetReal
...
\textbf{record} OnMouseMoveYSetReal
...
\textbf{record} OnMouseDownEditInteger

Integer variable "Name of variable to change";

\textbf{end} OnMouseDownEditInteger;
...
\textbf{record} OnMouseDownEditReal

Real variable "Name of variable to change";

\textbf{end} OnMouseDownEditReal;
...
\textbf{record} OnMouseDownEditString

String variable "Name of variable to change";

\textbf{end} OnMouseDownEditString;
...
\textbf{annotation}(defaultComponentName = "world",
...
\textbf{connector} Frame "Frame of a mechanical system"

...

\textbf{flow} Modelica.SIunits.Force f{[}3{]}
\textbf{annotation}(unassignedMessage =
...
\textbf{parameter} String tab = "General";

\textbf{parameter} String group = "Parameters";

\textbf{parameter} Boolean enable = \textbf{true};

\textbf{parameter} Boolean showStartAttribute = \textbf{false};

\textbf{parameter} Boolean colorSelector = false;

\textbf{parameter} Selector loadSelector;

\textbf{parameter} Selector saveSelector;\\
\textbf{parameter} String groupImage = "";

\textbf{parameter} Boolean connectorSizing = \textbf{false;}

\textbf{end} Dialog;

\textbf{record} Selector

\textbf{parameter} String filter="";

\textbf{parameter} String caption="";
...
\textbf{model} DialogDemo

\textbf{parameter} Boolean b = true "Boolean parameter";

\textbf{parameter} Modelica.SIunits.Length length "Real parameter with
unit";

\textbf{parameter} Integer nInports=0
\textbf{annotation}(\textbf{Dialog}(\textbf{connectorSizing}=\textbf{true}));

\textbf{parameter} Real r1 "Real parameter in Group 1"
\textbf{annotation}(\textbf{Dialog}(\textbf{group}="Group 1"));

\textbf{parameter} Real r2 "Disabled Real parameter in Group 1"

\textbf{annotation}(\textbf{Dialog}(\textbf{group}="Group 1",
\textbf{enable} = \textbf{not} b));

\textbf{parameter} Real r3 "Real parameter in Tab 1"
\textbf{annotation}(\textbf{Dialog}(\textbf{tab}="Tab 1"));

\textbf{parameter} Real r4 "Real parameter in Tab 1 and Group 2"
...
%\includegraphics[width=2.25in,height=1.125in]{media/image44.png}
%includegraphics[width=2.625in,height=0.89583in]{media/image45.png}
...
\textbf{connector} StepIn // Only 1:1 connections are possible since
\textbf{input} used

\textbf{output} Boolean occupied;\\
\textbf{input} Boolean set;
...
\item
  ...
  \textbf{equation}\\
  \textbf{connect}(inPorts, step1.inPorts); // new connect equation
  \emph{\\
  }
\item
...
\item
  ...
  \textbf{connect}(a2, step1.inPorts{[}2{]});\\
  \textbf{connect}(a3, step1.inPorts{[}3{]});\\
  \textbf{connect}(a4, step1.inPorts{[}4{]});\\
  \emph{and the connection from a2 to step1. inPorts{[}2{]} is deleted.
  This results in\\
  } Step step1(nIn=3);\\
  \textbf{equation\\
  connect}(a1, step1.inPorts{[}1{]});\\
  \textbf{connect}(a3, step1.inPorts{[}2{]});\\
  \textbf{connect}(a4, step1.inPorts{[}3{]});
\end{enumerate}
...
\textbf{annotation}(uses(Modelica(version="3.1 Beta 1")));
...
\textbf{annotation}(version = "3.0.1",
...
\textbf{annotation}(uses(Modelica(version = "3.0.1"))); // Common case

\textbf{end} M1

\textbf{model} M2

\textbf{annotation}(uses(Modelica(version = "3.0.1", versionBuild =
4)));
...
\textbf{type} Access = \textbf{enumeration}(hide, icon, documentation,

diagram, nonPackageText, nonPackageDuplicate,

packageText, packageDuplicate);

\textbf{annotation}(Protection(access = Access.documentation));
...
\textbf{package} CommercialFluid // Access icon, documentation, diagram

\textbf{package} Examples // Access icon, documentation, diagram

\textbf{model} PipeExample // Access everything, can be copied

\textbf{end} PipeExample;

\textbf{package} Circuits // Access icon, documentation, diagram

\textbf{model} ClosedCircuit // Access everything, can be copied

\textbf{end} ClosedCircuit;

\textbf{end} Circuits;

\textbf{model} SecretExample // No access

\textbf{annotation}(Protection(access=Access.hide));

\textbf{end} SecretExample;

\textbf{annotation}(Protection(access=Access.nonPackageDuplicate));

\textbf{end} Examples;

\textbf{package} Pipe // Access icon

\textbf{model} StraightPipe // Access icon

\textbf{end} StraightPipe;

\textbf{annotation}(Protection(access=Access.icon));

\textbf{end} Pipe;

\textbf{package} Vessels // Access icon, documentation, diagram

\textbf{model} Tank // Access icon, documentation, diagram, text

\textbf{end} Tank;

\textbf{end} Vessels;

\textbf{annotation}(Protection(access=Access.nonPackageText));

\textbf{end} CommercialFluid;
...
\textbf{package} MyLibrary

\textbf{annotation}(Protection(License(libraryKey="15783-A39323-498222-444ckk4ll",

licenseFile="MyLibraryAuthorization\_Tool.mo\_lic), \ldots{}));
...
\textbf{package} MyLibraryAuthorization\_Tool

\textbf{annotation}(Authorization(\\
libraryKey="15783-A39323-498222-444ckk4ll",

licensor ="Organization A\textbackslash{}nRoad, Country",
...
\textbf{end} MyLibraryAuthorization\_Tool;
...
\subsection{Annotation Choices for Modifications and Redeclarations}

See \ref{annotation-choices-for-suggested-redeclarations-and-modifications}.

\subsection{Annotation for External Libraries and Include Files}
...
\chapter{The Modelica Standard Library}
...
\textbf{type} or \textbf{interface}: property of an instance,
...
\chapter{Modelica Concrete Syntax}
...
\end{lstlisting}
\chapter{Modelica DAE Representation}
...
\textbf{if} \textbf{m} == pre(\textbf{m}) \textbf{then} \textbf{break}
...
\chapter{Derivation of Stream Equations}

This appendix contains a derivation of the equation for stream
connectors from \ref{stream-connectors}.

\section{Reasons for avoiding the actual mixing enthalpy in connector definitions}
...
%includegraphics[width=6.33819in,height=2.97083in]{media/image54.emf}
...
%includegraphics[width=6.69306in,height=3.5125in]{media/image68.emf}
...
\subsection{Connection of 3 stream connectors where one mass flow rate is
...
\subsection{Connection of 3 stream connectors where two mass flow rates are
  positive (ideal splitting junction for uni-directional flow)}
...
\chapter{Modelica Revision History}
...
\item
  ...
  \ref{function-as-specialized-class}. Ticket
...
\item
  Removed misleading comment, \ref{execution-of-algorithm-in-a-model}. Ticket
...
\item
  ...
  functions, \ref{function-as-specialized-class}. Ticket
...
\item
  Clarified that ndims is allow for a scalar, \ref{array-dimension-and-size-function}. Ticket
...
\item
  Clarified calling function as specialized class, \ref{function-as-specialized-class}. Ticket
...
\item
  Functions can be defined without algorithm-section, \ref{function-as-specialized-class}.
\item
...
\item
  ...
  \ref{connectors-and-connetions}. Ticket \href{https://trac.modelica.org/Modelica/ticket/757}{\#757}.
  \end{quote}
\item
  \begin{quote}
...
\item
  ...
  complete array is initialized in this section, \ref{execution-of-algorithm-in-a-model}. Ticket
...
\subsection{Contributors to the Modelica Language, Version 3.2 Revision 2}
...
\section{Modelica 3.2 Revision 1}
...
\subsection{Main changes in Modelica 3.2 Revision 1}
...
\item
  \ref{function-as-specialized-class} Function\\
...
\item
  Clarification: Execution of an algorithm (new \ref{execution-of-algorithm-in-a-model}).
\item
...
\subsection{Contributors to the Modelica Language, Version 3.0}
...
\subsection{Main Changes in Modelica 3.0}
...
\section{Modelica 2.1}

Modelica 2.1 was released January 30, 2004. The Modelica 2.1
specification was edited by Hans Olsson and Martin Otter.

\subsection{Contributors to the Modelica Language, Version 2.1}
...
\subsection{Main Changes in Modelica 2.1}
...
\subsection{Contributors to the Modelica Language, Version 2.0}
...
\subsection{Contributors to the Modelica Language, Version 1.4}
...
\subsection{Contributors to the Modelica Standard Library}
...
\subsection{Main Changes in Modelica 1.4}
...
\textbf{final} Integer n=size(x,1);
