...
A Modelica \lstinline[basicstyle=\ttfamily]!operator record! can define the behavior for
operations such as constructing, adding, multiplying etc. This is done
using the specialized class \lstinline[basicstyle=\ttfamily]!operator! (a restricted class
similar to \lstinline[basicstyle=\ttfamily]!package!, see \autoref{specialized-classes}) comprised of functions
implementing different variants of the operation for the record class in
which the respective \lstinline[basicstyle=\ttfamily]!operator! definition resides. {[}\emph{The
...
the overloaded scalar operations.}{]} The \lstinline[basicstyle=\ttfamily]!operator! keyword is
...
The \lstinline[basicstyle=\ttfamily]!operator record! may also contain additional functions, and
declarations of components of the record. It is not legal to extend from
an \lstinline[basicstyle=\ttfamily]!operator record!, except as a short class definition modifying the
...
All functions defined inside the \lstinline[basicstyle=\ttfamily]!operator! class must return one
...
(\lstinline[basicstyle=\ttfamily]!true! for default value, \lstinline[basicstyle=\ttfamily]!false! otherwise). A call
...
\item
  ...
  $P_{m}$ \lstinline[basicstyle=\ttfamily]!true! and 1 $\le$ m $\le$ n \} is the set \{i: 1 $\le$
  i $\le$ n\}.
\end{itemize}
...
\item
  ...
  \item
    ...
    this uses the overloaded \lstinline[basicstyle=\ttfamily]!'0'! operator of the result array element
    type. If the operator \lstinline[basicstyle=\ttfamily]!'0'! is not defined for that class it is an
...
the operator .+ will always be defined in terms of \lstinline[basicstyle=\ttfamily]!'+'.!
