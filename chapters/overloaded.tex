...
\item
  ...
  {[}\emph{Informally, this means: If there is no direct match of \lstinline!a op b!, then it is tried to find a direct match by automatic type casts
...
\begin{lstlisting}[language=modelica]
  ...
  Complex c = a + b;
\end{lstlisting}
\emph{Due to inlining of functions, the equation for \lstinline!c! is
transformed to:}
\begin{lstlisting}[language=modelica]
