...
\item
  ...
  call is used (\emph{is an expression}), instead of restricting the input
  (\emph{is a Component Expression}).
\end{itemize}
...
The \lstinline!interval! must be strictly positive (\lstinline!interval>0.0!) of type \textbf{Real} unit \emph{s}.
...
\item
  ...
  {[}\emph{For example, assume \lstinline!y = subSample(u)!, and \lstinline!Clock(1, 10)! is
  associated with \lstinline!u! and \lstinline!Clock(2, 10)! is associated with \lstinline!y!, then
  this is correct, but it would be an error if \lstinline!y! is associated with a
  \lstinline!Clock(1, 3)!.} {]}
\item
  Exactly one Real interval clock. {[}\emph{Assume \lstinline!Clock c = Clock(2.5)!, then variables in the same base partition can be
  associated multiple times with \lstinline!c! but not multiple times with
  \lstinline!Clock(2.5)!}{]}
\item
...
{[}\emph{This is called \emph{conceptually}, because a tool might do this
...
All continuous-time partitions are collected together and form \emph{the}
continuous-time partition.
...
\lstinline!when!-clause with a Boolean condition, it is a \emph{clocked
...
integrators. Typically, a tool supports at least methods \lstinline!"External"!
and \lstinline!!ExplicitEuler"!. If a tool does not support the integration method
defined in a model, typically a warning message is printed and the
method is changed to \lstinline!"External"!.}{]}
...
\item
  ...
  variables. The attribute \lstinline!fixed! is true for variables to which the
