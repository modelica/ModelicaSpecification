...
instantiated from classes, including the basic data types---\lstinline[basicstyle=\ttfamily]!Real!,
\lstinline[basicstyle=\ttfamily]!Integer!, \lstinline[basicstyle=\ttfamily]!String!, \lstinline[basicstyle=\ttfamily]!Boolean!---and enumeration types, which are built-in
...
Members of a Modelica class can have two levels of visibility: \lstinline[basicstyle=\ttfamily]!public! or
\lstinline[basicstyle=\ttfamily]!protected!. The default is \lstinline[basicstyle=\ttfamily]!public! if nothing else is specified
...
All elements defined under the heading \lstinline[basicstyle=\ttfamily]!protected! are regarded as
protected. All other elements {[}\emph{i.e., defined under the heading}
\lstinline[basicstyle=\ttfamily]!public!\emph{, without headings or in a separate file}{]} are public
...
If the \lstinline[basicstyle=\ttfamily]!type-specifier! of the component declaration denotes a built-in
type (\lstinline[basicstyle=\ttfamily]!RealType!, \lstinline[basicstyle=\ttfamily]!IntegerType!, etc.), the flattened or instantiated
component has the same type.

If the \lstinline[basicstyle=\ttfamily]!type-specifier! of the component does not denote a built-in type,
...
Variables declared with the \lstinline[basicstyle=\ttfamily]!flow! or the \lstinline[basicstyle=\ttfamily]!stream! type prefix shall be a
subtype of Real.

Type prefixes (that is , \lstinline[basicstyle=\ttfamily]!flow!, \lstinline[basicstyle=\ttfamily]!stream!, \lstinline[basicstyle=\ttfamily]!discrete!, 
\lstinline[basicstyle=\ttfamily]!parameter!, \lstinline[basicstyle=\ttfamily]!constant!,
\lstinline[basicstyle=\ttfamily]!input!, \lstinline[basicstyle=\ttfamily]!output!) shall only be applied for type, record and connector
components -- see also record specialized class, \autoref{specialized-classes}.

An exception is \lstinline[basicstyle=\ttfamily]!input! for components whose type is of the special class
function type (these can only be used for function formal parameters and
has special semantics, see \autoref{functional-input-arguments-to-functions}), and the \lstinline[basicstyle=\ttfamily]!input! prefix is not
...
type prefixes \lstinline[basicstyle=\ttfamily]!parameter! and \lstinline[basicstyle=\ttfamily]!constant!, and in functions also type
prefixes \lstinline[basicstyle=\ttfamily]!input! and \lstinline[basicstyle=\ttfamily]!output! - see \autoref{external-objects}.

The type prefixes \lstinline[basicstyle=\ttfamily]!flow!, \lstinline[basicstyle=\ttfamily]!stream!, \lstinline[basicstyle=\ttfamily]!input!
and \lstinline[basicstyle=\ttfamily]!output! of a structured
...
occurring on elements of the component are correct; e.g. the \lstinline[basicstyle=\ttfamily]!flow! prefix
...
declared with the flow prefix). When any of the type prefixes \lstinline[basicstyle=\ttfamily]!flow!,
\lstinline[basicstyle=\ttfamily]!stream!, \lstinline[basicstyle=\ttfamily]!input! and \lstinline[basicstyle=\ttfamily]!output! are applied for a structured component, no
element of the component may have any of these type prefixes.
{[}\emph{For example,} \lstinline[basicstyle=\ttfamily]!input! \emph{can only be used, if none of the
elements has a} \lstinline[basicstyle=\ttfamily]!flow!, \lstinline[basicstyle=\ttfamily]!stream!, \lstinline[basicstyle=\ttfamily]!input! \emph{or} 
\lstinline[basicstyle=\ttfamily]!output! \emph{type
prefix}{]}. The corresponding rules for the type prefixes \lstinline[basicstyle=\ttfamily]!discrete!,
\lstinline[basicstyle=\ttfamily]!parameter! and \lstinline[basicstyle=\ttfamily]!constant! are described in \autoref{variability-of-structured-entities} for structured
components.

The prefixes \lstinline[basicstyle=\ttfamily]!input! and \lstinline[basicstyle=\ttfamily]!output! have a slightly different semantic meaning
depending on the context where they are used:

\begin{itemize}
\item
  In \emph{functions}, these prefixes define the computational causality
  of the function body, i.e., given the variables declared as \lstinline[basicstyle=\ttfamily]!input!, the
  variables declared as \lstinline[basicstyle=\ttfamily]!output! are computed in the function body, see
...
\item
  ...
  or block is used. Especially, the \lstinline[basicstyle=\ttfamily]!input! prefix defines that values for
  such a variable have to be provided from the simulation environment
  and the \lstinline[basicstyle=\ttfamily]!output! prefix defines that the values of the corresponding
...
\item
  In component \emph{models} and \emph{blocks}, the \lstinline[basicstyle=\ttfamily]!input! prefix defines
...
The \lstinline[basicstyle=\ttfamily]!output! prefix does not have a particular effect in a model or block
component and is ignored.
\item
  In \emph{connectors}, prefixes \lstinline[basicstyle=\ttfamily]!input! and \lstinline[basicstyle=\ttfamily]!output! define that the
  corresponding connectors can only be connected according to block
  diagram semantics, see \autoref{connect-equations-and-connectors} (e.g., a connector with an \lstinline[basicstyle=\ttfamily]!output!
  variable can only be connected to a connector where the corresponding
  variable is declared as \lstinline[basicstyle=\ttfamily]!input!). There is the restriction that
  connectors which have at least one variable declared as \lstinline[basicstyle=\ttfamily]!input! must be
...
\item
  In \emph{records}, prefixes \lstinline[basicstyle=\ttfamily]!input! and \lstinline[basicstyle=\ttfamily]!output! are not allowed, since
...
The prefixes \lstinline[basicstyle=\ttfamily]!discrete!, \lstinline[basicstyle=\ttfamily]!parameter!, \lstinline[basicstyle=\ttfamily]!constant! of a component declaration
...
\begin{itemize}
\item
  A variable \lstinline[basicstyle=\ttfamily]!vc! declared with the parameter or constant prefixes remains
  constant during transient analysis.
\item
  A \emph{discrete-time} variable vd has a vanishing time derivative
  (informally \lstinline[basicstyle=\ttfamily]!der(vd)=0!, but it is not legal to apply the \lstinline[basicstyle=\ttfamily]!der()! operator
...
\item
  A \emph{continuous-time} variable \lstinline[basicstyle=\ttfamily]!vn! may have a non-vanishing time
  derivative (\lstinline[basicstyle=\ttfamily]!der(vn)<>0! possible) and may also
...
even though it was not declared with the prefix \lstinline[basicstyle=\ttfamily]!discrete!. A Real
variable not assigned in any when-clause and without any type prefix is
a continuous-time variable.

The default variability for \lstinline[basicstyle=\ttfamily]!Integer!, \lstinline[basicstyle=\ttfamily]!String!, 
\lstinline[basicstyle=\ttfamily]!Boolean!, or \lstinline[basicstyle=\ttfamily]!enumeration!
variables is discrete-time, and it is not possible to declare
continuous-time \lstinline[basicstyle=\ttfamily]!Integer!, \lstinline[basicstyle=\ttfamily]!String!, \lstinline[basicstyle=\ttfamily]!Boolean!, or 
\lstinline[basicstyle=\ttfamily]!enumeration! variables.
...
\emph{There are no continuous-time} \lstinline[basicstyle=\ttfamily]!Boolean!, \lstinline[basicstyle=\ttfamily]!Integer! \emph{or} \lstinline[basicstyle=\ttfamily]!String!
\emph{variables.} \emph{In the rare cases they are needed they can be
faked by using} \lstinline[basicstyle=\ttfamily]!Real! variables, \emph{e.g.:}
\begin{lstlisting}[language=modelica]
  Boolean off1, off1a;
  Real off2;
equation
...
\emph{Since} \lstinline[basicstyle=\ttfamily]!off1! \emph{is a discrete-time variable, state events are
generated such that} \lstinline[basicstyle=\ttfamily]!off1! \emph{is only changed at event instants.
Variable} \lstinline[basicstyle=\ttfamily]!off2! \emph{may change its value during continuous integration.
Therefore,} \lstinline[basicstyle=\ttfamily]!u1! \emph{is guaranteed to be continuous during continuous
integration whereas no such guarantee exists for} \lstinline[basicstyle=\ttfamily]!u2!\emph{.}
...
A component declaration can have a \lstinline[basicstyle=\ttfamily]!condition-attribute!: "\lstinline[basicstyle=\ttfamily]!if!"
\emph{expression}.
...
classes \lstinline[basicstyle=\ttfamily]!Integer! and \lstinline[basicstyle=\ttfamily]!Real!, to large packages such as the Modelica
standard library.
...
\end{lstlisting}
\emph{the types of} \lstinline[basicstyle=\ttfamily]!f1! \emph{and} \lstinline[basicstyle=\ttfamily]!f2! \emph{are identical.}{]}
...
components that are partial or \lstinline[basicstyle=\ttfamily]!outer!. The environment is the
...
\emph{Flattening of class} \lstinline[basicstyle=\ttfamily]!C2! \emph{yields a local class Voltage with
nominal-modifier 1000. The variables} \lstinline[basicstyle=\ttfamily]!v1! \emph{and} \lstinline[basicstyle=\ttfamily]!v2! \emph{are
...
classes}{]} \lstinline[basicstyle=\ttfamily]!record, type, model, block, package, function,! and \lstinline[basicstyle=\ttfamily]!connector!
...
\hline \endhead
\lstinline[basicstyle=\ttfamily]!record! & Only public sections are allowed in the definition or
...
of a record may not have prefixes \lstinline[basicstyle=\ttfamily]!input!, \lstinline[basicstyle=\ttfamily]!output!,
\lstinline[basicstyle=\ttfamily]!inner!, \lstinline[basicstyle=\ttfamily]!outer!, \lstinline[basicstyle=\ttfamily]!stream,!
or \lstinline[basicstyle=\ttfamily]!flow!. Enhanced with implicitly available record constructor function,
...
and type..\\ \hline
\lstinline[basicstyle=\ttfamily]!type! & May only be predefined types, enumerations, array of
type, or classes extending from type.\\ \hline
\lstinline[basicstyle=\ttfamily]!model! & Identical to \lstinline[basicstyle=\ttfamily]!class!, the basic class concept, i.e., no
restrictions and no enhancements.\\ \hline
\lstinline[basicstyle=\ttfamily]!block! & Same as \lstinline[basicstyle=\ttfamily]!model! with the restriction that each connector
component of a \lstinline[basicstyle=\ttfamily]!block! must have prefixes \lstinline[basicstyle=\ttfamily]!input! and/or \lstinline[basicstyle=\ttfamily]!output! for all
...
\lstinline[basicstyle=\ttfamily]!function! & See \autoref{function-as-a-specialized-class} for restrictions
...
property.}{]}\\ \hline
\lstinline[basicstyle=\ttfamily]!connector! & Only public sections are allowed in the definition or in any of its
components (i.e., equation, algorithm, initial equation, initial
algorithm and protected sections are not allowed).

Enhanced to allow \lstinline[basicstyle=\ttfamily]!connect!(..) to components of connector classes. The
elements of a connector may not have prefixes \lstinline[basicstyle=\ttfamily]!inner!, or \lstinline[basicstyle=\ttfamily]!outer.! May only
contain components of specialized class connector, record and
type.\\ \hline
\lstinline[basicstyle=\ttfamily]!package! & May only contain declarations of classes and
constants. Enhanced to allow \lstinline[basicstyle=\ttfamily]!import! of elements of packages. (See also
\autoref{packages} on packages.)\\ \hline
\lstinline[basicstyle=\ttfamily]!operator record! & Similar to record; but operator overloading
...
\lstinline[basicstyle=\ttfamily]!operator! & Similar to package; but may only contain
declarations of functions. May only be placed directly in an operator
record. (See also \autoref{overloaded-operators}).\\ \hline
\lstinline[basicstyle=\ttfamily]!operator function! & Shorthand for an
...
\emph{In this case one can argue that both} \lstinline[basicstyle=\ttfamily]!UseCorrelation! \emph{(adding
an acausal equation) and} \lstinline[basicstyle=\ttfamily]!SpecialCorrelation! \emph{(adding a default to
...
\emph{In Modelica 2.2, model} \lstinline[basicstyle=\ttfamily]!Broken! \emph{will work with some models.
However, by just redeclaring it to model} \lstinline[basicstyle=\ttfamily]!SpecialCorrelation!\emph{, an
error will occur and it will be very difficult in a larger model to
figure out the source of this error. }

\emph{In Modelica 3.0, model} \lstinline[basicstyle=\ttfamily]!UseCorrelation! \emph{is no longer allowed
...
\begin{itemize}
\item
  For each declared component of specialized class \lstinline[basicstyle=\ttfamily]!type! (Real, Integer,
  String, Boolean, enumeration and arrays of those, etc.) or \lstinline[basicstyle=\ttfamily]!record!, or
  \lstinline[basicstyle=\ttfamily]!operator record! not declared as \lstinline[basicstyle=\ttfamily]!outer!, it is the ``number of unknown
...
\item
  Each declared component of specialized class \lstinline[basicstyle=\ttfamily]!type! or record declared
  as \lstinline[basicstyle=\ttfamily]!outer! is ignored {[}\emph{i.e., all variables inside the component
  are treated as known}{]}.
\item
  For each declared component of specialized class \lstinline[basicstyle=\ttfamily]!connector! component,
...
\item
  For each declared component of specialized class \lstinline[basicstyle=\ttfamily]!block! or \lstinline[basicstyle=\ttfamily]!model!, it is
...
\item
  ...
  model contains a component,} \lstinline[basicstyle=\ttfamily]!firstOrder! \emph{(of specialized class}
  \lstinline[basicstyle=\ttfamily]!model!\emph{) and} \lstinline[basicstyle=\ttfamily]!firstOrder! \emph{has `}\lstinline[basicstyle=\ttfamily]!input Real u!\emph{' then
  there must be a binding equation for} \lstinline[basicstyle=\ttfamily]!firstOrder.u!\emph{.}{]}
\item
...
\item
  ...
  {[}\emph{Example:}\\
\lstinline[basicstyle=\ttfamily]!Flange support(phi=phi, tau=torque1+torque2) if use_support;!\\
...
\emph{Model} \lstinline[basicstyle=\ttfamily]!Capacitor! \emph{is a locally balanced model according to
the following analysis:}

\emph{Locally unknown variables}: \lstinline[basicstyle=\ttfamily]!p.i, p.v, n.i, n.v, u!

%TODO-BACK Check text
\emph{Local equations}: 
\begin{eqnarray*}
0 &=& \text{p.i} + \text{n.i};\\
\text{u} &=& \text{p.v} - \text{n.v};\\
\text{C}*\text{der}(u) &=& \text{p.i};
\end{eqnarray*}
\emph{and 2 equations corresponding to the 2 flow-variables} \lstinline[basicstyle=\ttfamily]!p.i!
\emph{and} \lstinline[basicstyle=\ttfamily]!n.i.!
...
\emph{If the equation} ``\lstinline[basicstyle=\ttfamily]!u = p.v - n.v!'' \emph{would be missing in the
...
\emph{If the equation} ``\lstinline[basicstyle=\ttfamily]!u = p.v - n.v!\emph{'' would be replaced by the
equation ``}\lstinline[basicstyle=\ttfamily]!u = 0!\emph{'' and the equation} \lstinline[basicstyle=\ttfamily]!C*der(u) = p.i! \emph{would
be replaced by the equation ``}\lstinline[basicstyle=\ttfamily]!C*der(u) = 0!\emph{'', there would be 5
...
\textbf{globally balanced model}, but we can check that} \lstinline[basicstyle=\ttfamily]!Circuit!
\emph{is \textbf{locally balanced}.}

\emph{Counting on model} \lstinline[basicstyle=\ttfamily]!Circuit! \emph{results in the following balance
sheet:}

\emph{Locally unknown variables (8):} \lstinline[basicstyle=\ttfamily]!p.i, p.v, n.i, n.v!\emph{, and 2
flow variables for} \lstinline[basicstyle=\ttfamily]!t (t.p.i, t.n.i)!

\emph{and 2 flow variable for} \lstinline[basicstyle=\ttfamily]!c (c.p.i, c.n.i).!
...
variables} \lstinline[basicstyle=\ttfamily]!p!\emph{,} \lstinline[basicstyle=\ttfamily]!h!\emph{,} \lstinline[basicstyle=\ttfamily]!Xi! \emph{are marked as input to get
...
\emph{This partial model defines that} \lstinline[basicstyle=\ttfamily]!T,d,u! \emph{can be computed from
the medium model, provided} \lstinline[basicstyle=\ttfamily]!p,h,Xi! \emph{are given. Every medium with
one or multiple substances and one or multiple phases, including
incompressible media, has the property that} \lstinline[basicstyle=\ttfamily]!T,d,u! \emph{can be computed
from} \lstinline[basicstyle=\ttfamily]!p,h,Xi!\emph{. A particular medium may have different ``independent
...
\emph{The local number of unknowns in model} \lstinline[basicstyle=\ttfamily]!SimpleAir! \emph{(after
flattening) is:}

\begin{itemize}
\item
  3 \emph{(}\lstinline[basicstyle=\ttfamily]!T!\emph{,} \lstinline[basicstyle=\ttfamily]!d!\emph{,} \lstinline[basicstyle=\ttfamily]!u!\emph{: variables defined in}
  \lstinline[basicstyle=\ttfamily]!BaseProperties! \emph{and inherited in} \lstinline[basicstyle=\ttfamily]!SimpleAir!\emph{), plus}
\item
  2\emph{+}\lstinline[basicstyle=\ttfamily]!nXi! \emph{(}\lstinline[basicstyle=\ttfamily]!p!\emph{,} \lstinline[basicstyle=\ttfamily]!h!\emph{,} 
	\lstinline[basicstyle=\ttfamily]!Xi!\emph{: variables inside
  connectors defined in} \lstinline[basicstyle=\ttfamily]!BaseProperties! \emph{and inherited in}
  \lstinline[basicstyle=\ttfamily]!SimpleAir!\emph{)}
\end{itemize}
...
\emph{The generic medium model} \lstinline[basicstyle=\ttfamily]!BaseProperties! \emph{is used as a
...
\emph{The local number of unknowns of} \lstinline[basicstyle=\ttfamily]!DynamicVolume! \emph{is:}
...
\item
  2\emph{+}nXi \emph{flow variables in the} \lstinline[basicstyle=\ttfamily]!port! \emph{connector. }
\end{itemize}

\emph{Therefore,} \lstinline[basicstyle=\ttfamily]!DynamicVolume! \emph{is a \textbf{locally balanced}
model.}

\emph{Note, when the} \lstinline[basicstyle=\ttfamily]!DynamicVolume! \emph{is used and the} \lstinline[basicstyle=\ttfamily]!Medium!
\emph{model is redeclared to ``}\lstinline[basicstyle=\ttfamily]!SimpleAir!\emph{'', then a tool will try
to select} \lstinline[basicstyle=\ttfamily]!p,T! \emph{as states, since these variables have}
\lstinline[basicstyle=\ttfamily]!StateSelect.prefer! \emph{in the} \lstinline[basicstyle=\ttfamily]!SimpleAir! \emph{model (this means that
the default states} \lstinline[basicstyle=\ttfamily]!U,M! \emph{are derived quantities). If this state
selection is performed, all intrinsic medium variables are computed
from} \lstinline[basicstyle=\ttfamily]!medium.p! \emph{and} \lstinline[basicstyle=\ttfamily]!medium.T!\emph{, although} 
\lstinline[basicstyle=\ttfamily]!p! \emph{and} \lstinline[basicstyle=\ttfamily]!h!
...
for} \lstinline[basicstyle=\ttfamily]!p,h,Xi!\emph{, in order that the equation count is correct. The
actual computational causality can be different as it is demonstrated
with the} \lstinline[basicstyle=\ttfamily]!SimpleAir! \emph{model.}
...
\item
  4+2\emph{*}nXi \emph{(inside the} port \emph{connector), plus}
\item
  2+nXi \emph{(the input variables in the connectors of the} \lstinline[basicstyle=\ttfamily]!medium!
  \emph{model)}
\end{itemize}
...
%TODO-BACK Format port.h
\emph{Therefore,} \lstinline[basicstyle=\ttfamily]!FixedBoundary_pTX! \emph{is a \textbf{locally
balanced} model. The predefined boundary variables} \lstinline[basicstyle=\ttfamily]!p! \emph{and} \lstinline[basicstyle=\ttfamily]!Xi!
\emph{are provided via equations to the input arguments} \lstinline[basicstyle=\ttfamily]!medium.p!
\emph{and} \lstinline[basicstyle=\ttfamily]!medium.Xi!\emph{, in addition there is an equation for} \lstinline[basicstyle=\ttfamily]!T!
\emph{in the same way -- even though} \lstinline[basicstyle=\ttfamily]!T! \emph{is not an input. Depending
on the flow direction, either the specific enthalpy in the port
(}\lstinline[basicstyle=\ttfamily]!port.h!\emph{) or h is used to compute the enthalpy flow rate}
\lstinline[basicstyle=\ttfamily]!H_flow!\emph{. ``h'' is provided as binding equation to the medium. With
the equation ``}\lstinline[basicstyle=\ttfamily]!medium.T = T!\emph{'', the specific enthalpy ``h'' of the
...
definitions use \lstinline[basicstyle=\ttfamily]!RealType!, \lstinline[basicstyle=\ttfamily]!IntegerType!,
\lstinline[basicstyle=\ttfamily]!BooleanType!, \lstinline[basicstyle=\ttfamily]!StringType!, \lstinline[basicstyle=\ttfamily]!EnumType!
as mnemonics corresponding to machine representations. {[}\emph{Hence
the only way to declare a subtype of e.g.} \lstinline[basicstyle=\ttfamily]!Real! \emph{is to use the}
\lstinline[basicstyle=\ttfamily]!extends! \emph{mechanism.}{]}

\subsection{Real Type}\doublelabel{real-type}

The following is the predefined \lstinline[basicstyle=\ttfamily]!Real! type:
...
The following is the predefined \lstinline[basicstyle=\ttfamily]!Integer! type:
\begin{lstlisting}[language=Modelica]
...
The following is the predefined \lstinline[basicstyle=\ttfamily]!Boolean! type:
\begin{lstlisting}[language=Modelica]
...
The following is the predefined \lstinline[basicstyle=\ttfamily]!String! type:
\begin{lstlisting}[language=Modelica]
...
inside the scope of E. Each enumeration literal in the \lstinline[basicstyle=\ttfamily]!enum-list! has
type E.
...
The type conversion function \lstinline[basicstyle=\ttfamily]!Integer(<expression of enumeration type>)! returns the ordinal number of the
enumeration value \lstinline[basicstyle=\ttfamily]!E.enumvalue!, to which the expression is evaluated,
where \lstinline[basicstyle=\ttfamily]!Integer(E.e1) =1, Integer(E.en) = n!, for an enumeration type
\lstinline[basicstyle=\ttfamily]!E=enumeration(e1, ..., en)!.

\lstinline[basicstyle=\ttfamily]!String(E.enumvalue)! gives the string representation of the enumeration
value. 

{[}\emph{Example:} \lstinline[basicstyle=\ttfamily]!String(E.Small)! \emph{gives
"}\lstinline[basicstyle=\ttfamily]!Small!\emph{".}{]}
...
For an enumeration type named \lstinline[basicstyle=\ttfamily]!EnumTypeName!, the expression
\lstinline[basicstyle=\ttfamily]!EnumTypeName(<Integer expression>)! returns the
enumeration value \lstinline[basicstyle=\ttfamily]!EnumTypeName.e! such that \lstinline[basicstyle=\ttfamily]!Integer(EnumTypeName.e)! is
...
The attributes \lstinline[basicstyle=\ttfamily]!start! and \lstinline[basicstyle=\ttfamily]!fixed! define the initial conditions for a
variable. ``\lstinline[basicstyle=\ttfamily]!fixed=false!'' means an initial guess, i.e., value may be
changed by static analyzer. ``\lstinline[basicstyle=\ttfamily]!fixed=true!'' means a required value. The
resulting consistent set of values for ALL model variables is used as
initial values for the analysis to be performed.

The attribute \lstinline[basicstyle=\ttfamily]!nominal! gives the nominal value for the variable. The user
...
If ``\lstinline[basicstyle=\ttfamily]!unbounded=true!'' it indicates that the state may grow without
...
``}\lstinline[basicstyle=\ttfamily]!tol*(abs(nominal)+(if x.unbounded then 0 else abs(x)))!\emph{''. A
default value is not provided in order that in cases such as
``}\lstinline[basicstyle=\ttfamily]!a=b!\emph{'', where ``}\lstinline[basicstyle=\ttfamily]!b!\emph{'' has a nominal value but not
``}\lstinline[basicstyle=\ttfamily]!a!\emph{'', the nominal value can be propagated to the other
variable).}{]} {[}\emph{For external functions in C89,} \lstinline[basicstyle=\ttfamily]!RealType!
...
\lstinline[basicstyle=\ttfamily]!RealType/double! \emph{matches the IEC 60559:1989 (ANSI/IEEE 754-1985)
double format. Typically} \lstinline[basicstyle=\ttfamily]!IntegerType! \emph{represents a 32-bit
...
The predefined \lstinline[basicstyle=\ttfamily]!StateSelect! enumeration type is the type of the
\lstinline[basicstyle=\ttfamily]!stateSelect! attribute of the \lstinline[basicstyle=\ttfamily]!Real! type. It is used to explicitly control
state selection.
...
The predefined \lstinline[basicstyle=\ttfamily]!AssertionLevel! enumeration type is used together with
\lstinline[basicstyle=\ttfamily]!assert!, \autoref{assert}.
\begin{lstlisting}[language=Modelica]
