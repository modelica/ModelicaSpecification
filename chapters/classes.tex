...
\item
  ...
  (informally \lstinline!der(vd)=0! between events and possibly infinite derivative at event,
  but it is not legal to apply the \lstinline!der()! operator
  to discrete-time variables as they are not continuous) and can change its values only at event
