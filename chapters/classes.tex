...
\subsection{Acyclic Bindings of Constants and Structural Parameters}\label{acyclic-bindings-of-constants-and-parameters}
...
\item
  ...
  \item
    The expression of its declaration equation does not depend on \lstinline!vs! itself, neither directly nor via cyclic dependency on other parameters.
    To satisfy this condition, dependencies shall be removed as needed by applying simplifications based on values of constants (except with \lstinline!Evaluate = false!)  and all other structural parameters that don't depend on \lstinline!vs!.
...
\begin{lstlisting}[language=modelica]
model ABCD
  parameter Real A[n, n]; // Requires that n is a structural parameter.
  parameter Integer n = size(A, 1);
end ABCD;

final ABCD a;
// Illegal since cyclic dependencies between size(a.A, 1) and a.n makes n a normal parameter.

ABCD b(redeclare Real A[2, 2] = [1, 2; 3, 4]);
// Legal since size of A is no longer dependent on n, making n a structural parameter.

ABCD c(n = 2); // Legal since n is no longer dependent on the size of A.

parameter Real r = 2 * sin(r); // Normal parameter, since r = 2 * sin(r) is cyclic.
...
// Cycle for T_start and h_start, but still structural since cycle disappears
// when evaluating use_T_start

// The unexpanded bindings have cycles for both x and y, making them normal parameters
