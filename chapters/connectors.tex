...
\begin{semantics}
The overdetermined type or record instance \lstinline!R! in connector instance \lstinline!a! is a \firstuse[definite root node]{(definite) root node}\index{root node!definite} in a virtual connection graph.
If there are any other roots (normal, unique, or potential) that is an error and the message argument will be used in the diagnostics.
If two unique roots are connected and the message arguments are different either or both arguments may be used.
...
It is used for the \lstinline!Modelica_StateGraph2!.
\end{nonnormative}
\end{semantics}
\end{operatordefinition}
...
\end{lstlisting}\end{synopsis}
\begin{semantics}
Returns an index for the connections between a and b, where msg is used in case of error.

\begin{nonnormative}
This is used for checking parallel connections in \lstinline!Modelica_StateGraph2!.
\end{nonnormative}
\end{semantics}
\end{operatordefinition}
