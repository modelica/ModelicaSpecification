...
A connect-equation\index{connect@\robustinline{connect}} has the following syntax:
...
If the \lstinline!expandable!\index{expandable@\robustinline{expandable}} qualifier is present on a connector definition, all instances of that connector are referred to as \firstuse{expandable connectors}\index{expandable connector}.
...
In order to generate equations for flow variables (using the \lstinline!flow!\index{flow@\robustinline{flow}} prefix), the sign used for the connector variable $z_{i}$ above is +1 for inside connectors and -1 for outside connectors ($z_{3}$ in the example above).
