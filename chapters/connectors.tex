...
This generation replaces the equality-equation for variables that are neither flow nor stream in \cref{generation-of-connection-equations}.

In order to handle any redundant connect-equation the following preparation is needed.
\begin{enumerate}
\item
  We build the connection sets in the normal way, keeping the overdetermined components as primitives.
\item
...
\end{enumerate}
If there are no redundant connect-equations the breakable branches can be chosen to correspond to the connect-equations for overdetermined connectors.
If there are redundant connect-equations there will always exist \lstinline!connect!-equations without a corresponding breakable branch
