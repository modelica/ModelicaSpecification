...
As a preparation of generating connection graph equations, any redundant breakable branches are removed in each connection set:
\begin{enumerate}
\item
  This is done by building a spanning tree within each connection set.
\item
  Breakable branches that are not part of the spanning tree are redundant and removed.
  (The exact set of branches that are considered redundant is not defined, as it depends on the choice of spanning tree.)
\end{enumerate}

The virtual connection graph is then transformed in the following way:
\begin{enumerate}
\item
...
\item
  For every optional non-redundant spanning-tree edge not in any of the spanning trees, the connection equations are generated according to \cref{generation-of-connection-equations}, except for overdetermined type or record instances \lstinline!R!.
  Here the equations \lstinline!0 = R.equalityConstraint(A.R, B.R)! are generated instead of \lstinline!A.R = B.R!.
\item
  For every redundant and removed spanning-tree edge (i.e., a \lstinline!connect(A, B)! equation), not in any of the spanning trees, the connection equations for flow-elements are generated according to \cref{generation-of-connection-equations}.
  Non-flow elements will not generate any equations.
\end{enumerate}
