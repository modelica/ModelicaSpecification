...
\item
  ...
  The restrictions for the lookup are:
  \begin{itemize}
  \item If the class is declared as \lstinline!package! or \lstinline!operator! (see below) all elements can be found.
  \item If the element is declared as \lstinline!encapsulated!\indexinline{encapsulated} it can also be found in other cases.
  \item A deprecated case is that if the class satisfies the restrictions for a \lstinline!package! (without being declared as such), it is still treated as a \lstinline!package!.
...
\begin{nonnormative}
Note that a \lstinline!operator! class may only contain declarations of functions and thus fulfil the requirements for a package (see \cref{specialized-classes}).
