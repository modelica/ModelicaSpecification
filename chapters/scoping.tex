...
\begin{itemize}
\item
  Among declared named elements (\lstinline[basicstyle=\ttfamily]!class-definition! and
  \lstinline[basicstyle=\ttfamily]!component-declaration!) of the class (including elements inherited from
  base-classes).
\item
  Among the import names of qualified import statements in the
  \textbf{instance} scope. The import name of \lstinline[basicstyle=\ttfamily]!import A.B.C!; is \lstinline[basicstyle=\ttfamily]!C! and the
  import name of \lstinline[basicstyle=\ttfamily]!import D=A.B.C;! is \lstinline[basicstyle=\ttfamily]!D!.
\item
...
For a composite name of the form \lstinline[basicstyle=\ttfamily]!A.B! or \lstinline[basicstyle=\ttfamily]!A.B.C!, etc. lookup is performed
as follows:

\begin{itemize}
\item
  The first identifier (\lstinline[basicstyle=\ttfamily]!A!) is looked up as defined above.
\item
  If the first identifier denotes a component, the rest of the name
  (e.g., \lstinline[basicstyle=\ttfamily]!B! or \lstinline[basicstyle=\ttfamily]!B.C!) is looked up among the declared named component
...
\item
  ...
  The rest of the name (e.g., \lstinline[basicstyle=\ttfamily]!B! or \lstinline[basicstyle=\ttfamily]!B.C!{]} is looked up among the
...
that the environment is empty. See also} \lstinline[basicstyle=\ttfamily]!MoistAir2! \emph{example in
...
For a name starting with dot, e.g.: \lstinline[basicstyle=\ttfamily]!.A! {[}\emph{or} \lstinline[basicstyle=\ttfamily]!.A.B!\emph{,} 
\lstinline[basicstyle=\ttfamily]!.A.B.C!
...
\item
  ...
  of the name [e.g., \lstinline[basicstyle=\ttfamily]!B! or \lstinline[basicstyle=\ttfamily]!B.C!] is looked up among the declared named
...
An element declared with the prefix \lstinline[basicstyle=\ttfamily]!outer! references an element instance
with the same name but using the prefix \lstinline[basicstyle=\ttfamily]!inner! which is nearest in the
enclosing instance hierarchy of the \lstinline[basicstyle=\ttfamily]!outer! element declaration.

An \lstinline[basicstyle=\ttfamily]!outer! element reference in a simulation model requires that one
corresponding \lstinline[basicstyle=\ttfamily]!inner! element declaration exist or can be created in a
unique way:

\begin{itemize}
\item
  If there are two (or more) \lstinline[basicstyle=\ttfamily]!outer! declarations with the same name, both
  lacking matching \lstinline[basicstyle=\ttfamily]!inner! declarations, and the \lstinline[basicstyle=\ttfamily]!outer! declarations are
  not of the same class it is in error.
\item
  If there is one (or more) \lstinline[basicstyle=\ttfamily]!outer! declarations of a partial class it is
  an error.
\item
  In other cases, i.e. if a unique non-partial class is used for all
  \lstinline[basicstyle=\ttfamily]!outer! declarations of the same name lacking a matching inner
  declaration, then an \lstinline[basicstyle=\ttfamily]!inner! declaration of that class is automatically
...
An \lstinline[basicstyle=\ttfamily]!outer! element component may be of a partial class {[}\emph{but the
referenced} \lstinline[basicstyle=\ttfamily]!inner! \emph{component must be of a non-partial class}{]}.
{[}\lstinline[basicstyle=\ttfamily]!inner!\emph{/}\lstinline[basicstyle=\ttfamily]!outer! \emph{components may be used to model simple
...
corresponding} \lstinline[basicstyle=\ttfamily]!inner! \emph{declaration in a more deeply nested level of
the model hierarchy.}{]}
...
The \lstinline[basicstyle=\ttfamily]!inner! component shall be a subtype of the corresponding \lstinline[basicstyle=\ttfamily]!outer!
component. {[}\emph{If the two types are not identical, the type of the}
\lstinline[basicstyle=\ttfamily]!inner! \emph{component defines the instance and the} \lstinline[basicstyle=\ttfamily]!outer!
\emph{component references just part of the} \lstinline[basicstyle=\ttfamily]!inner! \emph{component}{]}.
...
An element declared with both the prefixes \lstinline[basicstyle=\ttfamily]!inner! and \lstinline[basicstyle=\ttfamily]!outer! conceptually
introduces two declarations with the same name: one that follows the
above rules for \lstinline[basicstyle=\ttfamily]!inner! and another that follows the rules for \lstinline[basicstyle=\ttfamily]!outer!.
{[}\emph{Local references for elements with both the prefix} \lstinline[basicstyle=\ttfamily]!inner!
\emph{and} \lstinline[basicstyle=\ttfamily]!outer! \emph{references the} \lstinline[basicstyle=\ttfamily]!outer! \emph{element. That in turn
references the corresponding element in an enclosing scope with the
prefix} \lstinline[basicstyle=\ttfamily]!inner!\emph{.}{]}

Outer component declarations may only have modifications
{[}\emph{including declaration equations}{]} if they also have the \lstinline[basicstyle=\ttfamily]!inner!
...
\lstinline[basicstyle=\ttfamily]!inner! prefix. For both cases those modifications are only applied to the
\lstinline[basicstyle=\ttfamily]!inner! declaration.
...
further functions, e.g. from \lstinline[basicstyle=\ttfamily]!derivative! annotation or from \lstinline[basicstyle=\ttfamily]!inverse!
