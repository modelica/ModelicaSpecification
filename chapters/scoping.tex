...
that the environment is empty. See also} \lstinline!MoistAir2! \emph{example in
...
The \lstinline!inner! component shall be a subtype of the corresponding \lstinline!outer!
component. {[}\emph{If the two types are not identical, the type of the}
\lstinline!inner! \emph{component defines the instance and the} \lstinline!outer!
\emph{component references just part of the} \lstinline!inner! \emph{component}{]}.
...
\emph{To recursively instantiate \lstinline!M! allowing the generation of flat
...
\emph{Note that if \lstinline!D! was consistently replaced by \lstinline!A! in the example
