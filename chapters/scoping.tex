...
A class declared with the keyword \lstinline!encapsulated!\index{encapsulated@\robustinline{encapsulated}} (see \lstinline[language=grammar]!class-definition! in the grammar) is called an \firstuse{encapsulated} class.
...
\item
  ...
  If the class does not satisfy the requirements for a package, the lookup is restricted to encapsulated\index{encapsulated@\robustinline{encapsulated}} elements only.
...
\item
  ...
  This is possible even if the class is encapsulated\index{encapsulated@\robustinline{encapsulated}} and import-clauses are not used for this.
...
An element declared with the prefix \lstinline!outer!\index{outer@\robustinline{outer}} references an element instance with the same name but using the prefix \lstinline!inner!\index{inner@\robustinline{inner}} which is nearest in the enclosing instance hierarchy of the \lstinline!outer! element declaration.
