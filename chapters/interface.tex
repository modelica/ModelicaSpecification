...
with the \lstinline!input! prefix, because then they must be connected.}{]} \lstinline!A!
model or block \lstinline!A! cannot be used instead of \lstinline!B!, if the particular
...
\item
  If A has an \lstinline!operator record! base-class then \lstinline!B! must also have one and
...
\item
  ...
  \item
    \lstinline!A! function class is only compatible with a function class, a package
