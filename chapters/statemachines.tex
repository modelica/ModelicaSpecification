...
since the active state last became active. In a hierarchical state machine where the enclosing state became active after local transitions
it is the ticks since the enclosing state became active.
This function can only be used in state machines.\\ \hline
\textbf{timeInState()} & Returns the time duration as Real in {[}s{]}
since the active state last became active. In a hierarchical state machine where the enclosing state became active after local transitions
it is the time duration since the enclosing state became active.
This function can only be used in state machines.\\ \hline
