...
\item
  ...
  This implies that states are active for at least one tick, even if an \emph{immediate} exit transition is active when entering the state.
\item
...
\item
  ...
  All equations for the variables defined in the state must be inside the state (this corresponds to restriction to balanced models, \cref{balanced-models}).
\end{itemize}
...
The special kinds of \lstinline!connect!-like equations listed below are used to define a state machine.
