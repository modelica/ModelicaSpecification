...
\subsubsection{Character Encoding and Byte Order Mark}\doublelabel{character-encoding-and-byte-order-mark}
...
A non-top-level entity shall begin with a within-clause (following any language version comment, see \autoref{language-version-comment}) which for the
...
\subsubsection{Language Version Comment}\doublelabel{language-version-comment}

Each Modelica file must use a specific version of the Modelica language.  This information may either be specified in the first line of a Modelica file, or be inherited within a package/class hierarchy.

When specified in the first line of a Modelica file, that line (after stripping any byte order mark) is called a \emph{language version comment}, and shall match the following regular expression:
% Abuse of lstlisting language 'grammar' below; consider defining a separate language for regular expressions.
\begin{lstlisting}[language=grammar]
^//![ ]\d+[.]\d+[r.]\d+$
\end{lstlisting}

\begin{example}
Language version comment specifying the language release \emph{Modelica 3.4 Revision 1}:
\begin{lstlisting}[language=modelica]
//! 3.4r1
\end{lstlisting}
\end{example}

The Modelica language version is given by the characters following the space character.  The two first sequences of digits are called the \emph{major} and \emph{minor} components of the version, while the last part is called either the \emph{revision} or the \emph{patch} component, depending on the separating character being \lstinline[language=grammar]![r]! or \lstinline[language=grammar]![.]!.  (The possibility to specify a verison with patch component is reserved for use with future language versioning systems.)  A language version consisting of only a major and minor component is extended with a revision component of ``0'' when referenced in a language version comment.

\begin{example}
Specification of language release \emph{Modelica 3.4}:
\begin{lstlisting}[language=modelica]
//! 3.4r0
\end{lstlisting}
\end{example}

For a complete list of existing Modelica versions, see \autoref{modelica-revision-history}.

When a package/class hierarchy is mapped to a directory hierarchy (see \autoref{mapping-a-package-class-hierarchy-into-a-single-file-nonstructured-entity}), a language version specified in a structured entity's \emph{package.mo} file is inherited by all sub-entities, and can be overridden by language version comments deeper down in the directory hierarchy.
...
A tool must not warn when reading a file using a language version that it can handle without risk of problems.
