...
An algorithm section is comprised of the keyword \lstinline[basicstyle=\ttfamily]!algorithm! followed by a
...
Equation equality \lstinline[basicstyle=\ttfamily]!=! or any other kind of equation (see \autoref{equations}) shall
...
The \lstinline[basicstyle=\ttfamily]!expression! is evaluated. The resulting value is stored into the
variable denoted by \lstinline[basicstyle=\ttfamily]!component-reference!.
...
{[}\emph{Example: The function} \lstinline[basicstyle=\ttfamily]!f! \emph{called below has three results
and two inputs}:
...
\end{lstlisting}
For-statements may optionally use several iterators (\lstinline[basicstyle=\ttfamily]!for-indices!), see
...
\end{lstlisting}
The \lstinline[basicstyle=\ttfamily]!expression! of a for-statement shall be a vector expression. It is
evaluated once for each for-statement, and is evaluated in the scope
immediately enclosing the for-statement. The loop-variable (\lstinline[basicstyle=\ttfamily]!IDENT!) is in
...
An iterator \lstinline[basicstyle=\ttfamily]!IDENT in range-expr! without the \lstinline[basicstyle=\ttfamily]!in range-expr! requires that
the \lstinline[basicstyle=\ttfamily]!IDENT! appears as the subscript of one or several subscripted
expressions. The dimension size of the array expression in the indexed
position is used to deduce the \lstinline[basicstyle=\ttfamily]!range-expr! as
\lstinline[basicstyle=\ttfamily]!1:size(array-expression,indexpos)! if the indices are a subtype of
Integer, or as \lstinline[basicstyle=\ttfamily]!E.e1:E.en! if the indices are of an enumeration type
\lstinline[basicstyle=\ttfamily]!E=enumeration(e1, ..., en)!, or as \lstinline[basicstyle=\ttfamily]!false:true! if the indices are of
type Boolean. If it is used to subscript several expressions, their
ranges must be identical. The \lstinline[basicstyle=\ttfamily]!IDENT! may also, inside a
...
reference to the variable \lstinline[basicstyle=\ttfamily]!IDENT!, and not for deducing ranges.
...
The iteration range can be specified as \lstinline[basicstyle=\ttfamily]!Boolean! or as an enumeration
type. This means iteration over the type from min to max, i.e. for
Boolean it is the same as \lstinline[basicstyle=\ttfamily]!false:true! and for an enumeration E it is the
same as E.min:E.max. This can be used for \lstinline[basicstyle=\ttfamily]!for! loops and reduction
expressions.
...
by replacing each ``,'' by `\lstinline[basicstyle=\ttfamily]!loop for!' and adding extra `\lstinline[basicstyle=\ttfamily]!end for!'. For
reduction-expressions it can be expanded into the usual form by
replacing each `,' by `\lstinline[basicstyle=\ttfamily]!) for!' and prepending the reduction-expression
with `\lstinline[basicstyle=\ttfamily]!function-name(!`.
...
\end{lstlisting}
The \lstinline[basicstyle=\ttfamily]!expression! of a while-statement shall be a scalar Boolean
...
\begin{enumerate}
\item The \lstinline[basicstyle=\ttfamily]!expression! of the while-statement is evaluated.
\item If the \lstinline[basicstyle=\ttfamily]!expression! of the while-statement is false, the execution
continues after the while-statement.
\item If the \lstinline[basicstyle=\ttfamily]!expression! of the while-statement is true, the entire body of
...
The \lstinline[basicstyle=\ttfamily]!expression! of an if- or elseif-clause must be scalar Boolean
...
\end{lstlisting}
The \lstinline[basicstyle=\ttfamily]!expression! of a when-statement shall be a discrete-time Boolean
...
\emph{Algorithms are activated when} \lstinline[basicstyle=\ttfamily]!x! \emph{becomes \textgreater{} 2:}
...
either} \lstinline[basicstyle=\ttfamily]!x! \emph{becomes \textgreater{} 2 or} \lstinline[basicstyle=\ttfamily]!sample(0,2)! \emph{becomes}
\lstinline[basicstyle=\ttfamily]!true! \emph{or} \lstinline[basicstyle=\ttfamily]!x! \emph{becomes less than 5:}
...
assignment to} \lstinline[basicstyle=\ttfamily]!y1! \emph{and} \lstinline[basicstyle=\ttfamily]!y3! \emph{in the algorithm.}
...
\end{lstlisting}
is similar to the following special if-statement, where \lstinline[basicstyle=\ttfamily]!Boolean b1[N];! and \lstinline[basicstyle=\ttfamily]!Boolean b2;! are necessary because the \lstinline[basicstyle=\ttfamily]!edge()! operator can
...
\end{lstlisting}
  with \lstinline[basicstyle=\ttfamily]!edge(A)= A and not pre(A)! and the additional guarantee, that the
