...
\begin{nonnormative}
The predominant use of unscaled SI units in \lstinline!unit!-attributes in libraries (as this is a good way to avoid unit inconsistency without tool assistance to verify unit consistency) should be kept in mind when using unitful literals where there is no implicit unit conversion.
That is, using units which are not equivalent to unscaled SI units for those unitful literals is likely to be a mistake.
