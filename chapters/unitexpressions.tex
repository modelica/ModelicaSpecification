...
If an implicit unit conversion would be different for absolute and relative quantities (due to unit offsets), the implicit unit conversion is only allowed if the modified component (or its type) has an \lstinline!absoluteValue! annotation (\cref{absoluteValue}).
