...
The set of allowed characters is currently not defined, but it is recommended to only use \lstinline[language=grammar]!NON-DIGIT!.
...
Similarly degree is spelled out as \lstinline!deg! for angles, and \lstinline!degC!, \lstinline!degF! and \lstinline!degRk! for temperatures (Celsius, Fahrenheit and Rankine).
...
In addition a tool may present \lstinline!deg! as $^{\circ}$ (for angle) and \lstinline!degC! as $^{\circ}$C (for temperature).
