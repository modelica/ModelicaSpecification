...
If an implicit unit conversion for an absolute quantity would not be a pure scaling due to unit offsets, the implicit unit conversion is only allowed if the modified component (or its type) has an \lstinline!absoluteValue! annotation.
