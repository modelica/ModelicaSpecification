...
Annotations are optional in the Modelica grammar, and when present, indicated using the \lstinline!annotation!\index{annotation@\robustinline{annotation}} keyword, see \lstinline[language=grammar]!annotation-clause! in the grammar (\cref{expressions1}).
...
\end{lstlisting}
Layer descriptions (start of syntactic description):\annotationindex{Icon}\annotationindex{Diagram}
\begin{lstlisting}[language=grammar]
icon-layer :
...
The following common definitions are used to define graphical annotations in the later sections.\annotationindex{DrawingUnit}\annotationindex{Point}\annotationindex{Extent}
\begin{lstlisting}[language=modelica]
...
All graphical entities have a visible attribute which indicates if the entity should be shown.\annotationindex{GraphicItem}
\begin{lstlisting}[language=modelica]
...
Filled shapes have the following attributes for the border and interior.\annotationindex{FilledShape}
\begin{lstlisting}[language=modelica]
...
Placements are defined in term of coordinate systems transformations:\annotationindex{Transformation}
\begin{lstlisting}[language=modelica]
...
A line is specified as follows:\annotationindex{Line}
\begin{lstlisting}[language=modelica]
record Line
  extends GraphicItem;
  Point points[:];
...
A polygon is specified as follows:\annotationindex{Polygon}
\begin{lstlisting}[language=modelica]
record Polygon
  extends GraphicItem;
  extends FilledShape;
...
A rectangle is specified as follows:\annotationindex{Rectangle}
\begin{lstlisting}[language=modelica]
record Rectangle
  extends GraphicItem;
  extends FilledShape;
...
An ellipse is specified as follows:\annotationindex{Ellipse}
\begin{lstlisting}[language=modelica]
record Ellipse
  extends GraphicItem;
  extends FilledShape;
...
A text string is specified as follows:\annotationindex{Text}
\begin{lstlisting}[language=modelica]
record Text
  extends GraphicItem;
  extends FilledShape;
...
A bitmap image is specified as follows:\annotationindex{Bitmap}
\begin{lstlisting}[language=modelica]
record Bitmap
  extends GraphicItem;
  Extent extent;
...
In a similar way, a variable can be changed when the mouse button is \emph{released}:\annotationindex{OnMouseUpSetBoolean}
\begin{lstlisting}[language=modelica]
...
A simple type or component of a simple type may have:\annotationindex{absoluteValue}
\begin{lstlisting}[language=modelica]
...
A model or block definition may contain:\annotationindex{defaultConnectionStructurallyInconsistent}
\begin{lstlisting}[language=modelica]
...
A class may have the following annotation:\annotationindex{obsolete}
\begin{lstlisting}[language=modelica]
...
A component declaration may have the following annotation:\annotationindex{unassignedMessage}
\begin{lstlisting}[language=modelica]
...
It is a Modelica package without classes that has a \lstinline!Protection! annotation of the following form which specifies a sequence of target records, which makes it natural to define start/end dates for different sets of targets individually:\index{Authorization@\robustinline{Authorization}!license file}\index{License@\robustinline{License}!license file}
\begin{lstlisting}[language=modelica]
