...
record Documentation
  ...
  String styleSheet = "" "Only at top, and alternatively String[:]";
end Documentation;
\end{lstlisting}
...
A top-level package may inside the \lstinline!Documentation! have an annotation \lstinline!styleSheet="modelica:/Package/Resources/style.css"!, which provides a cascading style sheet.
It may also be a vector of strings that corresponding to different style sheets that will be cascaded in the given order.
The given style sheet(s) is used when displaying the \lstinline!info! and \lstinline!revisions! annotations found in the \lstinline!Documentation! annotations of the package.
\begin{nonnormative}
...
\end{nonnormative}
The provided style sheet should not use type or universal selectors, due to possible interference with tool-specific styling.

Vendors shall use a \lstinline[language=CSS]!$NameOfVendor$-! prefix to style vendor generated HTML content surrounding the user provided documentation.
