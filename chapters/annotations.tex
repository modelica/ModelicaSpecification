...
A top-level package may contain an annotation \lstinline!styleSheet="modelica://Package/Resources/style.css"!, which provides a cascading stylesheet that is applied for the documentation.

The following are suggested classes for the generated documentation, to match the style sheet.
Note that all of them may not be used in a specific tool (or in all cases).
\begin{itemize}
\item
  ModelicaDescription  Description strings
\item
  ModelicaBaseClass  Base class in list of base classes.
\item
  ModelicaKeyword Modelica keywords.
\item
 ModelicaComment For all variants of comments in Modelica.
\item
 ModelicaClass For names of used classes.
\item 
 ModelicaFunction For names of called functions.
\item
 ModelicaTablePackageContent For the table of contents of a package.
\item
 ModelicaTableCrossReference For a table of where classes are used.
\item
 ModelicaTableInputs For a table of inputs to functions.
\item
 ModelicaTableOutputs For a table of outputs from functions.
\item
 ModelicaTableParameters For a table of parameters for a model (or block).
\item
 ModelicaTableConnectors For a table of connector for a model (or block).
\item
 ModelicaTableContents For a table of contents for a connectors.
\item
 ModelicaStartAttribute For start-attribute in table of parameters to separate from parameter values.
\item
 ModelicaParameterTab The table-heading in the table of parameters.
\item
 ModelicaParameterGroup The group-heading in the table of parameters.
\end{itemize}
