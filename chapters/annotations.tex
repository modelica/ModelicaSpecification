...
If the \lstinline!TestCase! annotation is missing it is a normal model -- there are thus no restriction on the use of the model, and the model shall not contain errors.
...
It makes it an error if the connector is connected as an inside connector in a connect-equation and thus appears in a connection set if:
\begin{itemize}
...
\begin{lstlisting}[language=modelica]
/*literal*/ constant String preferredView = $\mathit{view}$;
\end{lstlisting}
