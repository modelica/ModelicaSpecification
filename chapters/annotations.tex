...
\end{lstlisting}
\emph{This introduces a new graphical primitive} \lstinline[basicstyle=\ttfamily]!Circle! \emph{using the
...
\end{lstlisting}
\emph{This introduces a new attribute} \lstinline[basicstyle=\ttfamily]!__NameOfVendor_shadow!
\emph{for the} \lstinline[basicstyle=\ttfamily]!Rectangle! \emph{primitive using the simple variant of
vendor-specific annotations.}{]}
...
\end{lstlisting}
The ``\lstinline[basicstyle=\ttfamily]!Documentation!'' annotation can contain the ``\lstinline[basicstyle=\ttfamily]!info!'' annotation
giving a textual description, the ``\lstinline[basicstyle=\ttfamily]!revisions!'' annotation giving a list
...
How the tool interprets the information in ``\lstinline[basicstyle=\ttfamily]!Documentation!'' is
unspecified. Within a string of the ``\lstinline[basicstyle=\ttfamily]!Documentation!'' annotation, the
tags \lstinline[basicstyle=\ttfamily]!<HTML>! and \lstinline[basicstyle=\ttfamily]!</HTML>! or
\lstinline[basicstyle=\ttfamily]!<html>! and \lstinline[basicstyle=\ttfamily]!</html>! define
...
the HTML link command using scheme ``\lstinline[basicstyle=\ttfamily]!Modelica!'', e.g.,

\begin{lstlisting}[language=modelica]
<a href="Modelica://MultiBody.Tutorial">MultiBody.Tutorial</a>
\end{lstlisting}

Together with scheme ``\lstinline[basicstyle=\ttfamily]!Modelica!'' the (URI) fragment specifiers
\lstinline[basicstyle=\ttfamily]!#diagram!, \lstinline[basicstyle=\ttfamily]!#info!, \lstinline[basicstyle=\ttfamily]!#text!, \lstinline[basicstyle=\ttfamily]!#icon! may be used to reference different
layers. Example:
...
The \lstinline[basicstyle=\ttfamily]!preferredView! annotation defines the default view when selecting the
class. \lstinline[basicstyle=\ttfamily]!info! means info layer, i.e., the documentation of the class,
\lstinline[basicstyle=\ttfamily]!diagram! means diagram layer and \lstinline[basicstyle=\ttfamily]!text! means the Modelica text layer.
\begin{lstlisting}[language=grammar]
...
\hline \endhead
\lstinline[basicstyle=\ttfamily]!Evaluate!&
...
declared with the prefix \lstinline[basicstyle=\ttfamily]!parameter!.
...
{[}\lstinline[basicstyle=\ttfamily]!Evaluate! \emph{is for example used for axis of rotation parameters in
the} \lstinline[basicstyle=\ttfamily]!Modelica.Mechanics.MultiBody! \emph{library in order to improve the
efficiency of the generated code}{]}\\ \hline
\lstinline[basicstyle=\ttfamily]!HideResult! &
...
{[}\lstinline[basicstyle=\ttfamily]!HideResult! \emph{is for example used in the connectors of the}
\lstinline[basicstyle=\ttfamily]!Modelica.StateGraph! \emph{library to not show variables to the modeler
that are of no interest to him and would confuse him}{]}\\ \hline
\lstinline[basicstyle=\ttfamily]!Inline! &
...
\lstinline[basicstyle=\ttfamily]!LateInline!
&
Has only an effect within a function declaration.
...
library.}{]}\\ \hline
\lstinline[basicstyle=\ttfamily]!InlineAfterIndexReduction!\strut
&
Has only an effect within a function declaration.

If \lstinline[basicstyle=\ttfamily]!true!, the model developer proposes to inline the function after the
...
combined with annotations \lstinline[basicstyle=\ttfamily]!Inline! and \lstinline[basicstyle=\ttfamily]!LateInline!.

{[}\emph{This annotation is for example used in}
\lstinline[basicstyle=\ttfamily]!Modelica.Mechanics.Rotational.Sources.Move! \emph{to define that an input
signal is the derivative of another input signal.}{]}\\ \hline
\lstinline[basicstyle=\ttfamily]!GenerateEvents!\strut
&
Has only an effect within a function declaration
...
\lstinline[basicstyle=\ttfamily]!smoothOrder! defines the number of differentiations of the function, in
order that all of the differentiated outputs are continuous provided all
input arguments and their derivatives up to order \lstinline[basicstyle=\ttfamily]!smoothOrder! are
continuous {[}\emph{This means that the function is at least}
C\textsuperscript{smoothOrder}. \lstinline[basicstyle=\ttfamily]!smoothOrder = 1! \emph{means that the
...
The optional argument \lstinline[basicstyle=\ttfamily]!normallyConstant! of \lstinline[basicstyle=\ttfamily]!smoothOrder! defines that the
function argument \lstinline[basicstyle=\ttfamily]!NAME! is usually constant {[}\emph{A tool might check
whether the actual argument to} \lstinline[basicstyle=\ttfamily]!NAME! \emph{is a parameter expression at the
...
function: One, under the assumption that all} \lstinline[basicstyle=\ttfamily]!normallyConstant! \emph{arguments
...
\emph{This annotation is used by many functions of the} \lstinline[basicstyle=\ttfamily]!Modelica.Fluid!
\emph{library, such as}\\
\lstinline[basicstyle=\ttfamily]!Modelica.Fluid.Dissipation.PressureLoss.StraightPipe.dp_laminar_DP!\emph{,
...
\end{lstlisting}
The interpretation of "\lstinline[basicstyle=\ttfamily]!unit!" is with respect to printer output in
...
\end{lstlisting}
The \lstinline[basicstyle=\ttfamily]!origin! attribute specifies the origin of the graphical item in the
...
relative the \lstinline[basicstyle=\ttfamily]!origin! attribute, which by default is \{0, 0\}.

The \lstinline[basicstyle=\ttfamily]!rotation! attribute specifies the rotation of the graphical item
counter-clockwise around the point defined by the \lstinline[basicstyle=\ttfamily]!origin! attribute.
...
The attribute \lstinline[basicstyle=\ttfamily]!preserveAspectRatio! specifies a constraint on the shape of
components of the class. If \lstinline[basicstyle=\ttfamily]!preserveAspectRatio! is true, changing the
extent of components shall preserve the aspect ratio of the coordinate
system of the class.

The attribute \lstinline[basicstyle=\ttfamily]!initialScale! specifies the default component size as
\lstinline[basicstyle=\ttfamily]!initialScale! times the size of the coordinate system of the class. An
application may use a different default value of \lstinline[basicstyle=\ttfamily]!initialScale!.
...
defined as follows; where the array of \lstinline[basicstyle=\ttfamily]!GraphicsItem! represents an
...
\end{lstlisting}
The \lstinline[basicstyle=\ttfamily]!LinePattern! attribute \lstinline[basicstyle=\ttfamily]!Solid! indicates a normal line, \lstinline[basicstyle=\ttfamily]!None! an
invisible line, and the other attributes various forms of dashed/dotted
lines.

The \lstinline[basicstyle=\ttfamily]!FillPattern! attributes \lstinline[basicstyle=\ttfamily]!Horizontal!, \lstinline[basicstyle=\ttfamily]!Vertical!, 
\lstinline[basicstyle=\ttfamily]!Cross!, \lstinline[basicstyle=\ttfamily]!Forward!,
\lstinline[basicstyle=\ttfamily]!Backward! and \lstinline[basicstyle=\ttfamily]!CrossDiag! specify fill patterns drawn with the line color
over the fill color.

The attributes \lstinline[basicstyle=\ttfamily]!HorizontalCylinder!, \lstinline[basicstyle=\ttfamily]!VerticalCylinder! and \lstinline[basicstyle=\ttfamily]!Sphere! specify
...
The border pattern attributes \lstinline[basicstyle=\ttfamily]!Raised!, \lstinline[basicstyle=\ttfamily]!Sunken! and \lstinline[basicstyle=\ttfamily]!Engraved! represent frames
which are rendered in a tool-dependent way -- inside the extent of the
filled shape.

The \lstinline[basicstyle=\ttfamily]!smooth! attribute specifies that a line can be drawn as straight line
segments (\lstinline[basicstyle=\ttfamily]!None!) or using a spline (\lstinline[basicstyle=\ttfamily]!Bezier!), where the line's points
specify control points of a quadratic Bezier curve.

For lines with only two points, the \lstinline[basicstyle=\ttfamily]!smooth! attribute has no effect.
...
The values of the \lstinline[basicstyle=\ttfamily]!EllipseClosure! enumeration specify if and how the
...
has an annotation with a \lstinline[basicstyle=\ttfamily]!Placement! modifier to describe the placement.
...
coordinate system of the enclosing class. The \lstinline[basicstyle=\ttfamily]!extent! defines the
position, size and flipping of the component, relative to the \lstinline[basicstyle=\ttfamily]!origin!
attribute. The \lstinline[basicstyle=\ttfamily]!extent! is defined relative to the \lstinline[basicstyle=\ttfamily]!origin! attribute of the
...
The \lstinline[basicstyle=\ttfamily]!rotation! attribute specifies rotation of the extent around the point
defined by the \lstinline[basicstyle=\ttfamily]!origin! attribute.
...
\end{lstlisting}
If no \lstinline[basicstyle=\ttfamily]!iconTransformation! is given the \lstinline[basicstyle=\ttfamily]!transformation! is also used for
...
of the connectors. The attribute} \lstinline[basicstyle=\ttfamily]!transformation! \emph{gives the placement in
the diagram layer and} \lstinline[basicstyle=\ttfamily]!iconTransformation! \emph{gives the placement in the icon
...
\lstinline[basicstyle=\ttfamily]!primitivesVisible! attribute is false, components and connections are
...
A connection is specified with an annotation containing a \lstinline[basicstyle=\ttfamily]!Line! primitive
...
\end{lstlisting}
Note that the \lstinline[basicstyle=\ttfamily]!Line! primitive is also used to specify the graphical
...
\end{lstlisting}
The \lstinline[basicstyle=\ttfamily]!extent! attribute specifies the bounding box of the rectangle. If the
\lstinline[basicstyle=\ttfamily]!radius! attribute is specified, the rectangle is drawn with rounded
...
\end{lstlisting}
The \lstinline[basicstyle=\ttfamily]!extent! attribute specifies the bounding box of the ellipse.

Partial ellipses can be drawn using the \lstinline[basicstyle=\ttfamily]!startAngle! and \lstinline[basicstyle=\ttfamily]!endAngle!
...
\lstinline[basicstyle=\ttfamily]!startAngle! to \lstinline[basicstyle=\ttfamily]!endAngle!, where startAngle and endAngle are defined
counter-clockwise from 3 o'clock (the positive x-axis).

The closure attribute specifies whether the endpoints specified by
\lstinline[basicstyle=\ttfamily]!startAngle! and \lstinline[basicstyle=\ttfamily]!endAngle! are to be joined by lines to the centre of the
extent (\lstinline[basicstyle=\ttfamily]!closure=EllipseClosure.Radial!), joined by a single straight line
between the end points (\lstinline[basicstyle=\ttfamily]!closure=EllipseClosure.Chord!), or left
unconnected (\lstinline[basicstyle=\ttfamily]!closure=EllipseClosure.None!). In the latter case, the
ellipse is treated as an open curve instead of a closed shape, and the
\lstinline[basicstyle=\ttfamily]!fillPattern! and \lstinline[basicstyle=\ttfamily]!fillColor! are not applied (if present, they are
ignored).

The default closure is \lstinline[basicstyle=\ttfamily]!EllipseClosure.Chord! when \lstinline[basicstyle=\ttfamily]!startAngle! is 0 and
\lstinline[basicstyle=\ttfamily]!endAngle! is 360, or \lstinline[basicstyle=\ttfamily]!EllipseClosure.Radial! otherwise. 
\emph{{[}The
default for a closed ellipse is not} \lstinline[basicstyle=\ttfamily]!EllipseClosure.None!\emph{, since that
would result in} \lstinline[basicstyle=\ttfamily]!fillColor! and \lstinline[basicstyle=\ttfamily]!fillPattern! \emph{being ignored, making it
impossible to draw a filled ellipse.} \lstinline[basicstyle=\ttfamily]!EllipseClosure.Chord! \emph{is equivalent
...
\end{lstlisting}
The \lstinline[basicstyle=\ttfamily]!textColor! attribute defines the color of the text. The text is drawn
with transparent background and no border around the text (and without
outline). The contents inherited from \lstinline[basicstyle=\ttfamily]!FilledShape! is deprecated.
...
%TODO-BACK How to format in emph?
\begin{itemize}
\item
...
The style attribute \lstinline[basicstyle=\ttfamily]!fontSize! specifies the font size. If the \lstinline[basicstyle=\ttfamily]!fontSize!
...
height is used as height for the text (unless \lstinline[basicstyle=\ttfamily]!fontSize! attribute is
...
If the string \lstinline[basicstyle=\ttfamily]!fontName! is empty, the tool may choose a font. The font
names \lstinline[basicstyle=\ttfamily]!"serif"!, \lstinline[basicstyle=\ttfamily]!"sans-serif"!, and \lstinline[basicstyle=\ttfamily]!"monospace"! shall be recognized. If
possible the correct font should be used - otherwise a reasonable match,
or treat as if font-name was empty.

The style attribute \lstinline[basicstyle=\ttfamily]!textStyle! specifies variations of the font.
...
\end{lstlisting}
The \lstinline[basicstyle=\ttfamily]!Bitmap! primitive renders a graphical bitmap image. The data of the
...
When the attribute \lstinline[basicstyle=\ttfamily]!fileName! is specified, the string refers to an
...
formats include \lstinline[basicstyle=\ttfamily]!PNG!, \lstinline[basicstyle=\ttfamily]!BMP! and \lstinline[basicstyle=\ttfamily]!JPEG!, other supported file formats are
unspecified.

When the attribute \lstinline[basicstyle=\ttfamily]!imageSource! is specified, the string contains the
...
be dependent on class variables using the \lstinline[basicstyle=\ttfamily]!DynamicSelect! expression.
\lstinline[basicstyle=\ttfamily]!DynamicSelect! has the syntax of a function call with two arguments,
...
may be associated with a \lstinline[basicstyle=\ttfamily]!GraphicItem! or a component as an array named
\lstinline[basicstyle=\ttfamily]!interaction!. The \lstinline[basicstyle=\ttfamily]!interaction! array may occur as an attribute of a
...
interaction annotation contains \lstinline[basicstyle=\ttfamily]!OnMouseDownSetBoolean!:
...
The \lstinline[basicstyle=\ttfamily]!OnMouseMoveXSetReal! interaction object sets the variable to the
position of the cursor in X direction in the local coordinate system
mapped to the interval defined by the \lstinline[basicstyle=\ttfamily]!minValue! and \lstinline[basicstyle=\ttfamily]!maxValue! attributes.
...
\end{lstlisting}
The \lstinline[basicstyle=\ttfamily]!OnMouseMoveYSetReal! interaction object works in a corresponding way
as the \lstinline[basicstyle=\ttfamily]!OnMouseMoveXSetReal! object but in the Y direction.
...
The \lstinline[basicstyle=\ttfamily]!OnMouseDownEditInteger! interaction object presents an input field
...
small or too large value according to the \lstinline[basicstyle=\ttfamily]!min! and \lstinline[basicstyle=\ttfamily]!max! parameter values
...
\end{lstlisting}
The \lstinline[basicstyle=\ttfamily]!OnMouseDownEditReal! interaction object presents an input field when
...
\end{lstlisting}
The \lstinline[basicstyle=\ttfamily]!OnMouseDownEditString! interaction object presents an input field
when the graphical item or component is clicked on. The field shows the
...
When an \lstinline[basicstyle=\ttfamily]!outer! component of the class does not have a corresponding \lstinline[basicstyle=\ttfamily]!inner!
...
\end{lstlisting}
When an instance of model \lstinline[basicstyle=\ttfamily]!World! is dragged in to the diagram layer, the
...
If \lstinline[basicstyle=\ttfamily]!false!, then the variable defines a relative quantity, and if true an
...
If \lstinline[basicstyle=\ttfamily]!true!, it is stated that a default connection will result in a
...
respective \lstinline[basicstyle=\ttfamily]!model! or \lstinline[basicstyle=\ttfamily]!block! and for every input \lstinline[basicstyle=\ttfamily]!u! providing an equation
\lstinline[basicstyle=\ttfamily]!0=f(u)!, and for every (potential,flow) pair of the form \lstinline[basicstyle=\ttfamily]!(v,i)!, providing
an equation of the form \lstinline[basicstyle=\ttfamily]!0=f(v,i)!.
...
The annotations \lstinline[basicstyle=\ttfamily]!tab! and \lstinline[basicstyle=\ttfamily]!group! define the placement of
the component or of variables in a dialog with optional tab and group
specification. If \lstinline[basicstyle=\ttfamily]!enable = false!, the input field may
be disabled {[}\emph{and no input can be given}{]}. If
\lstinline[basicstyle=\ttfamily]!showStartAttribute = true! the dialog should allow the user to
...
If \lstinline[basicstyle=\ttfamily]!colorSelector=true!, it indicates that an rgb-value selector can be
...
The annotation \lstinline[basicstyle=\ttfamily]!groupImage! references an image using an URI (see
...
\lstinline[basicstyle=\ttfamily]!false! or \lstinline[basicstyle=\ttfamily]!true! value {[}\emph{since if the value is an
expression, the connectorSizing functionality is conditional and this
will then lead easily to wrong models}{]}. If \lstinline[basicstyle=\ttfamily]!connectorSizing = false!, this annotation has no effect. 
If \lstinline[basicstyle=\ttfamily]!connectorSizing = true!, the corresponding variable must be declared with the
\lstinline[basicstyle=\ttfamily]!parameter! prefix, must be a subtype of a scalar Integer and
...
If \lstinline[basicstyle=\ttfamily]!connectorSizing = true!, a tool may set the parameter value
...
Annotation ``\lstinline[basicstyle=\ttfamily]!Dialog!'' is defined as:
...
A \lstinline[basicstyle=\ttfamily]!Selector! displays a file dialog to select a file. Setting \lstinline[basicstyle=\ttfamily]!filter! only
...
``text2'', respectively. Parameter \lstinline[basicstyle=\ttfamily]!caption! is the text displayed in the
dialog menu. Parameter \lstinline[basicstyle=\ttfamily]!loadSelector! is used to select an existing file
for reading, whereas parameter \lstinline[basicstyle=\ttfamily]!saveSelector! is used to define a file for
writing.
...
\begin{itemize}
\item
  Main release versions: """ \lstinline[basicstyle=\ttfamily]!UNSIGNED-INTEGER! \{ "." \lstinline[basicstyle=\ttfamily]!UNSIGNED-INTEGER! \}
  """ {[}\emph{Example: "2.1"}{]}
\item
  Pre-release versions: """ \lstinline[basicstyle=\ttfamily]!UNSIGNED-INTEGER! \{ "." \lstinline[basicstyle=\ttfamily]!UNSIGNED-INTEGER! \}
  " " \{\lstinline[basicstyle=\ttfamily]!S-CHAR!\} """ {[}\emph{Example: "2.1 Beta 1"}{]}
\item
  Un-ordered versions: """ \lstinline[basicstyle=\ttfamily]!NON-DIGIT! \{\lstinline[basicstyle=\ttfamily]!S-CHAR!\} """\\
  {[}\emph{Example: "Test 1"}{]}
\end{itemize}
...
\begin{itemize}
\item
  \lstinline[basicstyle=\ttfamily]!version = CURRENT-VERSION-NUMBER!\\
  Defines the version number of the model or package. All classes within
  this top-level class have this version number.
\item
  \lstinline[basicstyle=\ttfamily]!conversion(noneFromVersion = VERSION-NUMBER)!\\
  Defines that user models using the \lstinline[basicstyle=\ttfamily]!VERSION-NUMBER! can be upgraded to
  the \lstinline[basicstyle=\ttfamily]!CURRENT-VERSION-NUMBER! of the current class without any changes.
\item
  \lstinline[basicstyle=\ttfamily]!conversion(from(version = Versions, [to=VERSION-NUMBER,] Convert))!\\
  where \emph{Versions} is \lstinline[basicstyle=\ttfamily]!VERSION-NUMBER! \textbar{}
   \lstinline[basicstyle=\ttfamily]!{VERSION-NUMBER,VERSION-NUMBER, ...}!\
  and \lstinline[basicstyle=\ttfamily]!Convert is script="..."! \textbar{}
   \lstinline[basicstyle=\ttfamily]!change={conversionRule(), ..., conversionRule()}!\\*[.5ex]
  Defines that user models using the \lstinline[basicstyle=\ttfamily]!VERSION-NUMBER! or any of the given
  \lstinline[basicstyle=\ttfamily]!VERSION-NUMBER! can be upgraded to the given \lstinline[basicstyle=\ttfamily]!VERSION-NUMBER! (if the
  to-tag is missing this is the \lstinline[basicstyle=\ttfamily]!CURRENT-VERSION-NUMBER!) of the current
  class by applying the given conversion rules. The script consists of
  an unordered sequence of  \lstinline[basicstyle=\ttfamily]!conversionRule();! (and optionally Modelica
  comments). The  \lstinline[basicstyle=\ttfamily]!conversionRule! functions are defined in \autoref{conversion-rules}.
  \emph{{[}The to-tag is added for clarity and optionally allows a tool
  to convert in multiple steps.{]}}
\item
  \lstinline[basicstyle=\ttfamily]!uses(IDENT (version = VERSION-NUMBER) )!\\
  Defines that classes within this top-level class uses version
  \lstinline[basicstyle=\ttfamily]!VERSION-NUMBER! of classes within the top-level class \lstinline[basicstyle=\ttfamily]!IDENT!.
\end{itemize}

The annotations \lstinline[basicstyle=\ttfamily]!uses! and \lstinline[basicstyle=\ttfamily]!conversion! may contain several different
sub-entries.
...
\end{lstlisting}
\emph{In this example the model} \lstinline[basicstyle=\ttfamily]!A! \emph{uses an older version of the
Modelica library and can be upgraded using the given script, and model}
\lstinline[basicstyle=\ttfamily]!B! \emph{uses an older version of the Modelica library but no changes are
...
Convert class \lstinline[basicstyle=\ttfamily]!OldClass! to \lstinline[basicstyle=\ttfamily]!NewClass!.
...
convertMessage for the same \lstinline[basicstyle=\ttfamily]!OldClass!.
...
Convert class \lstinline[basicstyle=\ttfamily]!OldClass! to \lstinline[basicstyle=\ttfamily]!NewClass! if the literal modifier for
\lstinline[basicstyle=\ttfamily]!oldElement! has the value \lstinline[basicstyle=\ttfamily]!whenValue!, and also remove the modifier for
\lstinline[basicstyle=\ttfamily]!oldElement!.

These are considered before \lstinline[basicstyle=\ttfamily]!convertClass! and \lstinline[basicstyle=\ttfamily]!convertMessage! for the same
\lstinline[basicstyle=\ttfamily]!OldClass!.

\paragraph*{convertElement("OldClass","OldName","NewName")}\doublelabel{convertelementoldclassoldnamenewname}

In \lstinline[basicstyle=\ttfamily]!OldClass! convert element \lstinline[basicstyle=\ttfamily]!OldName! to \lstinline[basicstyle=\ttfamily]!NewName!. 
Both \lstinline[basicstyle=\ttfamily]!OldName! and \lstinline[basicstyle=\ttfamily]!NewName!
...
Normal case; if any modifier among \lstinline[basicstyle=\ttfamily]!OldModifier! exist then replace all of
them with the \lstinline[basicstyle=\ttfamily]!NewModifiers!. The defaults (if present) are used if there
are multiple \lstinline[basicstyle=\ttfamily]!OldModifier! and not all are set in the component instance.

%TODO-BACK How to change emph + code: simplify?
If \lstinline[basicstyle=\ttfamily]!simplify! is specified and true then perform obvious simplifications
to clean up the new modifier; otherwise leave as is. \emph{Note:}
\lstinline[basicstyle=\ttfamily]!simplify! \emph{is primarily intended for converting enumerations and emulated
...
For any use of \lstinline[basicstyle=\ttfamily]!OldClass! (or element of \lstinline[basicstyle=\ttfamily]!OldClass!) report that conversion
...
A top-level class, \lstinline[basicstyle=\ttfamily]!IDENT!, with version \lstinline[basicstyle=\ttfamily]!VERSION-NUMBER! can be stored in
one of the following ways in a directory given in the \lstinline[basicstyle=\ttfamily]!MODELICAPATH!:

\begin{itemize}
\item
  The file \lstinline[basicstyle=\ttfamily]!IDENT ".mo"! {[}\emph{Example:} \lstinline[basicstyle=\ttfamily]!Modelica.mo!{]}
\item
  The file \lstinline[basicstyle=\ttfamily]!IDENT " " VERSION-NUMBER ".mo"! {[}\emph{Example:} \lstinline[basicstyle=\ttfamily]!Modelica 2.1.mo!{]}
\item
  The directory \lstinline[basicstyle=\ttfamily]!IDENT! {[}\emph{Example:} \lstinline[basicstyle=\ttfamily]!Modelica!{]} with the file
  package.mo directly inside it
\item
  The directory \lstinline[basicstyle=\ttfamily]!IDENT " " VERSION-NUMBER! {[}\emph{Example:} 
	\lstinline[basicstyle=\ttfamily]!Modelica 2.1!{]} with the file package.mo directly inside it
\end{itemize}
...
\begin{itemize}
\item
  ``\lstinline[basicstyle=\ttfamily]!version!'' is the version number of the released library,
  see \autoref{version-handling}.
\item
  ``\lstinline[basicstyle=\ttfamily]!versionDate!'' is the date in UTC format (according to ISO
  8601) when the library was released. This string is updated by the
  library author to correspond with the version number.
\item
  ``\lstinline[basicstyle=\ttfamily]!versionBuild!'' is the optional build number of the library.
...
\item
  ``\lstinline[basicstyle=\ttfamily]!dateModified!'' is the UTC date and time (according to ISO
...
\item
  ``\lstinline[basicstyle=\ttfamily]!revisionId!'' is a tool specific revision identifier
...
key. An encrypted package that has the \lstinline[basicstyle=\ttfamily]!Protection! annotation is
...
\lstinline[basicstyle=\ttfamily]!Access.documentation!):
...
\begin{enumerate}
\item
  \lstinline[basicstyle=\ttfamily]!Access.hide!\\
  Do not show the class anywhere (it is not possible to inspect any part
  of the class).
\item
  \lstinline[basicstyle=\ttfamily]!Access.icon!\textbf{\\
...
\item
  \lstinline[basicstyle=\ttfamily]!Access.documentation!\\
  Same as \lstinline[basicstyle=\ttfamily]!Access.icon! and additionally the documentation annotation (as
...
\item
  \lstinline[basicstyle=\ttfamily]!Access.diagram!\\
  Same as \lstinline[basicstyle=\ttfamily]!Access.documentation! and additionally, the diagram annotation,
  and all components and connect equations that have a graphical
  annotation can be accessed.
\item
  \lstinline[basicstyle=\ttfamily]!Access.nonPackageText!\\
  Same as \lstinline[basicstyle=\ttfamily]!Access.diagram! and additionally if it is not a package: the
  whole class definition can be accessed (but cannot be copied).
\item
  \lstinline[basicstyle=\ttfamily]!Access.nonPackageDuplicate!\\
  Same as \lstinline[basicstyle=\ttfamily]!Access.nonPackageText! and additionally if it is not a package:
  the class, or part of the class, can be copied and pasted.
\item
  \lstinline[basicstyle=\ttfamily]!Access.packageText!\\
  Same as \lstinline[basicstyle=\ttfamily]!Access.diagram! (note: \textbf{not} including all rights of
  \lstinline[basicstyle=\ttfamily]!Access.nonPackageDuplicate!) and additionally the whole class
  definition can be accessed (but cannot be copied).
\item
  \lstinline[basicstyle=\ttfamily]!Access.packageDuplicate!\\
  Same as \lstinline[basicstyle=\ttfamily]!Access.packageText! and additionally the class, or part of the
  class, can be copied and pasted.
\end{enumerate}

The ``\lstinline[basicstyle=\ttfamily]!access!'' annotation holds for the respective class and all classes
that are hierarchically on a lower level, unless overriden by a
Protection annotation with ``\lstinline[basicstyle=\ttfamily]!access!'' {[}\emph{e.g. if the annotation is
...
Overriding ``\lstinline[basicstyle=\ttfamily]!access=Access.hide!'' and ``\lstinline[basicstyle=\ttfamily]!access=Access.packageDuplicate!''
...
this, but reuse the} \lstinline[basicstyle=\ttfamily]!Access.XXX! \emph{definition, e.g., for} \lstinline[basicstyle=\ttfamily]!Access.icon!
\emph{only the variables can be stored in a result file that can also be
inspected in the class, and for} \lstinline[basicstyle=\ttfamily]!Access.nonPackageText! \emph{all public
...
In this section annotations within the ``\lstinline[basicstyle=\ttfamily]!Protection!'' annotation are
...
\end{lstlisting}
The \lstinline[basicstyle=\ttfamily]!License! annotation has only an effect on the top of an encrypted
class and is then valid for the whole class hierarchy. {[}\emph{Usually
the licensed class is a package}{]}. The \lstinline[basicstyle=\ttfamily]!libraryKey! is a secret string
from the library vendor and is the protection mechanism so that a user
cannot generate his/her own authorization file since the \lstinline[basicstyle=\ttfamily]!libraryKey! is
unknown to him/her.

The \lstinline[basicstyle=\ttfamily]!features! annotation defines the required license options. If the
...
other annotations, the ``\lstinline[basicstyle=\ttfamily]!features!'' annotation holds for the respective
...
according to the ``\lstinline[basicstyle=\ttfamily]!features!'' annotation is provided in the
...
license manager, or a license file (called ``\lstinline[basicstyle=\ttfamily]!licenseFile!'') must be
present. The license file is standardized. It is a Modelica package
without classes that has a \lstinline[basicstyle=\ttfamily]!Protection! annotation of the following form
...
\end{lstlisting}
The format of the strings used for \lstinline[basicstyle=\ttfamily]!libraryKey! and \lstinline[basicstyle=\ttfamily]!id! are not specified
(they are vendor specific). The \lstinline[basicstyle=\ttfamily]!libraryKey! is a secret of the library
developer. The \lstinline[basicstyle=\ttfamily]!operations! define the usage conditions and the following
are default names:
...
``\lstinline[basicstyle=\ttfamily]!libraryKey!'' and the target definitions, the authorization file must
be encrypted and must never show the \lstinline[basicstyle=\ttfamily]!libraryKey!. {[}\emph{All other
