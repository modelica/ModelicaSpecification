...
The annotation \lstinline!Evaluate! is only allowed for structural parameters and constants (\cref{component-variability-prefixes-discrete-parameter-constant}).

If \lstinline!Evaluate = true!, the model developer proposes to utilize the value of the declaration equation for the symbolic processing.
In that case, it is not possible to change a structural parameter value after translation.
A constant can never be changed after translation, and it is normal for its value to be used for the symbolic processing even without \lstinline!Evaluate = true!.

If \lstinline!Evaluate = false!, the model developer proposes to not utilize the value of the declaration equation for the symbolic processing.
...
The items of the \lstinline!Access! enumeration have the following meanings:
\begin{enumerate}
\item
