...
A top-level package may contain an annotation \lstinline!styleSheet="modelica:/Package/Resources/style.css"!, which provides a cascading stylesheet.
The given style sheet is used when displaying the \lstinline!info! and \lstinline!revisions! annotations found in the \lstinline!Documentation! annotation.
The provided style sheet shall not use type or universal selectors.
\begin{nonnormative}
It is recommended to use class and id selectors with a \lstinline!$NameOfLibrary$-! prefix to avoid collisions when the content is included in a larger context.
\end{nonnormative}

Vendors shall use a \lstinline!$NameOfVendor$-! prefix to style vendor generated HTML content surrounding the user provided documentation.
