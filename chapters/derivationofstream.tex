...
For this case, the return value of the \lstinline[basicstyle=\ttfamily]!inStream()! operator is arbitrary.
...
In this case, \lstinline[basicstyle=\ttfamily]!inStream()! is continuous (contrary to $h_{mix}$) and does not
...
equation system is removed by this transformation. However, \lstinline[basicstyle=\ttfamily]!inStream!(..)
...
with the ``\lstinline[basicstyle=\ttfamily]!min!'' attribute (the ``\lstinline[basicstyle=\ttfamily]!max!'' attribute could be also defined,
...
min attribute set accordingly). Connecting \lstinline[basicstyle=\ttfamily]!m1.c! with \lstinline[basicstyle=\ttfamily]!m2.c! and \lstinline[basicstyle=\ttfamily]!m3.c!, such
that
...
The \lstinline[basicstyle=\ttfamily]!inStream!() operator cannot be evaluated for a connector, on which
...
providing an explicit formula for the \lstinline[basicstyle=\ttfamily]!inStream! operator. Due to the
...
``\lstinline[basicstyle=\ttfamily]!min!''-attributes of the corresponding flow variables).
