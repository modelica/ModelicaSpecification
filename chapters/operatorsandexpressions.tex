...
An expression\index{if@\indexinline{if}!expression}\index{then@\indexinline{then}!if-expression}\index{else@\indexinline{else}!if-expression}
\begin{lstlisting}[language=modelica]
...
If-expressions with \lstinline!elseif!\index{elseif@\indexinline{elseif}!if-expression} are defined by replacing \lstinline!elseif! by \lstinline!else if!.
...
\begin{operatordefinition*}[Integer]\label{modelica:integer-of-enumeration}\index{Integer@\indexinline{Integer}!conversion operator}
\begin{synopsis}\begin{lstlisting}
Integer($e$)
\end{lstlisting}\end{synopsis}
\begin{semantics}
...
\begin{operatordefinition*}[<EnumTypeName>]\label{modelica:enumeration-of-integer}\index{enumeration@\indexinline{enumeration}!conversion operator}
\begin{synopsis}\begin{lstlisting}
EnumTypeName($i$)
\end{lstlisting}\end{synopsis}
\begin{semantics}
...
\begin{operatordefinition*}[String]\label{modelica:to-String}\index{String@\indexinline{String}!conversion operator}
\begin{synopsis}\begin{lstlisting}
String($b$, <options>)
String($i$, <options>)
...
\hline
\hline
\lstinline!sin($x$)!        \index{sin@\indexinline{sin}}     & Sine & \\
\lstinline!cos($x$)!        \index{cos@\indexinline{cos}}     & Cosine & \\
\lstinline!tan($x$)!        \index{tan@\indexinline{tan}}     & Tangent ($x$ shall not be: $\ldots$, -$\pi/2$, $\pi/2$, $3\pi/2$, $\ldots$) & \\
\lstinline!asin($x$)!       \index{asin@\indexinline{asin}}   & Inverse sine ($-1 \le x \le 1$) & \\
\lstinline!acos($x$)!       \index{acos@\indexinline{acos}}   & Inverse cosine ($-1 \le x \le 1$) & \\
\lstinline!atan($x$)!       \index{atan@\indexinline{atan}}   & Inverse tangent & \\
\lstinline!atan2($y$, $x$)! \index{atan2@\indexinline{atan2}} & Principal value of the arc tangent of $y/x$ & \Cref{modelica:atan2} \\
\lstinline!sinh($x$)!       \index{sinh@\indexinline{sinh}}   & Hyperbolic sine & \\
\lstinline!cosh($x$)!       \index{cosh@\indexinline{cosh}}   & Hyperbolic cosine & \\
\lstinline!tanh($x$)!       \index{tanh@\indexinline{tanh}}   & Hyperbolic tangent & \\
\lstinline!exp($x$)!        \index{exp@\indexinline{exp}}     & Exponential, base $\mathrm{e}$ & \\
\lstinline!log($x$)!        \index{log@\indexinline{log}}     & Natural (base $\mathrm{e}$) logarithm ($x > 0$) & \\
\lstinline!log10($x$)!      \index{log10@\indexinline{log10}} & Base 10 logarithm ($x > 0$) & \\
\hline
\end{tabular}
\end{center}
...
\begin{operatordefinition*}[sample]\label{modelica:event-sample}\index{sample@\indexinline{sample}!event-generating}
\begin{synopsis}\begin{lstlisting}
