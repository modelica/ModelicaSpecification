...
\hline
\hline
\lstinline!not!\index{not@\robustinline{not}} & Logical negation (unary operator)\\
\lstinline!and!\index{and@\robustinline{and}} & Logical \emph{and} (conjunction)\\
\lstinline!or!\index{or@\robustinline{or}} & Logical \emph{or} (disjunction)\\
\hline
\end{tabular}
\end{center}
...
An expression\index{if@\robustinline{if}!expression}\index{then@\robustinline{then}!if-expression}\index{else@\robustinline{else}!if-expression}
\begin{lstlisting}[language=modelica]
...
\hline
\hline
\lstinline!sin($x$)!        \index{sin@\robustinline{sin}}     & Sine & \\
\lstinline!cos($x$)!        \index{cos@\robustinline{cos}}     & Cosine & \\
\lstinline!tan($x$)!        \index{tan@\robustinline{tan}}     & Tangent ($x$ shall not be: $\ldots$, -$\pi/2$, $\pi/2$, $3\pi/2$, $\ldots$) & \\
\lstinline!asin($x$)!       \index{asin@\robustinline{asin}}   & Inverse sine ($-1 \le x \le 1$) & \\
\lstinline!acos($x$)!       \index{acos@\robustinline{acos}}   & Inverse cosine ($-1 \le x \le 1$) & \\
\lstinline!atan($x$)!       \index{atan@\robustinline{atan}}   & Inverse tangent & \\
\lstinline!atan2($y$, $x$)! \index{atan2@\robustinline{atan2}} & Principal value of the arc tangent of $y/x$ & \Cref{modelica:atan2} \\
\lstinline!sinh($x$)!       \index{sinh@\robustinline{sinh}}   & Hyperbolic sine & \\
\lstinline!cosh($x$)!       \index{cosh@\robustinline{cosh}}   & Hyperbolic cosine & \\
\lstinline!tanh($x$)!       \index{tanh@\robustinline{tanh}}   & Hyperbolic tangent & \\
\lstinline!exp($x$)!        \index{exp@\robustinline{exp}}     & Exponential, base $\mathrm{e}$ & \\
\lstinline!log($x$)!        \index{log@\robustinline{log}}     & Natural (base $\mathrm{e}$) logarithm ($x > 0$) & \\
\lstinline!log10($x$)!      \index{log10@\robustinline{log10}} & Base 10 logarithm ($x > 0$) & \\
\hline
\end{tabular}
\end{center}
