...
%TODO-BACK Add remaining lstlistings in column 2 and 3
\begin{table}[H]
\caption{Operators}
...
%TODO-BACK Check again if changes are appropriate
...
%TODO-BACK Did changes work?
\begin{longtable}{|p{5.1cm}|p{8cm}|} 
\hline \endhead
\lstinline[basicstyle=\ttfamily]!der!(expr) & The time derivative of \lstinline[basicstyle=\ttfamily]!expr!. If the expression \lstinline[basicstyle=\ttfamily]!expr! is a
...
\end{tabular} & 
Returns: \lstinline[basicstyle=\ttfamily]!expr(time--delayTime)! for~ time\textgreater{}time.start +
delayTime and \lstinline[basicstyle=\ttfamily]!expr(time.start)! for time \textless{}= time.start +
delayTime. The arguments, i.e., \lstinline[basicstyle=\ttfamily]!expr!, \lstinline[basicstyle=\ttfamily]!delayTime! and \lstinline[basicstyle=\ttfamily]!delayMax!, need to be
...
\textless{}= delayMax, otherwise an error occurs. If \lstinline[basicstyle=\ttfamily]!delayMax! is not
...
\item
  ...
  \item
    ndims(A)
  \item
    cardinality(c), see restrictions for use in \autoref{cardinality-deprecated}.
  \item
