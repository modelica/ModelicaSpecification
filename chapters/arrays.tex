...
Returns the size of dimension $i$ of array expression \lstinline!A! where $0 \leq i \leq \text{\lstinline!ndims(A)!}$.  If \lstinline!A! refers to a component of an expandable connector, then the component must be a declared component of
the expandable connector, and it must not use colon (\lstinline!:!) to specify the array size of dimension $i$.\\ \hline
\lstinline!size(A)! &
Returns a vector of length \lstinline!ndims(A)! containing the dimension sizes of \lstinline!A!.  If \lstinline!A! refers to a component of an expandable connector, then the component must be a declared component of
the expandable connector, and it must not use colon (\lstinline!:!) to specify the size of any array dimension.\\ \hline
\end{longtable}
