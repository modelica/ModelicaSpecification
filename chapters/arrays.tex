...
Exponentiation \lstinline!a ^ s! is defined if \lstinline!a! is a square matrix of a subtype of \lstinline!Real!, and \lstinline!s! is a scalar subtype of \lstinline!Integer! with $\text{\lstinline!s!} \geq 0$.
The result is a \lstinline!Real! matrix.
The exponentiation is done by repeated multiplication, e.g.:
\begin{lstlisting}[language=modelica]
a^3 = a * a * a;
a^0 = identity(size(a, 1)) * 1.0;
assert(size(a, 1) == size(a, 2), "Matrix must be square");
a^1 = a;
\end{lstlisting}
Exponentation of a matrix of integers is defined by first converting it to reals according to \cref{standard-type-coercion}, in order to avoid integer overflow.
