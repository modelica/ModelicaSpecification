...
evaluated once for each array constructor, and 1are evaluated in the
...
IDENT in class NAME
\end{lstlisting}
the \lstinline!NAME! must be (using normal lexical lookup) the
name of a \lstinline!model!, \lstinline!block!, \lstinline!connector!, \lstinline!record!,
or \lstinline!operator record!, \lstinline!type! specialized class.
The loop-variable, \lstinline!IDENT!, is resolved to every instance of class \lstinline!NAME!
that is on the same or a lower level as the parent of the component containing the iterator expression.
\begin{nonnormative}
For example if the iterator expression is in a model A and the simulation contains a component
of A with component path top.x.a or top.x.a[:] only components in top.x are considered;
and if the component-path is top.x[n].a only components in top.x[1] are considered for
top.x[1].a and only top.x[2] for top.x[2].a. If the component path for the model A is a
...
The full path name of the actual instance can be inquired with the built-in function
\lstinline!IDENT.getInstanceName()!.
...
\emph{In every iteration of the for loop, the iterator variable c is a reference to an instance
of class ObservedModel present in MyModel1. Therefore, the above model is equivalent to
