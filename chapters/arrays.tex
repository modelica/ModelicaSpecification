...
dimension $n$, where $n \geq$ \lstinline!ndims(A)! is required. Let \stinline!C = promote(A, n)!, with \lstinline!nA = ndims(A)!, then \lstinline!ndims(C) = n!, \lstinline!size(C, j) = size(A, j)! for 1 \textless{}= j \textless{}= \lstinline!nA!, \lstinline!size(C, j) = 1! for nA+1 <= j <= n, C{[}i\_1, ..., i\_nA, 1, ..., 1{]} = A{[}i\_1, ..., i\_nA{]}\\ \hline
\end{longtable}
...
(\lstinline!a! = 0.0 and \lstinline!b! \textless{}= 0.0, \lstinline!a! \textless{} 0 and \lstinline!b! is
not an \lstinline!Integer!) or overflow are undefined.
