...
An \lstinline!if!-equation is seen as a list of branches: one \lstinline!if!-branch, and zero or more \lstinline!elseif!-branches, and an optional \lstinline!else!-branch.
...
\item
  ...
  For \lstinline!if!-equations this requires that the condition (if any) of the branch containing the \lstinline!when!-equation and the conditions of all preceding branches are evaluable.
\end{itemize}
...
\begin{nonnormative}
Expressions in \lstinline!when!-clause bodies do not trigger state events since they are not active during continuous integration.
\end{nonnormative}
...
This activates the \lstinline!when!-clause body, and \lstinline!x! gets reinitialized.
\end{example}
...
M3 m3;
// Using m3.my.m.x.start = m3.my.m.xStart = 3.0 with confidence number 2
// over m3.mx.x.start = m3.mx.xStart = 4.0 with confidence number 3
\end{lstlisting}
\end{example}
\end{nonnormative}
