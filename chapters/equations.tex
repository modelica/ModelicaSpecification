...
The \lstinline[basicstyle=\ttfamily]!expression! of a for-equation shall be a vector expression. It is
...
\autoref{types-as-iteration-ranges} for more information. The loop-variable (\lstinline[basicstyle=\ttfamily]!IDENT!) is in scope
...
The \lstinline[basicstyle=\ttfamily]!expression! of an if- or elseif-clause must be a scalar Boolean
...
The \lstinline[basicstyle=\ttfamily]!expression! of a when-equation shall be a discrete-time Boolean
...
taking the start-value of the when-condition, as above where \lstinline[basicstyle=\ttfamily]!b! is a
parameter variable. The start-values of the special functions \lstinline[basicstyle=\ttfamily]!initial!,
\lstinline[basicstyle=\ttfamily]!terminal!, and \lstinline[basicstyle=\ttfamily]!sample! is false.
...
Any left hand side reference, (\lstinline[basicstyle=\ttfamily]!v!, \lstinline[basicstyle=\ttfamily]!out1!, ...), in a when-clause must
...
\end{lstlisting}
2
\emph{Here, variable} \lstinline[basicstyle=\ttfamily]!y !\emph{is held constant when the when-equation is
deactivated and} \lstinline[basicstyle=\ttfamily]!x! \emph{is computed from the first equation using the
value of} \lstinline[basicstyle=\ttfamily]!y! \emph{from the previous event instant.}
...
model} \lstinline[basicstyle=\ttfamily]!DoubleWhenConflict! \emph{below, since there are two equations}
(\lstinline[basicstyle=\ttfamily]!close = true; close = false;!) \emph{defining the same variable}
\lstinline[basicstyle=\ttfamily]!close!\emph{. A conflict between the equations will occur if both
conditions would become \lstinline[basicstyle=\ttfamily]!true! at the same time instant.}
...
when-equation using} \lstinline[basicstyle=\ttfamily]!elsewhen!\emph{, as in the} \lstinline[basicstyle=\ttfamily]!WhenPriority! \emph{model
...
become} \lstinline[basicstyle=\ttfamily]!true! \emph{at the same time instant since} \lstinline[basicstyle=\ttfamily]!condition1! \emph{with
associated conditional equations has a higher priority than} \lstinline[basicstyle=\ttfamily]!condition2!.
...
The \lstinline[basicstyle=\ttfamily]!reinit! operator can only be used in the body of a when-equation. It
...
The operator reinitializes \lstinline[basicstyle=\ttfamily]!x! with \lstinline[basicstyle=\ttfamily]!expr! at an event instant. \lstinline[basicstyle=\ttfamily]!x! is a 
\lstinline[basicstyle=\ttfamily]!Real!
variable (or an array of \lstinline[basicstyle=\ttfamily]!Real! variables) that must be selected as a
state (resp., states) , i.e. reinit on x implies
stateSelect=StateSelect.always on x. \lstinline[basicstyle=\ttfamily]!expr! needs to be type-compatible
with \lstinline[basicstyle=\ttfamily]!x!. The reinit operator can for the same variable (resp. array of
...
such a way that variables with an applied} \lstinline[basicstyle=\ttfamily]!reinit! \emph{operator are
...
\emph{Example for the usage of the} \lstinline[basicstyle=\ttfamily]!reinit! \emph{operator:}
...
is an assertion, where \lstinline[basicstyle=\ttfamily]!condition! is a Boolean expression, \lstinline[basicstyle=\ttfamily]!message! is a
string expression, and \lstinline[basicstyle=\ttfamily]!level! is a built-in enumeration with a default
...
If the \lstinline[basicstyle=\ttfamily]!condition! of an assertion is true, \lstinline[basicstyle=\ttfamily]!message! is not evaluated and
the procedure call is ignored. If the \lstinline[basicstyle=\ttfamily]!condition! evaluates to false
different actions are taken depending on the level input:

%TODO-BACK emph + code
\begin{itemize}
\item
  \lstinline[basicstyle=\ttfamily]!level = AssertionLevel.error!: The current evaluation is aborted. The
  simulation may continue with another evaluation {[}\emph{e.g., with a
  shorter step-size, or by changing the values of iteration
  variables}{]}. If the simulation is aborted, \lstinline[basicstyle=\ttfamily]!message! indicates the
  cause of the error.\\
  ...
  reaching the stop-time or explicitly with \lstinline[basicstyle=\ttfamily]!terminate()!, but the
  evaluation with \lstinline[basicstyle=\ttfamily]!terminal()=true! triggers an assert, the analysis
  failed.
\item
  \lstinline[basicstyle=\ttfamily]!level = AssertionLevel.warning!: The current evaluation is not aborted.
  \lstinline[basicstyle=\ttfamily]!message! indicates the cause of the warning {[}\emph{It is recommended
...
The \lstinline[basicstyle=\ttfamily]!terminate(...)! equation or statement {[}\emph{using function
...
generation expression, e.g. \lstinline[basicstyle=\ttfamily]!x > 2! o or \lstinline[basicstyle=\ttfamily]!floor(x)!, changes
...
evaluated. The integration is halted whenever}\lstinline[basicstyle=\ttfamily]!u-uMax! \emph{or} \lstinline[basicstyle=\ttfamily]!u-uMin!
...
argument of the \lstinline[basicstyle=\ttfamily]!noEvent(..)! function. The \lstinline[basicstyle=\ttfamily]!smooth(p,x)! operator also
allows relations used as argument to be taken literally. The \lstinline[basicstyle=\ttfamily]!noEvent!
feature is propagated to all subrelations in the scope of the \lstinline[basicstyle=\ttfamily]!noEvent!
function. For \lstinline[basicstyle=\ttfamily]!smooth! the liberty to not allow literal evaluation is
...
The \lstinline[basicstyle=\ttfamily]!smooth! function is useful, if e.g. the modeler can guarantee that
...
same time instant (}\lstinline[basicstyle=\ttfamily]!condition1! \emph{has a higher priority than}
\lstinline[basicstyle=\ttfamily]!condition2!\emph{).}
...
The \lstinline[basicstyle=\ttfamily]!slowSample! when-clause is evaluated at every 5th occurrence of the
\lstinline[basicstyle=\ttfamily]!fastSample! when-clause.
...
phase, also the derivatives, \lstinline[basicstyle=\ttfamily]!der(..)!, and the pre-variables, \lstinline[basicstyle=\ttfamily]!pre(..)!,
...
explicitly enabled with the \lstinline[basicstyle=\ttfamily]!initial()! operator; and only in one of the
two forms \lstinline[basicstyle=\ttfamily]!when initial() then! or when \lstinline[basicstyle=\ttfamily]!{...,initial(),...}!
then. In this case, the when-clause equations remain active during the
whole initialization phase. {[}\emph{If a when-clause equation} \lstinline[basicstyle=\ttfamily]!v = expr!; \emph{is not active during the initialization phase, the equation}
\lstinline[basicstyle=\ttfamily]!v = pre(v)! \emph{is added for initialization. This follows from the
...
\begin{enumerate}
\item
  As equations in an \lstinline[basicstyle=\ttfamily]!initial equation! section or as assignments in an
  \lstinline[basicstyle=\ttfamily]!initial algorithm! section. The equations and assignments in these
...
\item
  For all non-discrete (that is continuous-time) Real variables \lstinline[basicstyle=\ttfamily]!vc!, the
...
\item
  Implicitly by using the attributes \lstinline[basicstyle=\ttfamily]!start=value! and \lstinline[basicstyle=\ttfamily]!fixed=true! in the
  declaration of variables:
\begin{itemize}
\item
  For all non-discrete (that is continuous-time) Real variables \lstinline[basicstyle=\ttfamily]!vc!, the
  equation \lstinline[basicstyle=\ttfamily]!vc = startExpression! is added to the initialization
  equations, if \lstinline[basicstyle=\ttfamily]!start = startExpression! and \lstinline[basicstyle=\ttfamily]!fixed = true!.
\item
  For all discrete variables \lstinline[basicstyle=\ttfamily]!vd!, the equation \lstinline[basicstyle=\ttfamily]!pre(vd) = startExpression!
  is added to the initialization equations, if \lstinline[basicstyle=\ttfamily]!start = startExpression!
  and \lstinline[basicstyle=\ttfamily]!fixed = true!.
\item
  For all variables declared as constant and parameter, with \lstinline[basicstyle=\ttfamily]!fixed = true!; no equation is added to the initialization equations.
\end{itemize}
\end{enumerate}
...
declared as constant it is an error to have \lstinline[basicstyle=\ttfamily]!fixed = false!.
...
If a parameter has a modifier for the \lstinline[basicstyle=\ttfamily]!start!-attribute, does not have
\lstinline[basicstyle=\ttfamily]!fixed=false!, and neither has a binding equation nor is part of a record
...
All variables declared as parameter having \lstinline[basicstyle=\ttfamily]!fixed = false! are treated as
...
(including} \lstinline[basicstyle=\ttfamily]!der(..)! \emph{and} \lstinline[basicstyle=\ttfamily]!pre(..)! \emph{variables) is equal. Often,
this is a nonlinear system of equations and therefore it may be
necessary to provide appropriate guess values (i.e.,} \lstinline[basicstyle=\ttfamily]!start! \emph{values
and} \lstinline[basicstyle=\ttfamily]!fixed=false!\emph{) in order to compute a solution numerically.}
...
the when-clause and where the} \lstinline[basicstyle=\ttfamily]!pre()! \emph{operator is not explicitly
...
variables that also are state variables,} \lstinline[basicstyle=\ttfamily]!x1! \emph{and} \lstinline[basicstyle=\ttfamily]!x2!\emph{, i.e.,
...
providing initial values for} \lstinline[basicstyle=\ttfamily]!x1!\emph{,} \lstinline[basicstyle=\ttfamily]!x2!\emph{,} \lstinline[basicstyle=\ttfamily]!der(x1)!\emph{,}
\lstinline[basicstyle=\ttfamily]!der(x2)!\emph{, and} \lstinline[basicstyle=\ttfamily]!y! \emph{needs to be found. Two additional initial
