...
\begin{nonnormative}
The case of no \lstinline!start!-attribute is useful when no good default value for the parameter is known, or when the modeler wants to force the user of the model to make an active choice of parameter value.
...
The case of providing a \lstinline!start!-attribute is used in libraries to give defaults so that users can quickly combine models and simulate without setting parameters; but still easily find the parameters that need to be set.
\end{nonnormative}
