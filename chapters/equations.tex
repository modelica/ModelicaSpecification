...
If there is no value for the \lstinline!start!-attribute, it is an error unless a parameter value is explicitly provided before solving the initialization problem, and the parameter must not have a \lstinline!Dialog.enable! annotation set to false.
In this case, a tool may give a diagnostic message informing the user that additional parameter values must be provided later.
If there is a value for the \lstinline!start!-attribute, the value is used to add a parameter binding equation assigning the parameter to that \lstinline!start! value.
In this case a diagnostic message is recommended unless the parameter has a \lstinline!Dialog.enable! annotation set to false.

\begin{nonnormative}
The case of no value for the \lstinline!start!-attribute is useful when no good default value for the parameter is known, or when the modeler wants to force the user of the model to make an active choice of parameter value.
...
The case of providing a value for the \lstinline!start!-attribute is used in libraries to give rudimentary defaults so that users can quickly combine models and simulate without setting parameters, but still easily find the parameters that should be set properly.
When combined with \lstinline!Dialog.enable=false!, it is also useful for parameters that are not used in the current configuration.
When used in this way, the provided value is more of a dummy value than a rudimentary default, and the recommended diagnostic message will reduce risk of not having replaced the dummy value when the parameter is used in the current configuration.
\end{nonnormative}
