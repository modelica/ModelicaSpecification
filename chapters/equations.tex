...
\subsubsection{Restrictions on When-Equations}\label{restrictions-on-when-equations}

\paragraph{Restrictions on where a when-equation may occur}
\begin{itemize}
\item
...
\paragraph{Restrictions on equations within when-equations}\label{restrictions-on-equations-within-when-equations}
...
\begin{example}
The restriction for if-equations mean that both of the following variants are illegal:
\begin{lstlisting}[language=modelica]
  Real x, y;
equation
  if time<1 then
