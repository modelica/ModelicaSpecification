...
The \lstinline!if!-equations in equation sections which do not have exclusively parameter expressions as switching conditions shall satisfy the following:
\begin{itemize}
\item
  Have the same number of equations in each branch (a missing else is counted as zero equations and the number of equations is defined as in \cref{local-equation-size}).
\item
  If the equations are non-Real they shall have component-references (or a list of them) as their left-hand-side.
  Any indices for such component-references must be evaluable.
  All branches shall have the same set of variables in the non-Real equations.
\end{itemize}
...
\begin{nonnormative}
If first condition is violated, the single assignment rule would not hold, because the number of equations may change during simulation although the number of unknowns remains the same.
The second condition ensures that it is easy to solve the non-Real equations.
\end{nonnormative}
