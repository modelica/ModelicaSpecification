...
prefix \lstinline[basicstyle=\ttfamily]!stream!. A stream variable describes a quantity that is
...
If at least one variable in a connector has the \lstinline[basicstyle=\ttfamily]!stream! prefix,
...
\begin{itemize}
\item
  The \lstinline[basicstyle=\ttfamily]!stream! prefix can only be used in a connector
  declaration.
\item
  A stream connector must have exactly one scalar variable with the
  \lstinline[basicstyle=\ttfamily]!flow! prefix. {[}\emph{The idea is that all stream variables
...
\item
  ...
  equation is generated for every variable with the \lstinline[basicstyle=\ttfamily]!stream!
...
\item
  ...
  with the \lstinline[basicstyle=\ttfamily]!stream! prefix do not lead to connection equations.
\item
  Connection equations with stream variables are generated in a model
  when using the \lstinline[basicstyle=\ttfamily]!inStream()! operator or the
  \lstinline[basicstyle=\ttfamily]!actualStream()! operator, see \autoref{stream-operator-instream-and-connection-equations}
...
have the \lstinline[basicstyle=\ttfamily]!stream! prefix. The Medium definition and the stream
...
inquired with the built-in operator \lstinline[basicstyle=\ttfamily]!inStream()!. The value of
the stream variable corresponding to the actual flow direction can be
inquired through the built-in operator \lstinline[basicstyle=\ttfamily]!actualStream()!, see
...
\lstinline[basicstyle=\ttfamily]!inStream()! operator is designed to describe in a numerically
reliable way the bi-directional transport of specific quantities carried
by a flow of matter.

\lstinline[basicstyle=\ttfamily]!inStream(v)! is only allowed on stream variables \lstinline[basicstyle=\ttfamily]!v! and is
...
%TODO-BACK How to mj?
For the following definition it is assumed that \lstinline[basicstyle=\ttfamily]!N! inside connectors
m\textsubscript{j}.c (j=1,2,...,N) and \lstinline[basicstyle=\ttfamily]!M! outside connectors
...
together and a stream variable \lstinline[basicstyle=\ttfamily]!h_outflow! is associated with a flow
variable \lstinline[basicstyle=\ttfamily]!m_flow! in connector \lstinline[basicstyle=\ttfamily]!c!.
...
%TODO-BACK How to change textsubscript? Replace by [i]?
With these prerequisites, the semantics of the expression
\lstinline[basicstyle=\ttfamily]!inStream(m[i].c.h_outflow)! is given implicitly by
...
%TODO-BACK Automatic conversion to latex did not do the right thing for "mj"? But what is the right thing, if it should be Modelica Code has been converted in a wrong way? It's also not consistent with one of the next lstlistings, which has "m_j"
\begin{lstlisting}[language=modelica]
// Standard connection equation for flow variables
0 = sum(mj.c.m_flow for j in 1:N) + sum(-ck.m_flow for k in 1:M);
\end{lstlisting}

Whenever the \lstinline[basicstyle=\ttfamily]!inStream()! operator is applied to a stream
...
%TODO-BACK How to change textsubscript?
...
If the argument of \lstinline[basicstyle=\ttfamily]!inStream()! is an array, the implicit
equation system holds elementwise, i.e., \lstinline[basicstyle=\ttfamily]!inStream()! is
vectorizable.
...
fulfilled and \lstinline[basicstyle=\ttfamily]!inStream(..)! can have any value}{]}. However,
...
required that the \lstinline[basicstyle=\ttfamily]!inStream()! operator is appropriately
...
(one-to-one connection), the result of \lstinline[basicstyle=\ttfamily]!inStream()! is trivial
...
The \lstinline[basicstyle=\ttfamily]!actualStream(v)! operator is provided for convenience, in
...
definition it is assumed that an (inside or outside) connector \lstinline[basicstyle=\ttfamily]!c!
contains a stream variable \lstinline[basicstyle=\ttfamily]!h_outflow! which is associated with a flow
variable \lstinline[basicstyle=\ttfamily]!m_flow! in the same connector \lstinline[basicstyle=\ttfamily]!c!:
