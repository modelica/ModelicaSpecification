...
The simulation starts with solving the initialization problem at the starting time giving values for all variables that are consistent with the result of the flattening.
\end{definition}

\begin{definition}[Transient analysis]\index{transient analysis}\index{initial value problem!see{transient analysis}}
Starting from the result of the initialization problem the model is then simulated forward in time.
This generates values for the variables at different times -- and uses numerical methods for handling the hybrid DAE generated by the flattening.
\end{definition}
...
The translation starts with the flattening and includes everything needed for the simulation of the model, but not the simulation itself.
\end{definition}

\begin{nonnormative}
The model can be analyzed in other ways than simulation, e.g., linearization, and parameter estimation, but they are not described in the specification.
...
Typically the translation in addition to flattening involves symbolic manipulation of the hybrid DAE and transforming the result into computer code that can simulate the model.
\end{nonnormative}
