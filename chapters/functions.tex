...
\item
  ...
  In particular, different external functions must not have \lstinline!Include! annotations providing exported definitions of the same funciton symbol to avoid linking errors.
  ...
  \item
    ...
    Include guards should be used (either in the \lstinline!Include! annotation or in the source file) to avoid relying on the deprecated feature that shall not include multiple copies in the same translation unit.
    (Having a \lstinline[language=C]!static! definition allows the same source file to be included by multiple \lstinline!Include! annotations.)
  \item
