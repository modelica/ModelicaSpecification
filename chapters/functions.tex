...
\item
  A deprecated semantics for functions without \lstinline!pure! or \lstinline!impure! keyword is that in the following cases: external functions and functions defined in Modelica directly or indirectly (through functions without the \lstinline!pure! keyword) calling external functions without \lstinline!pure! keyword, they are assumed to be impure, but without any restriction on calling them.
\end{itemize}
