...
In this case, the struct is assumed to have standard padding and the same members in the same order as the record class, and the \lstinline!CStructName! and \lstinline!CName! annotations apply as usual.
...
The annotation \lstinline!CStructName = "c_tag"! specifies that \lstinline[language=C]!c_tag! is the struct tag in the C mapping of the Modelica record class.
By default, the struct tag is the same as the record class name.
A default struct tag which is not a valid in C is allowed, provided that the record class does not appear in the input and output types of any \lstinline!external!-clause.

If multiple record classes with \lstinline!CStructName! annotation have the same \lstinline!c_tag!, the records must be type compatible.
\end{semantics}
\end{annotationdefinition}
