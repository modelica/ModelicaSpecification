...
\item
  ...
  The name and a definition of the struct may be specified using the annnotations in \cref{annotations-for-c-mapping-of-records}.
\item
...
The \lstinline!Include! annotation (providing the struct definition, see \cref{modelica:Include}) is required for record classes which appear in the types of inputs or outputs of an \lstinline!external!-clause.
A deprecated semantics is that the \lstinline!Include! annotation may be omitted.
In this case, the struct is assumed to have standard padding and the same members in the same order as the record class, and any \lstinline!CStructName! or \lstinline!CName! annotations apply as usual.
...
The struct members may be in a different order compared to the record components, and additional padding or members may exist.
When there is no \lstinline!CStructName! annotation (\cref{modelica:CStructName}), it is assumed that the name of the struct is the same as the Modelica record class name.
...
\hline
\hline
{\lstinline!CStructName!} & Name of struct in C & \Cref{modelica:CStructName}\\
...
The \lstinline!Include! and \lstinline!IncludeDirectory! annotations are also used with the same semantics for external functions.

\begin{annotationdefinition}[CStructName]
\begin{synopsis}\begin{lstlisting}
...
The annotation \lstinline!CStructName = "c_name"! specifies that \lstinline[language=C]!c_name! is the name of the struct in the C mapping of the Modelica record class.

By default the struct member names are the same as the record component names, but a different name can be specified using the \lstinline!CName! annotation.
For records with a \lstinline!CStructName! annotation, the \lstinline!CName! annotation is required for components whose name are not valid struct member names in C.

If multiple record classes with \lstinline!CStructName! annotation have the same \lstinline!c_name!, the records must be type compatible.
\end{semantics}
\end{annotationdefinition}
