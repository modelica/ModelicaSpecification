...
\begin{lstlisting}[language=modelica]
function findValue "Returns position of value in array"
  input Integer x[:];
  input Integer val;
...
\end{lstlisting}
The same result could be obtained using \lstinline!break!, but then additional measures would be needed to avoid triggering the assert.
\end{example}
