...
This implies \lstinline!Inline = true! unless overridden by specifying one of the inlining annotations with value \lstinline!true! (in particular, \lstinline!GenerateEvents = true! cannot be combined with \lstinline!Inline = false!).

\begin{nonnormative}
In case a given inlining annotation proposes to inline at a stage when it is too late for a tool to process \lstinline!GenerateEvents = true!, it is recommended to give a diagnostic and instead perform inlining of the function at the nearest stage where \lstinline!GenerateEvents = true! can still be respected.
\end{nonnormative}
As event-generation can be crucial for correct simulation, a diagnostic is strongly recommended in case events will not be generated for a function with `GenerateEvents = true`.
\end{semantics}
\end{annotationdefinition}
