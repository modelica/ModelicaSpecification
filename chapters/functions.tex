...
\item
  ...
  \item
    A small piece of C code defining \lstinline[language=C]!myfoo! as a wrapper around a function in a linked library that does the real job, where the wrapper forwards the arguments as well as passes function pointers for a selection of functions from \filename{ModelicaUtilities.h}.
    This allows the linked library to be built and distributed without direct access to \filename{ModelicaUtilities.h} or any library providing the implementation of \filename{ModelicaUtilities.h}, and when the external function is used with a particular tool, it will use that tool's implementation of \filename{ModelicaUtilities.h}.
  \end{itemize}
  \end{nonnormative}
\item
  The
